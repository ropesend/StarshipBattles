...
class ShipResourceManager:
    ...
    def get_current_fuel(self) -> float:
        ...
        return self._ship.resource_levels.get('fuel', max_fuel)
    ...
    def consume_fuel(self, amount: float) -> bool:
        ...
        stats = self._ship.get_calculated_stats()
        max_fuel = stats.get('resource_storage', {}).get('fuel', 0)
        current = self._ship.resource_levels.get('fuel', max_fuel)

        if current < amount:
            return False

        new_level = current - amount
        self._ship.resource_levels['fuel'] = new_level
        return True
    ...
    def get_current_energy(self) -> float:
        ...
        return self._ship.resource_levels.get('energy', max_energy)
    ...
    def consume_energy(self, amount: float) -> bool:
        ...
        stats = self._ship.get_calculated_stats()
        max_energy = stats.get('resource_storage', {}).get('energy', 0)
        current = self._ship.resource_levels.get('energy', max_energy)

        if current < amount:
            return False

        new_level = current - amount
        self._ship.resource_levels['energy'] = new_level
        return True

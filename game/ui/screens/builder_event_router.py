"""
BuilderEventRouter - Extracted event handling logic from BuilderSceneGUI.

This class handles all event routing for the Ship Builder screen using
...
class BuilderEventRouter:
    """Routes events to appropriate handlers in the Ship Builder.
    
    Uses composition pattern - receives reference to BuilderSceneGUI and
    delegates to its components as needed.
    """
    
    def __init__(self, gui: 'BuilderSceneGUI'):
        """Initialize with reference to the parent GUI.
        
        Args:
            gui: The BuilderSceneGUI instance to route events for.
        """
        self.gui = gui
    ...
    def _handle_button_pressed(self, event) -> bool:
        ...
        elif event.ui_element == gui.test_data_btn:
            gui._load_test_data()
        elif event.ui_element == gui.select_data_btn:
            gui._on_select_data_pressed()
        elif event.ui_element == gui.verbose_btn:

...
class ClickModeDispatcher:
    ...
    def _handle_select_mode_click(self, mx: int, my: int, button: int) -> bool:
        ...
        if button == 1:  # Left Click: Select
        ...
        elif button == 3:  # Right Click: dispatch on current selection, not cursor target.
            # QA Obs 4+6 (2026-05-16): selection-only, strict. Cursor
            # position only positions the menu; what menu opens is
            # determined by ``selected_object``. No auto-selection.
            ...
            if getattr(selected, "owner_id", None) != current_player_id:
                return False
            if is_planet(selected):
                opener = getattr(self.scene.ui, "open_planet_context_menu", None)
                if opener is not None:
                    opener(selected, mx, my)
                    return True
                return False
            if is_fleet(selected):
                self.scene.ui.open_fleet_context_menu(selected, mx, my)
                return True

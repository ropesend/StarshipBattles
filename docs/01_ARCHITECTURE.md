...
| `replay/`        | Battle replay capture + playback (PROJ-312). `IReplayCaptureSink` protocol, `ReplaySpec` / `ReplayOutcome` / `ReplayRecord` JSON-safe DTOs, `replay_serialization.py` to_dict/from_dict pairs (`REPLAY_SCHEMA_VERSION = "1.0.0"`), `replay_player.py` headless playback. Strategy-side persistence (`ReplayStore`, `ReplayResolver`) lives in `game/strategy/services/`. See `combat_simulation.md` § 11 Replay Capture & Playback. |

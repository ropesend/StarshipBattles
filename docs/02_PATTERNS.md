...
> **Last verified:** 2026-04-27 — PROJ-312 Phase 1 extended Pattern #18 (Per-Battle RNG) to cover the AI chain (`AIControllerFactory._rng` → `AIController._rng` → `ErraticBehavior._rng`) and added the AST guard regression contract. Earlier verification: PROJ-297 fixed CLAUDE.md/README references and corrected the body count to 27.

...
- **Per-species sub-block** — when a `view: ColonyDemographicView` is passed, `format_planet_info(planet, view=view)` (`game/ui/screens/strategy_detail_fmt.py`) replaces the legacy single-line `- {race_id}: {count} [+/~/-]` rendering with an indented sub-block per species: header line (race_name, count, happiness category) + 3 metric lines (Habitability + Happiness, Growth + Food ratio, Allocation). Growth is shown as a signed percentage via `format_signed_float(rate * 100, 1) + "% / turn"`. Happiness category labels come from `_happiness_category(happiness)`: `Content` ≥ 1.5, `Settled` ≥ 0.5, else `Unhappy`. Species iteration honours `view.species` ordering (PROJ-288 already sorts largest count first).
- **Per-resource projection grid** — `_build_projection_grid` renders a 5-column table (Resource / Harvest / Upkeep / Yard / Net) with one row per resource in `view.resource_projections`. Cell text comes from the pure helper `_projection_grid_rows(view)` (testable without pygame). Display sign convention: harvest as-is, upkeep + yard rendered as drains via negation, net as-is. Net cell colour from `_net_cell_color`: `HP_HEALTHY` positive, `HP_CRITICAL` negative, `TEXT_LIGHT` zero. A compact `Stockpile: ...` summary line below the grid preserves current/max signal without doubling the grid's vertical footprint.
- **Backward compatibility** — when no view is supplied (legacy callers in `build_queue_panel_factory`, `planet_list_window`, `planet_selection_window` that don't yet thread a facade), both `format_planet_info` and `_build_resource_grid` fall through to their pre-PROJ-289 layouts. The strategy screen wires the facade call from `strategy_detail_formatter._show_planet_report`: `view = facade.get_colony_demographic_view(obj.id)` when `obj.owner_id is not None`.

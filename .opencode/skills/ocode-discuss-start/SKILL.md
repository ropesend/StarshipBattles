...
Build participants per spec Â§1.1: canonical ring `[claude, codex, opencode]`
...
## Step 2 â€” Whitespace warning on the parent leaf
...
## Step 3 â€” Pre-flight: refuse to clobber a leaf-shaped path

If the parent folder already contains files matching the v2.6 message
...
## Step 4 â€” Generate child leaf and folder structure
...
## Step 5 â€” Read optional `<leaf>/topic.md`
...
## Step 6 â€” Compose and write `arc01_001_opencode_to_<P[1]>.md`

The recipient of message 1 is `participants[1 mod n]` = `participants[1]`.

Body must include, in order:

1. **`## User-supplied context`** â€” only if inline context or `topic.md` is
...
2. **`## Turn topology`** â€” required for every arc-starter message
...
3. **Cold-start context** â€” other agents have no shared memory with you. Convey:
...
# OpenCode â†’ <recipient>, message arc01-001
...
**Optional:**
- `agent_turn: <int>` â€” informational only.
- `message_cap: <int>` â€” required iff extension accepted (then `10Ã—n`).
- `extension_requested_cap: <int>` â€” set to propose extension.
- `extension_accepted: true` â€” set when accepting extension.

### Status values

- `continue` â€” keep discussing.
- `consensus` â€” agents have converged. **Does not end alone**; the last `n`
  messages must all be `consensus` for unanimous-terminal termination.
- `needs-user` â€” only the user can answer. Same unanimous rule.
...
## Step 7 â€” Discussion loop

The active per-arc cap starts at `5 Ã— n` (n=2 â†’ 10; n=3 â†’ 15). Extension
takes it to `10 Ã— n`.
...
1. **Wait for the next message addressed to you.** Compute `i_in` = smallest
   ...
   The glob MUST resolve to **exactly one** file. Zero â†’ keep waiting.
   >1 â†’ fork handling.

2. **Branch on what appeared:**
   - `outcome.md` appeared â†’ done; read, summarize, exit.
   - Target message appeared â†’ read and validate per validation rules
...
3. **Apply termination rules** (re-read last `n` messages):
   - **Unanimous terminal**: last `n` messages all carry the same terminal
     ...
     summarize, exit.
   - **Cap reached**: incoming `message_index == active_cap` â†’ cap reached.
     Write `outcome.md`, summarize, exit.

4. **Re-read any plans listed in `## Plans touched`** before composing your
   reply.

5. **Handle extension request**, if any. Active cap starts at `5Ã—n`. Accept
...
7. **Compose your reply.** Status: `continue` / `consensus` / `needs-user`.
   At cap: must use `needs-user` (per spec Â§5.3 â€” a cap is forced stop,
...
1. **Schema**: required fields present; types correct; `from != to`;
   `from`/`to` âˆˆ `{claude, codex, opencode}`.
...
## Step 8 â€” Write outcome.md (exactly once, race-safe)
...
implementation_owners: [<agent>, <agent>]   # required iff owner == multiple, â‰¥2 entries, âŠ† participants
...
- **Canonical ring rotation**: `[claude, codex, opencode]` rotated so opencode
  is at index 0 (always, since opencode is the starter here): for 3-party â†’
  `[opencode, claude, codex]`; for OpenCode+Codex 2-party â†’ `[opencode, codex]`.
...
- **v2.3 readback** (for old Claude+Codex transcripts without `participants`):
  ...
  for v2.3 outcome readback only; v2.6 writers never emit it.

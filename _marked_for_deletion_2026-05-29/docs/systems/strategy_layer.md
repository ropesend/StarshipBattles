...
- **`CommandRegistry`** (metadata) — `game/strategy/engine/commands/registry.py`.
  Holds one `CommandSpec` per Command DTO with all routing metadata
  (handler class, OrderType, category, execution model, facade helper
  name, serializer codec). Populated at module-import via
  `seed_default_commands(command_registry)`. Single source of truth
  for: the runtime dispatch handler set, the OrderType frozensets, the
  action-time ability map, and the facade `dispatch_*` helper names.
- **`CommandHandlerRegistry`** (runtime, above) — instantiated handlers.

The metadata registry is built by `@command_spec(...)` decorators on
each handler class plus a per-module `register(registry)` function:

```python
# game/strategy/engine/handlers/<domain>.py
from game.strategy.engine.commands.registry import (
    CommandRegistry, CommandSpec, command_spec,
)

@command_spec(
    command_class=IssueFooCommand,
...
def register(registry: CommandRegistry) -> None:
    for handler_cls in (FooCommandHandler,):
...
`@command_spec(...)` is **METADATA-ONLY** — it attaches
`__command_spec_kwargs__` to the class and returns the class
unchanged. It does NOT call `command_registry.register(...)` at
import time. Registration is exclusively performed by
`seed_default_commands(registry)`, which imports every handler
module and calls each module's `register(registry)`.

This shape avoids the duplicate-registration foot-gun: re-importing
already-imported modules does not re-run decorators (Python caches
`sys.modules`), so `reset_command_registry()` (clear + reseed) cannot
re-register from the decorator side; only `seed_default_commands`
performs the registration. If both paths registered, a second seed
after a clear would double-register.

#### Adding a new command (authoring rule)

1. Add the Command DTO to `game/strategy/engine/commands/__init__.py`.
2. Add the handler class in
   `game/strategy/engine/handlers/<domain>.py` (or a new domain
   module). Decorate with `@command_spec(...)`.
3. Add the handler to that module's `register(registry)` function (or
   create one if the module is new + add it to
   `seed_default_commands` in `commands/registry.py`).
4. If a NEW `OrderType` is required, add it to
   `game/strategy/data/order_types.py` and the relevant frozenset
   (`MOVEMENT_/ACTION_/PLANET_ACTION_ORDER_TYPES`) — pinned by a
   contract test; drift fails the build.
5. Add tests at `tests/unit/strategy/engine/...`.

You do NOT need to edit `registry_factory.py`,
`action_time_resolver.py`, `command_dispatch_slice.py`, or
`strategy_session_facade.py` — they all derive from
`command_registry`. The strategy session facade auto-installs a bound
`dispatch_<facade_helper_name>` method per spec at module import.

The AST regression test
...
### Order Handlers (PROJ-368)

**Package:** `game/strategy/engine/order_handlers/`

The `OrderProcessor` (`game/strategy/engine/order_processor.py`) is a
thin (~170 LOC) facade. Every public method delegates to a registered
handler in an `OrderHandlerRegistry` keyed by `OrderType`. Adding a
new order type means writing a new handler class and one registration
line in the factory — no `if order.type == ...` ladders on the
processor.

| Module | Handler | Order types |
|--------|---------|-------------|
| `base.py` | `IOrderHandler` (Protocol), `BaseOrderHandler` (event-bus mixin), `OrderHandlerRegistry`, `OrderExecutionResult` | (infrastructure) |
| `join_fleet.py` | `JoinFleetHandler` | `JOIN_FLEET` (single-fleet + BUG-122 batch instant pipeline) |
| `colonize.py` | `ColonizeHandler` | `COLONIZE` |
| `transfer.py` | `TransferHandler` (single instance, three keys) | `TRANSFER`, `LOAD_POPULATION`, `UNLOAD_POPULATION` (with 7 explicit `_dispatch_*` private branches: planet load × {resource, passengers, drop_pod}, planet unload × {resource, passengers, drop_pod}, fleet ↔ fleet) |
| `self_destruct.py` | `SelfDestructHandler` | `SELF_DESTRUCT` (lifted from `superweapon_order_processor.py`) |
| `superweapons.py` | `SuperweaponHandlerAdapter` × 5 | `IMPLODE_PLANET`, `STELLERATE_STAR`, `OPEN_WARP_POINT`, `CLOSE_WARP_POINT`, `CREATE_DYSON_SPHERE` (each adapter wraps the existing `SuperweaponOrderProcessor.process_*` method) |
| `registry_factory.py` | `create_default_order_handler_registry(*, event_bus, superweapon_processor=None)` | (registration entry point) |

**Static guards:** PROJ-368 ships two AST-walker tests that block the
facade from re-accreting old shape:

- `tests/unit/strategy/engine/test_order_processor_no_legacy_helpers.py`:
  asserts `order_processor.py` < 200 LOC, no legacy `_execute_*` /
  `_load_pod_*` / `_unload_pod_*` / `_deploy_drop_pod` /
  `_validate_tick_inputs` / `_elect_canonical_merges` /
  `_emit_join_cancelled` private helpers, no `if order.type ==
  OrderType.X` comparisons.
- `tests/unit/strategy/engine/order_handlers/test_handler_registry_completeness.py`:
  every `OrderType` in `ACTION_ORDER_TYPES \ PLANET_ACTION_ORDER_TYPES`
  + `JOIN_FLEET` has a registered handler.

**Parallel with `engine/handlers/`:** the strategy layer has *two*
registry-based dispatch systems (see `docs/02_PATTERNS.md` for the
cross-reference):

- `engine/handlers/` — **command** handlers (UI Command DTO → Order
  creation). PROJ-309 sub-phase 3.5.
- `engine/order_handlers/` — **order** handlers (action tick → state
  mutation). PROJ-368.

Both follow the same shape: a Protocol typing seam, a `Base*Handler`
mixin, a `*HandlerRegistry`, a `create_default_*_registry()` factory,
and per-domain modules with sibling tests under
`tests/unit/strategy/engine/`.
...
**Warp connectivity guarantee.** For N≥2, `GalaxyWarpGenerator._build_edge_candidates` enumerates *all* unique system pairs and feeds them, sorted by `(distance, i, j)`, into a Kruskal MST. Because `MAX_SYSTEM_COUNT=150` caps the input at 11,175 pairs, the exact-enumeration cost is trivial and connectivity is guaranteed for any system layout — including sparse seeds where the only two systems sit many `SpatialIndex` cell-widths apart. (An earlier k-nearest approximation could leave such systems isolated when the spatial-index expansion heuristic terminated before reaching the far neighbour.)

...
- [x] Implement the enum + class skeleton with property accessors. Methods `generate_bio` etc. raise `NotImplementedError` for now.
...
- [x] Implement using PROJ-296 `LLMBackgroundCall`:
...
- [x] Implement: call `cancel_bio()` first, then `generate_bio()`
- [x] Run tests, confirm pass
...
- [x] Implement: wrap the `call.start()` in try/except LLMConfigError → set ERROR state.
...
- [x] ~12 new tests in `test_race_description_llm_controller.py`

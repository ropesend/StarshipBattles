...
- [x] Implement (pattern: copy `_show_save_update_dialog()` template at race_setup_screen.py:1093-1159):
...
- [x] Implement: small helper `_show_llm_error_popup(error: LLMException)` that maps exception type → message and constructs a UIWindow.
- [x] Run tests, confirm pass
...
### Task 6.4: Manual smoke (USER TASK) [Simple]
**File:** N/A
**Tests:** Manual

These items require a real `DEEPSEEK_API_KEY` and are deferred to the
user. The implementation agent leaves them UNCHECKED.
...
**Notes:** Manual smoke deferred to user. All automated tests pass.
...
- [x] ~6 new tests
- [x] `pytest tests/unit/ui/screens/test_race_setup_screen.py` — all green

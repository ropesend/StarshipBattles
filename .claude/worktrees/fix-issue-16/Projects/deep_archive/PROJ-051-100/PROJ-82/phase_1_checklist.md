...
- [x] Remove the resource block (lines 147-159) from `format_planet_info()`:
  ```python
  ...
  ```
- [x] Verify function still returns valid HTML for all other sections (planet stats, colony status, facilities)
- [x] Run tests to confirm no crashes
...
- [x] Add imports at top of file:
  ...
  import os
  ```
- [x] Add `_load_resource_icons(self, icon_size=24)` method:
...
- [x] Call `_load_resource_icons()` in `__init__` before building grid
...
- [x] Define constant `RESOURCE_PANEL_HEIGHT = 100` at module level
- [x] Add `production_rates: Optional[Dict[str, float]] = None` parameter to `__init__` signature
- [x] Store `self.production_rates = production_rates or {}`
- [x] Reduce `text_h` calculation: change `rect.height - 20` to `rect.height - 20 - RESOURCE_PANEL_HEIGHT` (line ~71)
- [x] Reduce atmosphere graph height: change `graph_h = rect.height - 180` to `rect.height - 180 - RESOURCE_PANEL_HEIGHT` (line ~105)
- [x] If `show_complexes=True`, also reduce complexes container height by `RESOURCE_PANEL_HEIGHT` (line ~83)
- [x] Create resource grid UIPanel at bottom:
  ```python
  ...
  ```
- [x] Add `_build_resource_grid(self)` method that populates the grid:
...
- [x] Track all grid UI elements in `self._resource_grid_items: List` for cleanup
- [x] Add `_update_resource_grid(self)` method to refresh values when planet changes:
  - Kill existing grid items
  - Rebuild grid with new planet data
- [x] Add `production_rates` parameter to `update_planet()` method signature
- [x] Store new `self.production_rates` in `update_planet()`, then call `_update_resource_grid()`
- [x] Call `_build_resource_grid()` in `__init__` after other components
- [x] Update `kill()` method to also kill `self.resource_panel`
- [x] Update `get_height_required()` to return `350 + RESOURCE_PANEL_HEIGHT` (was 350)
...
- [x] Add helper method `_compute_planet_production(self, planet) -> Dict[str, float]`:
  ```python
  def _compute_planet_production(self, planet):
      ...
      return rates
  ```
- [x] Pass `production_rates=self._compute_planet_production(obj)` when creating PlanetReportPanel (~line 646)
- [x] Pass `production_rates=self._compute_planet_production(planet)` in any `update_planet()` calls (N/A - no update_planet calls in codebase)
...
- [x] Run `pytest tests/integration/ui/test_build_queue_enhanced_planet_report.py` — all pass (18 passed)
- [x] Run `pytest tests/ --testmon` — no regressions (7342 passed)

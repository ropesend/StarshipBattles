...
- [x] Create new module with module docstring explaining purpose
- [x] Move `_load_portrait_thumbnail(design, size=50)` as `load_portrait_thumbnail(design, size=50)` (drop leading underscore - now public API)
...
- [x] Move `_load_topdown_thumbnail(design, target_height=50)` as `load_topdown_thumbnail(design, target_height=50)`
...
- [x] Move `_get_visible_bounding_box(surface)` as `_get_visible_bounding_box(surface)` (keep private - internal helper)
...
- [x] Verify the module has no dependency on UIWindow, pygame_gui, or any UI state

**Notes:** Fixed a boundary condition bug in _get_visible_bounding_box: changed `max_x <= min_x` to `max_x < min_x` to correctly handle single-pixel visible areas.

---

### Task 1.2: Write unit tests for design_image_helper.py [Simple]
**File:** `tests/unit/ui/screens/test_design_image_helper.py`
- [x] Create test file with `TestLoadPortraitThumbnail` class:
...
- [x] Create `TestLoadTopdownThumbnail` class:
...
- [x] Create `TestGetVisibleBoundingBox` class:
...
- [x] All tests must initialize pygame display: `pygame.init(); pygame.display.set_mode((1, 1), pygame.NOFRAME)` in setup_method

**Notes:** 13 tests created, all passing.

---

### Task 1.3: Update DesignSelectorWindow to delegate to helper [Simple]
**File:** `game/ui/screens/design_selector_window.py`
- [x] Add import: `from game.ui.screens.design_image_helper import load_portrait_thumbnail, load_topdown_thumbnail`
- [x] Replace `_load_portrait_thumbnail` method body with delegation:
  ```python
  def _load_portrait_thumbnail(self, design, size=50):
      return load_portrait_thumbnail(design, size)
  ```
- [x] Replace `_load_topdown_thumbnail` method body with delegation:
  ```python
  def _load_topdown_thumbnail(self, design, target_height=50):
      return load_topdown_thumbnail(design, target_height)
  ```
- [x] Remove `_get_visible_bounding_box` method entirely (no longer called from this class)
- [x] Remove any imports that are no longer needed (e.g., `os` if only used by image methods)
- [x] Verify `os` import: check if it is still used elsewhere in the file before removing
- [x] Run existing tests: `pytest tests/integration/ui/test_design_selector.py` - all must pass unchanged

**Notes:** Removed `os` and `log_warning` imports from design_selector_window.py as they're no longer needed.

---

## Phase Completion Checklist
- [x] All task checkboxes above are checked
- [x] `pytest tests/integration/ui/test_design_selector.py` passes (existing tests)
- [x] `pytest tests/unit/ui/screens/test_design_image_helper.py` passes (new tests)
...
- **New design_image_helper.py:** 196 lines
- **New tests:** 13 tests added
- **Full test suite:** 7570 passed

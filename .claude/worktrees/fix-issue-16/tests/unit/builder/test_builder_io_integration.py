...
class TestBuilderIOIntegration:
    ...
    def _create_ship_io_integrated(self):
        """Create a WorkshopShipIO configured for integrated (in-game) mode."""
        context = MagicMock()
        context.mode = WorkshopMode.INTEGRATED
        context.savegame_path = "fake/savegame/path"
    ...
    def test_select_target_integrated_calls_set_target_on_panel(self):
        """Integrated-mode select_target must call set_target on the weapons report panel.

        Regression test for issue #16: the integrated branch previously referenced
        a non-existent attribute `_weapons_report_panel_ref` instead of calling
        `_get_weapons_report_panel()`, crashing the run loop with AttributeError.
        """
        (
            ship_io,
            design_loader_adapter,
        ...
        with patch(
            "game.ui.screens.workshop_ship_io.DesignLibrary",
            return_value=mock_library,
        ), patch(
            "game.ui.screens.workshop_ship_io.DesignSelectorWindow"
        ) as mock_selector_cls:
            ship_io.select_target()

            # Capture the on_select_callback the screen registered with the selector window
            assert mock_selector_cls.called, "DesignSelectorWindow was not constructed"
            on_target_selected = mock_selector_cls.call_args.kwargs["on_select_callback"]

            # Simulate the user clicking Select on a row
            on_target_selected("design_42")

        # The fix: weapons report panel's set_target must be called with the loaded ship

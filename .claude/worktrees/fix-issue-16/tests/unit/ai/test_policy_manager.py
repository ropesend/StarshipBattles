"""
Unit tests for PolicyManager module-level accessor pattern.

Tests the accessor behavior, direct construction, data clearing, and lazy loading.

Replaces test_strategy_manager_singleton.py for the new PolicyManager.
"""
...
class TestPolicyManagerAccessor:
    """Tests for module-level accessor pattern behavior."""

    def test_get_default_returns_same_object(self):
        """Test that get_default_policy_manager() returns the same object on multiple calls."""
        from game.ai.policy_manager import get_default_policy_manager

        instance1 = get_default_policy_manager()
        instance2 = get_default_policy_manager()

        assert instance1 is instance2, "get_default_policy_manager() should return same object"
    ...
    def test_direct_init_creates_new_instance(self):
        ...
        default = get_default_policy_manager()
        new_instance = PolicyManager()
    ...
    def test_clear_preserves_instance(self):
        ...
        instance1.movement_policies = {'move1': {}}
    ...
    def test_clear_resets_loaded_flag(self):
        ...
        instance = get_default_policy_manager()

        # Mark as loaded
        instance._loaded = True
        instance.targeting_policies = {'test': {}}

        # Clear should reset loaded flag
        instance.clear()

        assert instance._loaded is False
        assert instance.targeting_policies == {}


class TestPolicyManagerThreadSafety:
    """Tests for thread-safe accessor creation."""

    def test_concurrent_accessor_calls_return_same_object(self):
        """Test that concurrent calls to get_default_policy_manager() all get the same object."""
        from game.ai.policy_manager import get_default_policy_manager
        ...
        def get_instance():
            try:
                inst = get_default_policy_manager()
                results.append(inst)
...
class TestPolicyManagerDataLoading:
    """Tests for data loading behavior."""

    def test_load_data_populates_policies(self):
        """Test that load_data populates policy dictionaries."""
        from game.ai.policy_manager import get_default_policy_manager
...
class TestPolicyManagerLazyLoading:
...
class TestPolicyManagerDefaults:

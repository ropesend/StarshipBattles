...
Usage:
    python Projects/complexity_loop/trim_execution_log.py
    python Projects/complexity_loop/trim_execution_log.py --keep 20
"""
...
DEFAULT_PLAN = WORKSPACE / "Projects" / "complexity_loop" / "cycle_plan.md"
DEFAULT_ARCHIVE = WORKSPACE / "Projects" / "complexity_loop" / "cycle_execution_archive.md"
DEFAULT_KEEP = 20
...
def main():
    parser = argparse.ArgumentParser(
        description="Trim execution log in cycle_plan.md"
    )
    parser.add_argument(
        "--plan-file",
    ...
    parser.add_argument(
        ...
        help=f"Number of recent rows to keep (default: {DEFAULT_KEEP})",
    )
    parser.add_argument(
        "--archive",
        default=str(DEFAULT_ARCHIVE),
        help=f"Archive file path (default: {DEFAULT_ARCHIVE})",
    )
    args = parser.parse_args()

    stats = trim_execution_log(Path(args.plan_file), args.keep, Path(args.archive))

    if stats["log_rows_archived"] > 0 or stats["projects_stripped"] > 0:
        print(
            f"Trimmed: {stats['log_rows_archived']} rows archived, "
            f"{stats['projects_stripped']} projects stripped, "

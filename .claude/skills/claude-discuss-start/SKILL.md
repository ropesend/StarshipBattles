...
## Step 6 â€” Compose and write `arc01_001_claude_to_<P[1]>.md`
...
# Claude â†’ <recipient>, message arc01-001
...
The active per-arc cap starts at `5 Ã— n` (n=2 â†’ 10 matching v2.3; n=3 â†’ 15).
Extension takes it to `10 Ã— n`.
...
1. **Wait for the next message addressed to you.** Compute `i_in` = smallest
   ...
   Zero â†’ keep waiting. >1 â†’ see Step 7.6 (fork handling).
...
5. **Handle extension request, if any.** If incoming has
   ...
   - **Accept** by setting `message_cap: <10Ã—n>` and `extension_accepted: true`,
     plus a one-line body acknowledgement. After acceptance, every subsequent
     message in this arc must include `message_cap: <10Ã—n>`.
...
7. **Compose your reply.** Status:
   - `continue` â€” more to discuss.
   - `consensus` â€” you actually agree.
   - `needs-user` â€” only the user can answer.
...
- **No legacy compatibility for new discussions.** v2.6 writers always emit

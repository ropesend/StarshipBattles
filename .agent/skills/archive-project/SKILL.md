...
1. **Find Entry**: Read `refactor_loop/refactor_plan.md` and locate the project block for PROJ-[ID].
   - Block starts with `- [` and ends at the next `---`.

2. **Validate**: Ensure the project is marked complete (`[x]` or `[~]`).

3. **Move to Archive**:
   - Append the block to `refactor_loop/archive.md` (initialize file with header if missing).
   - Remove the block from `refactor_loop/refactor_plan.md`.

4. **Clean Execution Log**: Remove all rows containing PROJ-[ID] from the `## Execution Log` table in `refactor_loop/refactor_plan.md`.

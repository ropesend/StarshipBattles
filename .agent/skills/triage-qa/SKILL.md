...
1. **For Bugs:**
   Follow the instructions in the `add-bug` skill (`.agent/skills/add-bug/SKILL.md`). You will provide the cleaned-up descriptions and image links as the "raw description" to this skill so it can generate the `BUG-XX.md` tickets.
2. **For Projects:**
   Do NOT use the `/start-project` skill automatically. Creating a project is a heavy cognitive task that the user handles manually.

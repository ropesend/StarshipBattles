...
This file is the Claude-specific startup adapter for Starship Battles. It keeps
the rules Claude Code must see immediately, then points to the shared policy and
documentation that own the details.

Read `AGENTS.md` first for shared project policy. Read this file before using
...
Claude usage logging is automatic for `claude-*` skills through hooks in
`.claude/settings.json`:

- `UserPromptExpansion` records typed `/claude-<name>` commands.

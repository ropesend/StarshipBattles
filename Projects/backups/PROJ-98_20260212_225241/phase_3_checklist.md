...
- [x] Add `NUMERIC_COLUMNS` set at module level containing column IDs that sort numerically:
...
- [x] Add `sort_sources(self, sources, sort_column_id, sort_descending, get_column_value_fn)` method:
...
**Notes:** Implemented with proper handling for both numeric and string columns. "-" values sort to end.

### Task 3.2: Write sort tests [Medium]
**File:** `tests/unit/ui/screens/test_empire_build_queue_filter_manager.py`

- [x] Add `TestSortSources` class with these tests:
...
**Notes:** All 8 tests passing.

### Task 3.3: Integrate ColumnManager into the window [Medium]
**File:** `game/ui/screens/empire_build_queue_window.py`

- [x] Add import: `from game.ui.screens.planet_list_columns import ColumnManager` (top of file)
- [x] In `__init__()` (around line 145): replace `self._build_header_labels(manager, main_w)` with:
  ```python
  ...
  ```
- [x] Remove `_build_header_labels()` method entirely (lines 177-193)
- [x] Remove `_header_labels` attribute references (check `kill()` and `_handle_column_toggle_click()`)
- [x] Add `_apply_sort_and_refresh()` helper method:
  ```python
...
**Notes:** Removed _build_header_labels, updated kill(), header_labels is now managed by ColumnManager.

### Task 3.4: Add sort polling to update() [Simple]
**File:** `game/ui/screens/empire_build_queue_window.py` (lines 485-491)

- [x] In `update()`, after `super().update(time_delta)`, add:
  ```python
  ...
  ```
- [x] Keep existing scrollbar check below

**Notes:** Added before scrollbar check.

### Task 3.5: Update column toggle to use ColumnManager [Simple]
**File:** `game/ui/screens/empire_build_queue_window.py` (line 650)

- [x] In `_handle_column_toggle_click()`: replace `self._build_header_labels(self.ui_manager, 0)` with `self.column_mgr.rebuild_headers()`
...
- [x] After `self.filtered_sources = self._filter_sources(self.all_sources)` and before `self._refresh_list()`, insert:
  ```python
  self._filter_mgr.sort_sources(
      self.filtered_sources,
      self.column_mgr.sort_column_id,
...
- [x] Add `self.column_mgr.kill()` before `super().kill()`
- [x] Remove any `_header_labels` cleanup if still present

**Notes:** Added column_mgr.kill().

### Task 3.8: Write integration tests [Medium]
**File:** `tests/unit/ui/screens/test_empire_build_queue_window.py`

- [x] Update `_make_window()` helper to create `column_mgr` mock (or real ColumnManager with mocked UI)
- [x] Add `TestColumnSortingAndReorder` class:
...
**Notes:** 4 new integration tests added, all passing.

### Task 3.9: Full test run [Simple]
**Tests:** `pytest tests/ -n 12`

- [x] All 7648 tests pass (7636 + 12 new)
- [x] No regressions

**Notes:** 7648 passed, 5 warnings (font warnings, acceptable).

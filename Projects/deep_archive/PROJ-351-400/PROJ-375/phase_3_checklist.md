...
- [x] Define dropdown-handler config (e.g., `dict[str, DropdownConfig]`) capturing options-source, resolver, viewmodel-setter, optional confirmation
- [x] Replace `_handle_movement_dropdown` (line 493) with a config-driven dispatch entry
- [x] Replace `_handle_targeting_dropdown` (line 505) with a config-driven dispatch entry
- [x] Replace `_handle_role_dropdown` (line 517) with a config-driven dispatch entry (uses registry-loop resolver)
- [x] Replace `_handle_class_dropdown` (line 441) with a config-driven dispatch entry (with confirmation dialog config)
- [x] Replace `_handle_vehicle_type_dropdown` (line 464) with a config-driven dispatch entry (with confirmation dialog config)
- [x] Update event-router dispatch to call the unified handler
- [x] Verify: `pytest tests/unit/ui/screens/test_workshop_event_router.py` passes
- [x] Verify: LOC delta ≈ -50
...
- [x] Decide: extend `DataListWindowMixin` (recommended) vs new `ListWindowUpdateMixin` sibling — record choice in `decisions.md`
- [x] Implement the shared `_update_template(slider_fields, column_manager, preset_manager)` (or equivalent) in the chosen home
- [x] Implement shared `_set_all_filters` / `_toggle_filter` in the same place
- [x] Migrate `PlanetListWindow.update()` (lines 541-573) to call the shared template; keep its `_sync_slider_text` and filter-helper specifics
- [x] Migrate `StarListWindow.update()` (lines 389-421) to call the shared template; keep its `_sync_slider_text` and filter-helper specifics
- [x] Verify: `pytest tests/unit/ui/screens/test_planet_list_window.py tests/unit/ui/screens/test_star_list_window.py` passes
- [x] Verify: LOC delta ≈ -60
...
- [x] Choose extraction shape: shared static helper vs mixin (record in `decisions.md`)
- [x] Implement shared `_rebuild_modifier_icons` in chosen location (private module-level function or `ComponentItemBase` mixin)
- [x] Replace `IndividualComponentItem._rebuild_modifier_icons` (lines 195-237) with delegating call
- [x] Replace `LayerComponentItem._rebuild_modifier_icons` (lines 472-514) with delegating call
- [x] Verify: `pytest tests/unit/ui/screens/builder/test_structure_list_items.py` passes
- [x] Verify: LOC delta ≈ -40

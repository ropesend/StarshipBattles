...
|------|------|-------|-------|
| `game/strategy/services/replay_ship_builder.py` | Production (modify) | 0 | Replace `registries = registry_provider.get_registries()` with explicit `GameRegistries(...)` construction using individual protocol getters. Mirrors `build_context_ship_builder` at `battle_runner.py:240-247`. |
| `tests/unit/strategy/services/test_replay_ship_builder_registry_contract.py` | Test (new) | 0 | TDD failing test pinning that `build_replay_ship_builder` accepts a `DefaultRegistryProvider` (or `TestRegistryProvider`) and does not call non-existent `get_registries()`. |
| `game/app_bootstrap.py` | Production (modify) | 1, 2 | Construct `ReplayStore`, call `set_default_capture_sink(store)` + `set_replay_store(store)` (Phase 1); construct + `start()` `ReplayVerificationCoordinator` with Combat Lab fallback adapter; extend `BootstrapResult` with `replay_store` + `replay_verification_coordinator` fields (Phase 2). Wrap each in `_timed_phase` blocks. Insertion point: after `InputMapper.load(...)`, before profiler total/save_history/return. |
...
| `tests/unit/test_app_bootstrap_invariants.py` | Test (modify) | 1, 2 | Add Invariant 7: sink + store registered after `InputMapper.load(...)`. Add Invariant 8: `replay_verification_coordinator.start()` is called and the listener is registered. Add autouse fixture: `shutdown_all_coordinators(timeout=5.0)` + `reset_default_capture_sink()` + `set_replay_store(None)` after each test. Patch SOURCE modules, not lazy-import sites. |
| `tests/unit/test_app_bootstrap_profiling.py` | Test (modify) | 1 | Add the same autouse cleanup fixture (the new bootstrap wiring spawns a non-daemon worker thread that would leak between tests). |
| `tests/unit/test_run_loop_shutdown_ordering.py` | Test (new) | 2 | Pin `shutdown_all_calls` → `shutdown_all_coordinators` → `pygame.quit()` order via SOURCE-module monkeypatches. Test name `test_shutdown_order_llm_then_coordinator_then_pygame` makes a future flip loud. |
...
| `tests/integration/replay/test_combat_lab_verification.py` | Test (new) | 4 | Combat Lab synthetic record (`instance_snapshot=None` for all ships) + fallback wired (`DesignOnlyMaterializer` adapter) → sidecar `status=PASSED`. No fallback wired → sidecar `status=ERROR` with diagnostic message. |

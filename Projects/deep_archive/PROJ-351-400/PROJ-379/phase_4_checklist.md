...
- [x] Append a row dated 2026-05-XX (closeout date) to `Projects/active_projects/PROJ-377/decisions.md`:
...
- [x] **Verify:** PROJ-377 plan.md and decisions.md remain mutually consistent — no stale "MIN-002 deferred" claims.
...
- [x] Run pre-flight: confirm review daemon is running per `.claude/skills/claude-delegate-review/SKILL.md` (check PID file at `AgentCoordination/opencodereview/local/review_daemon.pid`).
- [x] Compose review request payload listing in-scope commits (Phase 1, 2, 3 SHAs), in-scope files (`_build_galaxy_fixture.py`, regenerated JSONs, `test_save_round_trip.py` modifications, `test_golden_fixture_field_coverage.py`, deleted `_capture_baseline.py`, PROJ-377 cross-link), and 6-8 specific focus areas:
...
- [x] Submit via `python Tools/agent_coordination/create_review_request.py --payload-file <path>`. Capture request id.
- [x] Update PROJ-379 `decisions.md` with the review request id.
...
- [x] Wait for OpenCode review to complete (poll `AgentCoordination/opencodereview/completed_review_requests/req_<id>.md`).
- [x] Read the report; spawn a verifier subagent (`general-purpose`) with the standard verifier prompt template (see `Projects/protocols/03c_phase_aware_execution.md` or prior project closeouts for the canonical shape):
...
- [x] Read the verifier's summary; apply confirmed "fix now" remediations; defer "fix later" items.
...
- [x] Apply each verifier-confirmed "fix now" recommendation. Run focused tests after each.
- [x] Update `Projects/active_projects/PROJ-379/decisions.md` with a final closeout row capturing: review request id, finding counts (CRIT/MAJ/MIN/INFO), remediations applied vs. deferred, links to `report.md` and `verifier_report.md`.
- [x] Sharded suite green at HEAD.
- [x] Commit: `PROJ-379 review remediation: <summary>` (only if remediations applied) OR `PROJ-379 closeout: log OpenCode review + verifier outcome` (if no remediations needed).
...
- [x] Update Quick Status row 4 (Phase 4) to `Complete`.
- [x] Update Current State to "All phases complete; ready for user verification" or similar.
- [x] Update Audit Log with cycle 1 (date, OpenCode finding count, resolution summary).
- [x] Update Completion Checklist boxes that are now achievable.
...
- [x] PROJ-377 `decisions.md` has the cross-link row pointing at PROJ-379.
- [x] OpenCode review dispatched, completed, verified.
- [x] Remediations applied (if any) or deferred with rationale.
- [x] Sharded suite green at HEAD; pass count = pre-PROJ-379 baseline + 7.
- [x] PROJ-379 `decisions.md` final closeout row appended.

...
- [x] Write tests for PlanetDataSource:
...
- [x] Create `PlanetDataSource(ITableDataSource)`:
...
- [x] Verify: `pytest tests/unit/ui/screens/test_planet_data_source.py -v` passes

**Notes:** 29 tests created and passing
...
- [x] Update `update()` method (header press checks):
...
- [x] Update sort_planets calls to use `self.column_manager.sort_column_id` and `self.column_manager.sort_descending`
- [x] Keep preset system logic (capture/apply state):
...
- [x] Update existing tests (mock structures may need adjustment)
- [x] Verify: `pytest tests/unit/ui/screens/test_planet_list_components.py tests/unit/ui/screens/test_planet_list_filters.py -v` passes

**Notes:** All 68 planet list tests passing
...
- [x] Run `pytest tests/unit/ui/ -v` — all UI tests pass (3125 passed)
- [x] Run `pytest tests/ -n 12` — no regressions (12,601 passed, 1 skipped)
- [x] Verify `planet_list_window.py` no longer imports from `planet_list_renderer` or `planet_list_columns`
- [x] Verify `VirtualListRenderer` has 0 reverse dependencies
- [x] Verify `planet_list_columns.ColumnManager` only imported by `empire_build_queue_window.py` (migrated in Phase 4)

**Notes:** Full test suite: 12,601 passed (+29 new from PlanetDataSource)
...
- [x] PlanetListWindow uses VirtualTable + PlanetDataSource + SingleSelect
- [x] All planet list tests pass (or adapted equivalents)
- [x] PlanetDataSource has comprehensive tests (func, attr, fmt, images)
- [x] Preset system still works with new TableColumnManager

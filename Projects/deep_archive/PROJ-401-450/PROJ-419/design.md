...
- **This bundle:** 5 verified, 0 uncertain (resolved), 0 INFO (resolved), 0 deferred.
- **Project siblings:** PROJ-413, PROJ-414, PROJ-415, PROJ-416, PROJ-417, PROJ-418, PROJ-420, PROJ-421.
- **Cluster identity:** `light_cleanup`.
- **Severity breakdown:** 5× MINOR.

### Quick Wins
This project bundles 5 zero-call-site fixes (stale comments at 4 sites and 3
dead `import pygame_gui` lines in `screen_router.py`). All fit into a single
small PR with no caller migration required.

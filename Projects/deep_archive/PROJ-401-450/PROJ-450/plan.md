...
| 2. Substrate widening (`_staging_yard: List[CarriedVehicle \| DropPod]`) + serde normalization | Complete | [phase_2_checklist.md](phase_2_checklist.md) |
| 3. UI reader migration + DTO / validator / write-service tightening | Complete | [phase_3_checklist.md](phase_3_checklist.md) |
| 4. Integration test migration (9+ `.append`/`.extend` sites in `test_fms_planet_*`) | Complete | [phase_4_checklist.md](phase_4_checklist.md) |
| 5. Static guard updates (type-pin tightening) | Complete | [phase_5_checklist.md](phase_5_checklist.md) |

## Current State
**Last Updated:** 2026-05-19
**Active Phase:** All phases complete; codex audit next
**Last Action:** Phase 5 complete. Added `test_planet_staging_yard_substrate_is_typed_not_dict` to `tests/static_guards/test_no_legacy_storage_fields.py`. Guard exercises BOTH the typed-input path (`add_to_staging_yard(CarriedVehicle(...))`) AND the dict-input path (`add_to_staging_yard({...})`), then asserts every entry in `planet._staging_yard` `isinstance` of `CarriedVehicle | DropPod`. Any future regression that bypasses the public API to `.append(legacy_dict)` is caught at static-guard time. Sharded 23412/23412 GREEN. F-B-013 fully closed; DI-2026-05-18-001 substrate half fully closed.

Phase 4 complete. Migrated 6 direct `.append`/`.extend` mutations across `test_fms_planet_lay_mines.py` (3 sites) and `test_fms_planet_launch.py` (3 sites). Renamed dict helpers `_mine_dict`/`_fighter_dict`/`_satellite_dict` → typed `_mine_typed`/`_fighter_typed`/`_satellite_typed` returning `CarriedVehicle`. Mutation sites now call `planet.add_to_staging_yard(typed_helper())`. `_StubPlanet.add_to_staging_yard` methods migrated to isinstance-aware mass access. `test_fms_planet_recovery.py` already typed-aware from Phase 1; `test_fms_a_e2e.py` `.clear()` already migrated in Phase 2.0. Sharded 23411/23411 GREEN. Phase 4 closes BLOCKER #2.

Phase 3 complete. UI reader at `strategy_detail_fmt.py:285-297` migrated to typed `CarriedVehicle`/`DropPod` access with dict-mock backward-compat branch. Phase-2 dict-projection bridge **REPLACED** with permanent typed read-only property `Planet.staging_yard -> tuple[CarriedVehicle | DropPod, ...]` (no setter; AttributeError on `planet.staging_yard = X`). Validator simplified (only `isinstance(item, CarriedVehicle)` branch remains; DropPod silently skipped per protocol). `IPlanetMutator.add_staging_item` + `pop_staging_item` annotations tightened to the typed union. `planet_slice` + `planet_dto` migrated to typed attribute access. 2 new UI tests + 4 test-file migrations from dict-subscript to typed access. `DropPod` lives at `game/strategy/data/bay_inventory.py`. Two `# Intentional:` markers converted to `# Intentional broad catch: <reason>` per convention. Sharded 23411/23411 GREEN. F-B-013 substrate fully closed.

Phase 2 complete. Substrate widened: `Planet._staging_yard: List[CarriedVehicle | DropPod]`. Added `__post_init__` normaliser for kwarg construction. `planet_serde._normalize_to_typed()` promotes save-shape dicts on load. `planet_to_dict` emits per-entry `.to_dict()` on save. `IStagingYardHolder` protocol typing tightened. `production_spawner` constructs typed `CarriedVehicle`/`DropPod` directly. Save fixture `galaxy_proj372_populated.json` regenerated (drop-pod entries now have `payload` nested instead of flat `name`/`vehicle_type`/`owner_id` at top level). Temporary `staging_yard` dict-projection bridge property installed (Phase 3 deletes). 3 new typed-substrate tests + 5 test file migrations to typed assertions. LOC ceilings raised: planet.py 485→525, galaxy_protocols.py 210→230. Phase 3 bridge-delete will let them drop again. Sharded 23409/23409 GREEN.

Phase 1 complete. Path A engine API cleanup landed. Moved 3 helpers (`_is_carried_vehicle_dict`, `_pod_from_dict`, `_staging_yard_carried_vehicle`) from `transfer_branches.py:41-87` to `planet.py` as module-level private helpers. Widened `Planet.add_to_staging_yard()` to accept `Dict | CarriedVehicle | DropPod` (normalizes to legacy dict shape internally; substrate still `List[Dict[str, Any]]` until Phase 2). Added `Planet.pop_staging_yard_typed(index) -> CarriedVehicle | DropPod | None`. Dropped flatten/inflate calls in `transfer_branches.py:412, 454-460` and `issuer_adapter.py:363`. 9 new tests (8 unit + 1 integration). LOC: planet.py 477 (+52, ceiling raised 425→485); transfer_branches.py shrank correspondingly. Sharded 23406/23406 GREEN. Substrate stays dict-typed; Phase 2 widens.
...
**Next Action:** Dispatch end-of-project codex audit per protocol §10. After response, address any verified issues, then merge to main.

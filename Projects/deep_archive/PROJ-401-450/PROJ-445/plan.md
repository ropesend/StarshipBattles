...
| 2. Engine boundary tightening (DI-001/006/007 fix sites + transfer_branches private slots) | Complete (partial — private-slot migration + Fleet fractional ratchet deferred to joint phase with PROJ-444 F-A-007 / F-A-010) | [phase_2_checklist.md](phase_2_checklist.md) |
...
**Active Phase:** Phase 3 (queued — not started)
**Last Action:** Phase 2 complete with two documented partial deferrals (see decisions.md row 2026-05-18 #5). Closed in this phase: **Task 2.1 (F-B-002)** — `_dispatch_carried_vehicle_load` rollback now routes through `planet.add_to_staging_yard` instead of the raw `staging_yard.append`, preserving the `max_staging_mass` invariant against any future capacity-rule changes; regression test in [`test_transfer_handler.py`](../../../tests/unit/strategy/engine/order_handlers/test_transfer_handler.py) confirmed via git-stash RED-GREEN cycle. **Task 2.3 (F-B-014)** — audit confirmed no plain-string CLOSE_WARP_POINT emitters; both legacy branches in `_parse_close_target` and `superweapon_order_processor.py` retired; legacy back-compat test rewritten as `TestCloseWarpPointTargetShape`; updated `test_close_warp_point_no_destination` in [`test_superweapon_edge_cases.py`](../../../tests/unit/strategy/engine/test_superweapon_edge_cases.py) for the new "Invalid warp point params" structural-rejection message + companion test for the precheck's "No destination" path. **Task 2.5 (DI-2026-05-18-002)** — `CommandRegistry.serializer_codec_for` now raises `ValueError` on multi-spec ambiguity (option a chosen, recorded in decisions.md); two new ratchet tests in [`test_order_persistence_from_metadata.py`](../../../tests/unit/strategy/engine/test_order_persistence_from_metadata.py) — one for the failure mode, one for the matching-codec multi-spec pass-through. **Task 2.4 partial (F-B-019)** — `IProductionResourceSource.production_consume_resource` Protocol docstring tightened with the explicit affordability/consumption symmetry clause; new contract ratchet file [`test_production_resource_source_contract.py`](../../../tests/unit/strategy/engine/test_production_resource_source_contract.py) covers Planet (whole + fractional) + Fleet (empty-baseline `False → False`). **Task 2.2 partial (F-B-003 + DI-2026-05-18-001 fleet-to-fleet half)** — `_dispatch_fleet_to_fleet` now dispatches on `cargo_type` before falling through to the generic-cargo path; two new helpers `_dispatch_fleet_to_fleet_drop_pod` and `_dispatch_fleet_to_fleet_vehicle` cover the previously-silent-no-op cases; three new tests cover success path, both cargo types, and dest-full edge. Full sharded suite green: 23357 passed, 0 failed. `python Projects/scripts/validate_phase.py PROJ-445 2` reports PASSED. **Deferred to a joint-phase work item with PROJ-444 Phase 2 (F-A-007 + F-A-010):** (a) migrate the 9+ `ship._cargo_mgr.X(...)` call sites in `transfer_branches.py` to the future public `ship.X(...)` delegators; (b) extend the IProductionResourceSource contract ratchet with Fleet-stocked-fractional fixture cases once PROJ-444 ships the rounding fix. PROJ-444 Phase 2 had not started at this writing.

_Phase 1 hand-off retained for traceability:_ F-B-001 fixed (`LayMinesOrderHandler.execute_for_issuer` accepts `registries: Any = None`); parametrised planet-FMS dispatch integration test at [`tests/integration/test_fms_planet_lay_mines.py`](../../../tests/integration/test_fms_planet_lay_mines.py); extended contract ratchet at [`test_issuer_execution_contract.py`](../../../tests/unit/strategy/engine/test_issuer_execution_contract.py) covering LayMines + LaunchSatellites; subcategory-tag spelling ratchet at [`test_command_registry_contract.py`](../../../tests/unit/strategy/engine/test_command_registry_contract.py). F-B-001 fixed: `LayMinesOrderHandler.execute_for_issuer` now accepts `registries: Any = None` (mirrors `recover_fighters.py:107-120`). Added integration test [`tests/integration/test_fms_planet_lay_mines.py`](../../../tests/integration/test_fms_planet_lay_mines.py) parametrised across all 5 planet-FMS order types, driven through `ActionExecutionEngine._execute_planet_action` (catches the registries-kwarg drift on any future handler that ships in the same shape). Extended contract ratchet at [`tests/unit/strategy/engine/test_issuer_execution_contract.py`](../../../tests/unit/strategy/engine/test_issuer_execution_contract.py) with `test_lay_mines_signature_accepts_galaxy_and_registries` + `test_launch_satellites_signature_accepts_galaxy_and_registries` (closes the PROJ-438 ratchet parity gap Codex flagged). Added `test_planet_fms_subcategory_tag_spelling_or_set_size` at [`tests/unit/strategy/engine/test_command_registry_contract.py`](../../../tests/unit/strategy/engine/test_command_registry_contract.py) locking the planet-FMS surface at 5 entries and catching typo regressions (verified by temporarily mutating `"planet_fms"` → `"planet-fms"` on `handlers/lay_mines.py:40`, observing the test fail, then reverting). Full sharded suite green: 23326 passed, 0 failed. `python Projects/scripts/validate_phase.py PROJ-445 1` reports PASSED.
**Next Action:** **PHASE 3** — annotation + ratchet test polish. The phase scope is small, mechanical, and contained entirely inside `game/strategy/engine/` + `game/strategy/services/`: (1) F-B-006 annotate `_get_system_at_hex`, drop `# type: ignore`; (2) F-B-007 + F-B-008 type `event_bus` parameters on engine `__init__`s; (3) F-B-009 annotate `resolve_requested` return as `int | ValidationResult`; (4) F-B-010 annotate `TurnEngine.planet_modifier_effect_engine`; (5) F-B-011 annotate 4 `_get_*_mutator` accessors as `-> Any`; (6) F-B-012 delete the two dead `try/except ImportError → pytest.skip` guards in `test_superweapon_registry_contract.py`; (7) F-B-015 fix stale `_cargo_contents` reference in `IProductionResourceSource` docstring → `ShipCargoManager`; (8) F-B-016 drop "Phase 7 deletes the legacy path" stale docstring in `conflict_modifier_collection.py`; (9) F-B-021 annotate `ReplayStore._iter_replay_files`. Each is a touch of a few lines.

**Blockers:** None for Phase 3 (no cross-bucket dependencies). The joint-phase work item with PROJ-444 (F-A-007 ShipInstance delegators + F-A-010 Fleet rounding) is queued — it does NOT block Phase 3 or Phase 4 here, but it is the only way to complete the partial deferrals from Phase 2 (private-slot migration sweep + Fleet stocked-fractional ratchet case). Surface that joint phase when both projects' Phase-2 work is in (PROJ-444 needs to start Phase 2 first; their current state shows Phase 1 complete, Phase 2 not started).

> **STOP CONDITION REACHED:** Phase 2 is complete to the extent it can be without cross-bucket coordination. A future session must explicitly trigger Phase 3 work — do not auto-continue.

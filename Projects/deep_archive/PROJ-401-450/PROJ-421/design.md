...
- **Project siblings:** PROJ-413, PROJ-414, PROJ-415, PROJ-416, PROJ-417, PROJ-418, PROJ-419, PROJ-420.
- **Cluster identity:** `pattern30_slot_cleanup`.
- **Severity breakdown:** 1× MAJOR.

### UNCERTAIN Resolution
LEG-02-001 entered this project from the UNCERTAIN bucket. The audit's
original framing ("migrate 8 non-modal slots") was fabricated — verifier
confirmed all 9 windows referenced are `StrategyModalWindow` subclasses.
The user opted to keep the finding and reframe it as "remove redundant
slot-nulls" since Pattern #31 auto-deregistration on `kill()` already
handles the cleanup these nulls were originally intended for, and verifier
grepped to confirm no caller code reads the slots for None.

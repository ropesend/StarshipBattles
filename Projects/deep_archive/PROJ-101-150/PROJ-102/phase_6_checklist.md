...
- [x] Create `SuperweaponOrderProcessor` class

- [x] `process_implode_planet(self, fleet, empire, galaxy, component_registry=None)`:
...
- [x] `process_stellerate_star(self, fleet, empire, galaxy, empires, component_registry=None)`:
...
- [x] `process_open_warp_point(self, fleet, empire, galaxy, component_registry=None)`:
...
- [x] `process_close_warp_point(self, fleet, empire, galaxy, component_registry=None)`:
...
- [x] `process_create_dyson_sphere(self, fleet, empire, galaxy, component_registry=None)`:
  - Find star system at fleet location
...
- [x] `process_self_destruct(self, fleet, empire, galaxy)`:
...
- [x] Add optional `empires` parameter to `process_end_turn_orders()` signature:
  ```python
  def process_end_turn_orders(self, fleet, empire, galaxy, component_registry=None, empires=None):
  ```

- [x] Add order type routing in `process_end_turn_orders()` (after TRANSFER case):
  ```python
...
- [x] Route each order type to the correct processor method
- [x] Pass `empires` to `process_stellerate_star()` (needed for destroying all ships)
- [x] Return True if fleet was consumed, False otherwise

- [x] Handle SELF_DESTRUCT separately: it should execute at START of turn, not end
...
- [x] Update TurnEngine to pass `empires` when calling `process_end_turn_orders()`
...
- [x] Test `process_stellerate_star()`:
...
- [x] Test `process_open_warp_point()`:
...
- [x] Test `process_close_warp_point()`:
  - Both ends of warp link removed
  - Ship consumed
  - WARP_POINT_CLOSED event logged

- [x] Test `process_create_dyson_sphere()`:
  - Star removed
...
- [x] Test component consumption pattern: ship removed, fleet not removed if other ships remain

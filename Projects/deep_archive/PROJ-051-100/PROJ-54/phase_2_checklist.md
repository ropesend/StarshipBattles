...
- [x] Read current `__init__` method (line 27)
- [x] Update signature to add `portrait_surface=None`:
  ```python
  def __init__(self, manager, rect, planet, container=None, portrait_surface=None):
  ```
- [x] Store parameter for potential later use: `self._init_portrait_surface = portrait_surface`
- [x] At end of `__init__` (after line 102), call portrait update:
  ```python
  # Apply initial portrait if provided
  if portrait_surface is not None:
      self._update_portrait(portrait_surface)
  ```
- [x] Update docstring (lines 16-25) to document new parameter:
  ```python
  """
  Args:
      ...existing args...
  ...
  ```
- [x] Test backward compatibility: Verify existing BuildQueueScreen code still works without passing portrait_surface
...
- [x] Update `__init__` signature (line 27) to add `show_complexes=True`:
  ```python
  def __init__(self, manager, rect, planet, container=None, portrait_surface=None, show_complexes=True):
  ```
- [x] Locate complexes list creation code (lines 71-95)
- [x] Wrap in conditional:
  ```python
  if show_complexes:
      ...
      self.complex_items = []
  ```
- [x] Adjust info text width calculation (around line 67):
  ```python
  ...
  ```
- [x] Update `_update_complexes_list()` method (line 199) to check if enabled:
  ```python
  def _update_complexes_list(self):
      ...
      # ... existing code
  ```
- [x] Update docstring to document new parameter
- [x] Test with `show_complexes=False`: Panel should adapt layout, no complexes list visible
...
- [x] Locate PlanetReportPanel creation (lines 102-121)
- [x] Update to use new `portrait_surface` parameter at init time:
  ```python
  ...
  ```
- [x] Remove redundant `update_planet()` call if no longer needed
- [x] Test: Open build queue, verify planet panel displays correctly
...
- [x] Add test for `portrait_surface` parameter in __init__:
  ```python
  ...
  ```
- [x] Add test for `show_complexes=False`:
  ```python
  ...
  ```
- [x] Run all existing tests, verify backward compatibility (no failures)
...
- [x] `portrait_surface` parameter works at __init__
- [x] `show_complexes=False` hides complexes list, adapts layout
- [x] BuildQueueScreen uses cleaner initialization
- [x] All tests pass (no new failures)
- [x] Update status at top of this file to `Complete`
- [x] Update `plan.md` phase table row 2 to `Complete`

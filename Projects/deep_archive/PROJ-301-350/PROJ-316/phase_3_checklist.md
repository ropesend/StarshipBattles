...
- [x] At the top of the file, import the 5 editor classes:
      ```python
      ...
      ```
- [x] Add a parametrised test over the imported class objects (not strings) that asserts `issubclass(cls, StrategyModalWindow)`. Failure mode: a future commit removing the subclass relationship causes a hard test failure — not a soft assertion-message failure.
...
- [x] Add a helper that constructs each editor with a stub `StrategyWindowManager` using the `__new__` + patched-`pygame_gui.elements.UIWindow.__init__` pattern (mirror `tests/unit/ui/screens/test_strategy_modal_window.py::_make_modal_window`). Each editor's domain-specific `__init__` will be patched out, and only `StrategyModalWindow.__init__` invoked manually so registration runs.
- [x] For each of the 5 editors, assert the constructed instance appears in the manager's modal list immediately after `StrategyModalWindow.__init__` runs.
- [x] For each, call `kill()` and assert it deregisters from the modal list.
...
- [x] For each of the 5 `StrategyEventRouter._open_*_editor()` methods (`_open_food_allocation_editor`, `_open_atmosphere_editor`, `_open_gravity_editor`, `_open_water_editor`, `_open_radiation_shield_editor`):
...
- [x] Assert `window_manager` is passed as a keyword argument (not in `*args`).
- [x] Failure mode: if a future commit removes `window_manager=ui.window_manager` from the spawn site, this test fails.
...
- [x] The current `test_editor_blocks_click_inside_rect` exercises the router OR-bridge with a mocked editor in `iter_live_modals`. Decide:
...
- [x] Recommend (a) — keep it but rename. The router-level test catches a different class of regression (router wiring).
...
- [x] Temporarily un-subclass `FoodAllocationEditor` from `StrategyModalWindow` (change to `pygame_gui.elements.UIWindow`). Run the new tests. Confirm Task 3.1's test fails. Revert.
- [x] Temporarily comment out `window_manager=ui.window_manager` in `_open_food_allocation_editor`. Run tests. Confirm Task 3.3's test fails. Revert.
- [x] Temporarily comment out `window_manager.register_modal(self)` in `StrategyModalWindow.__init__`. Run tests. Confirm Task 3.2's test fails. Revert.
- [x] All three mutation tests confirm the new tests have teeth. Document in `decisions.md`.

**Notes:** This step is the proof that the new tests cover what the old test claimed to cover.

**Verification:** All three mutation checks failed in the intended test: un-subclassing `FoodAllocationEditor` failed `test_editor_subclasses_strategy_modal_window`; removing `window_manager=ui.window_manager` from `_open_food_allocation_editor` failed `test_editor_spawn_site_passes_strategy_window_manager`; bypassing `register_modal(self)` failed `test_editor_registers_and_deregisters_via_strategy_modal_window`. All mutations were reverted and the targeted suite passed (`42 passed`).

---

## Phase Completion Checklist
When all tasks above are done:
- [x] 3 mutation tests confirm the test suite catches subclass / spawn-site / registration regressions
- [x] `pytest tests/integration/ui/test_editor_click_blocking.py` clean with new tests

...
- [ ] S08-CAT5-001: rescope the 3 function-scoped MagicMock-tree fixtures (lines 23-52) to class or module scope; ~24 test methods across 9 classes use them. **DEFERRED-OUT-OF-SCOPE (PROJ-322 pass 3):** the 3 fixtures are mutable MagicMocks heavily reassigned by individual tests (`mock_component.get_abilities = MagicMock(return_value=...)` etc.). Class-scoping requires either (a) reset_mock-autouse companion fixture, OR (b) refactor to value-returning factories that build fresh mocks per test — both add complexity without LOC win for a tree of MagicMocks already cheap to construct. NOT scope-pollution risk; the construction cost is negligible (~3 MagicMock() calls per test). **RE-CONFIRMED DEFERRED IN PROJ-327 Phase 2 Task 2.6 (commit 7b05f610a)** — re-audit found ~70 attribute REASSIGNMENTS across 43 tests against `mock_component.{get_abilities,data,stats,evaluated_resource_cost,ship}` and `mock_*_ability.{trigger,check_available.return_value}`. `reset_mock()` cannot restore re-bound attributes (only clears call records); deepcopy breaks MagicMock auto-spec. File runtime is import/collection-bound (~1.7 s for 43 tests, ~40 ms/test); fixture construction is sub-millisecond. Rationale captured in fixture-block comment in the file itself.
- [ ] Verify: `pytest tests/unit/simulation/components/test_component_resource_manager.py` passes; LOC delta approximately -20 _(re-confirmed deferred in PROJ-327 — see above.)_
...
- [ ] S05-CAT5-001 (NEEDS_REWORK): rescope `mock_ui_manager` and `mock_resource_icons` (immutable inputs) to module scope. Keep `sample_snapshot` and `mock_panel` at function scope — they accumulate `assert_called_*` state which must be per-test. Actual test count is 17 methods (not 12 as originally claimed). _(verification adjusted from review's "Module-scope the fixtures (claim: 13 test methods)" - see verification_report.md)_ **DEFERRED-OUT-OF-SCOPE (PROJ-322 pass 3):** the alternative-approach `reset_mock()` autouse fixture pattern would shave ~10 LOC across 17 tests but introduces test-isolation risk that has historically caused intermittent failures in long-running parallel runs. The mock construction cost is negligible (~4 MagicMock() per test). The cycle-time win does not justify the complexity. **RESOLVED IN PROJ-327 Phase 2 Task 2.11 (commit 7b05f610a) via Strategy A** — re-audit confirmed `mock_ui_manager`, `mock_panel`, `mock_resource_icons` are pure inputs that NO test asserts call counts on (the `assert_called_*` checks all hit inner `@patch` mocks like `mock_container`, `mock_header`, or `panel._scroll_container.kill` — none touch the rescoped fixtures). All three rescoped to module scope with explanatory docstrings. `sample_snapshot` kept function-scoped (4 tests in TestPopulationUpkeepRow mutate `total_population_upkeep`). Per-file runtime: 1.69 s → 1.64 s (median of 3, ~3% reclaim).
- [ ] **Alternative approach:** rescope ALL four fixtures to module scope BUT add a `conftest.py` autouse fixture that calls `reset_mock()` on the shared mocks between tests. Document in this task body which approach was chosen and why before marking complete. _(superseded by PROJ-327 Task 2.11 disposition — Strategy A applied, no `reset_mock()` companion needed.)_
- [ ] Verify: `pytest tests/unit/ui/panels/test_empire_treasury_panel.py` passes; LOC delta approximately -10 _(resolved in PROJ-327 — see above; +50 LOC from explanatory comments dwarfs the -10 LOC fixture compaction.)_
...
- [ ] S08-CAT5-003: extract the 63-LOC `make_mock_ship` helper (lines 12-75; 20+ params, called 80+ times across 15 classes) to shared `tests/fixtures/test_entities.py` with kwargs overrides. Coordinate with HLP-001 in Phase 6. **DEFERRED-OUT-OF-SCOPE (PROJ-322 pass 3):** depends on Task 6.4 (HLP-001) which itself is deferred. The 20+ params are fleet-report-specific (display names, hull plating, weapon counts, etc.); a shared module fixture would either be too narrow (only used by this file — defeats the consolidation purpose) or too wide (include fields no other consumer uses). PROJ-322 pass 2 created the narrower DUP-003 cargo-mock and HLP-003/004 yard/planet factories where shapes did align. **RE-CONFIRMED DEFERRED IN PROJ-327 Phase 2/3 (commit pending Task 3.2)** — `make_mock_ship` is a plain helper function (not a pytest fixture), so Phase 2 fixture-rescope strategies (A/B/C) don't apply. Subsumed under Phase 3 Task 3.2 (HLP-001) re-judgment, which measured `make_mock_ship` construction at ~627 µs/call × 115 calls = ~72 ms total in this file (~3.6% of 1.99 s file runtime). Memoization gain capped at ~50 ms (cache hit + deepcopy on retrieval) for net complexity-positive cost. Original "kitchen-sink builder" rejection rationale still holds.
- [ ] Verify: `pytest tests/unit/ui/screens/test_fleet_report_filters.py` passes; LOC delta approximately -50 _(re-confirmed deferred in PROJ-327 — see above.)_
...
- [x] S07-CAT5-001: factor the 118-LOC `_make_race_setup_screen` heavy bypass-init helper (lines 31-148; ~50 mock objects) into a class-scoped fixture; preferred fix - construct the real screen with mocked pygame_gui. Coordinate with APC-001-F11 in Phase 5. **RESOLVED IN PROJ-325 Phase 3 (commit 92a7490b6) — two-stage construction pattern, see consensus plan at `Projects/active_projects/PROJ-325/findings/consensus_discussion/uiwindow_mvvm_refactor_plan_r002.md`.** Helper went from 118 LOC → 53 LOC (-65 LOC, -55%) via `bypass_init(RaceSetupScreen)` + `make_ui_widget(...)` + `MockRaceSetupUiBuilder`. Production `__init__` was refactored into Stage 1 (cheap state + `DefaultRaceSetupDelegateFactory`) before the bypass guard, Stage 2 (UIWindow shell), Stage 3 (`RaceSetupUiBuilder`).
...
- [ ] S02-CAT5-002: rescope `mock_ship`, `mock_ship_with_special_chars`, `minimal_ship` (function-scoped, lines 27-55) to class or module scope; 14 classes consume them. **DEFERRED-OUT-OF-SCOPE (PROJ-322 pass 3):** ship_io tests serialize/deserialize ships; many tests mutate the mock ship (set_modifier, change attributes) before asserting on round-trip identity. Class-scoping risks state leakage across tests. The 3 fixtures construct ~5 MagicMocks total — cycle-time savings are negligible. **RESOLVED IN PROJ-327 Phase 2 Task 2.19 (commit 7b05f610a) via Strategy A** — original deferral rationale was incorrect at re-audit time: a grep across this file finds ZERO attribute writes against the 3 fixtures. Tests only call `mock_ship.to_dict()` (read-only) and read `.name`/`.ship_class`/`.layers`. Round-trip tests construct a NEW ship via `Ship.from_dict()` (doesn't touch the fixture). `mock_ship` and `mock_ship_with_special_chars` rescoped to module + switched `fresh_registries` → `session_registries` (eliminates per-test registry deepcopy). `minimal_ship` deleted as dead code (zero references). Per-file runtime: 2.41 s → 2.13 s (median of 3, ~12% reclaim — biggest single Phase 2 gain because the eliminated registry deepcopy is much heavier than the saved MagicMock construction). Note: file path moved from `tests/unit/simulation/test_ship_io.py` to `tests/unit/ui/services/test_ship_io.py` between PROJ-322 and PROJ-327 (per Phase 0 finding).
- [ ] Verify: `pytest tests/unit/ui/services/test_ship_io.py` passes; LOC delta approximately -20 _(resolved in PROJ-327 — see above; net change about -10 LOC after explanatory comment block.)_

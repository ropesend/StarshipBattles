...
- ~28 verified items across `game/strategy/` and one `game/app_bootstrap.py` fallback closure
- 3 phases: Critical (4 — GameSession combined cluster + `_registry` + `_get_nav_service` + `primary_star`), Major (~13), Minor (~11)
- Includes one UNCERTAIN item (`_build_capture_context` with new `ReplayCaptureContext` type)

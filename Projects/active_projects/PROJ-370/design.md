...
1. **Large blast radius.** ~30 files, ~80 mutation sites total. Mitigation: phased migration (one data class at a time), AST guard runs after each phase, ApplicationContext wires defaults so existing tests Don't Reach For The Mutator manually.

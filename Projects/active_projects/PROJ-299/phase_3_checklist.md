...
- [ ] Write failing tests (TDD; this is the densest test set in the project — pure function, lots of edge cases):
...
- [ ] Implement `build_bio_prompt(race_config: RaceConfig, captions: Dict[str, Optional[dict]]) -> List[Message]`:
...
- [ ] Write failing tests (mirror 3.1's tests for the socio variant)
- [ ] Implement `build_socio_prompt(...)` — same structure, different SYSTEM_PROMPT (focused on society/culture) and few-shot example
- [ ] DRY check: extract shared helpers (`_render_aptitudes`, `_render_preferences`, `_render_caption_or_note`) so both functions reuse them
- [ ] Run tests, confirm pass
...
- [ ] Construct a sample `RaceConfig` (use the QS race fixture or a fresh one)
- [ ] Call `build_bio_prompt(...)` with full captions; print the messages to console
- [ ] Manually verify the prompt reads like a coherent instruction to an LLM
- [ ] Estimate the token count (rough: `len(text) / 4`); confirm well under 2000 tokens for input
- [ ] Document any phrasing tweaks in the task notes
...
- [ ] ~16 new tests in `test_race_description_prompt_builder.py`

...
|-------|--------|-----------|
| 1. C1 — Treasury Total includes Population Upkeep + e2e test | Not Started | [phase_1_checklist.md](phase_1_checklist.md) |
...
**Last Action:** Project scaffolded from a dual cross-project audit of PROJ-283..290 (my session-end review + the prior 5-skeptic audit in `Temp Review Docs/`). Two impartial subagents settled disagreements: (a) the PROJ-289 view-kwarg threading IS a real UX regression (my high-severity call upheld over the prior audit's "by design" minor-severity); (b) the cache-rollback concern (M4 in the prior audit) is CLEARED — `TurnStateSnapshot.restore()` does a full deserialization that discards the stale planet objects with their cache fields. User confirmed all three design choices (Q1, Q2, Q4) — IRaceRegistry retrofit for engines, multi-resource migration for the editor, two-project split.
**Next Action:** Phase 1 Task 1.1 — write a failing test for C1 in `tests/unit/strategy/engine/test_empire_economy_calculator.py::TestTreasuryTotalIncludesUpkeep` that asserts `snapshot.total_expenses[r] == tributes + ships + complexes + population_upkeep` for an empire with non-zero upkeep. The current code at `game/strategy/engine/empire_economy_calculator.py:147-150` omits the upkeep term — the test will fail until the one-line fix (Task 1.2) lands.
**Blockers:** None — all of PROJ-283..290 must remain at `Awaiting Verification` until this project completes (PROJ-291 is the gate). PROJ-292 can run in parallel because the file overlaps are minimal.
**Context for Next Agent:** Read `findings/SUMMARY.md` first — it's a copy of the dual-audit summary that drove this project. The 3 Critical findings are confirmed real bugs. The C3 fix is the largest piece (engine retrofit) and reverses an earlier deferral in PROJ-287's decisions.md line 16. Mirror the PROJ-285 pattern that already added an optional `race_registry` kwarg to `HarvestingEngine` and `ProductionEngine` — both engines short-circuit gracefully when the kwarg is None, preserving 850+ pre-PROJ-285 MagicMock tests. Apply the same shape to `HappinessEngine` + `PopulationEngine` so the existing tests don't break. Phase 1 (C1) is a one-line code fix + e2e test that doubles as the integration test the audit also requested (M3 from prior audit). Phase 3 (C2 — editor migration) retires the `population_food_resource` shim as a free side-effect.

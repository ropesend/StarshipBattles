...
- [ ] First, **migrate the 13 broken fixtures** to the new schema. Find every `EconomyConfig(population_food_resource=..., food_per_pop_per_turn=...)` construction and replace with `EconomyConfig(population_consumption={"organics": 0.001, ...})`. The exact resources + rates depend on the test's assertion — most should use `{"organics": 0.001}` to match the prior single-resource behaviour, but some will benefit from a multi-resource setup.
- [ ] Add new tests (in a new class `TestMultiResourcePreview` or inline if the existing classes are the right home):
...
- [ ] Run the file. Confirm migrated fixtures pass on the OLD function signature (when called with single-resource), and new tests FAIL (function signature is still old; preview UI is still single-row).
...
- [ ] Update `compute_consumption_preview` signature:
  ```python
  ...
  ```
- [ ] Update line 258 (the broken call site):
  ```python
  consumption = compute_consumption_preview(
      pop, allocation, self._economy.population_consumption
  )
  ```
- [ ] Run Task 3.2's tests for the function signature — Tests 1, 2, 3 should pass.
...
- [ ] Update the row UI: each row currently shows one preview label per slider. Update it to show one preview label PER (slider × resource). For a single-resource economy this looks identical to the pre-migration UI. For a multi-resource economy the player sees one preview line per resource consumed.
- [ ] Update any title / heading text that referenced "Food" or "Organics" — pull the primary resource name from `economy_config.primary_resource` if a primary-resource label is needed.
- [ ] Run Task 3.2's Test 4 — should pass.
- [ ] Run the full file — all 13+ tests green.
...
- [ ] Run `grep -rn "population_food_resource" game/ tests/ | grep -v "population_consumption"` to find all remaining consumers.
- [ ] If ZERO production callers remain (only test references and the shim itself), delete the shim property from `economy_config.py` AND any test fixtures that were only testing the shim.
- [ ] If callers DO remain (e.g. label / title resolution that legitimately wants "the primary resource"), leave the shim in place and update its docstring to note it's the only remaining caller.
- [ ] Run the targeted test — green either way.
...
- [ ] Suite-wide regression check: `pytest tests/unit/ui/ tests/unit/strategy/ -q --ignore=tests/unit/quickstart/` — no new failures.

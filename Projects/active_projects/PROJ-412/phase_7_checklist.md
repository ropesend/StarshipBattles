# Phase 7: Secondary Phase Optimizations (energy / environmental / movement) + final docs

> **BEFORE MARKING THIS PHASE COMPLETE:**
> 1. Run `python Projects/scripts/validate_phase.py PROJ-412 7`
> 2. Only proceed if output shows PASSED
> 3. Update plan.md phase table AND Current State

**Status:** Not Started
**Objective:** Apply the cache pattern proven in Phase 5 to the remaining redundant per-tick scans (`PlanetEnergyEngine`, `EnvironmentalHazardEngine`, `FleetMovementEngine`). Each task is independently gated on remeasurement after Phase 5 lands — if a phase's bucket is < 50 ms / turn at that point, drop that task. Also captures the final benchmark + doc updates that close the project.

This is the cleanest place to stop if the user is satisfied with Phase 4+5 reduction. Re-evaluate scope at the start of this phase.

---

## Tasks

### Task 7.1: Reduce remaining `PlanetEnergyEngine` per-tick work [Simple] ⚠ scope reduced (codex consult)
...
- [ ] `PlanetEnergyEngine` **already has** `_energy_cache` keyed by planet_id with a facility-fingerprint cache invalidator and a public `invalidate_energy_cache(planet_id)` method ([planet_energy_engine.py:154-179, 204-240](../../../game/strategy/engine/planet_energy_engine.py#L154)). The bulk of the per-tick scan is already cached.
- [ ] If post-Phase-5 measurement still shows `planet_energy` as a material cost, identify what remains: most likely shield drain / auto-deactivate logic on the **active** code path, not the cached generation/capacity scan.
...
- [ ] If this phase is < 50 ms / turn at remeasurement (or already reduced by Phase 5's universal-pipeline cache benefiting from `IAbilitySource` invalidation hooks), **cut this task** and document the cut in `decisions.md`

**Notes:**

### Task 7.2: `EnvironmentalHazardEngine` short-circuit and storm cache [Simple]
...
### Task 7.3: `FleetMovementEngine` pathfinding memoization [Complex]
...
### Task 7.4: Final benchmark and documentation update [Medium]
...
- [ ] User has run the bench against their own save game on their hardware and the result is acceptable

...
- [ ] Create new file `game/ui/screens/strategy_window_manager.py`
- [ ] Create `class StrategyWindowManager` with constructor accepting:
...
- [ ] Move these methods into StrategyWindowManager:
...
- [ ] Track window references as instance attributes:
...
- [ ] Ensure imports: `pygame`, `pygame_gui`, `PlanetSelectionWindow`, `PlanetListWindow`, `FleetOrdersWindow`, `FleetReportWindow`, `BuildQueueListWindow`, `EmpireBuildQueueWindow`, `EventLogWindow`
- [ ] Add `handle_resize(self, width, height)` to update stored dimensions
...
- [ ] Add import: `from game.ui.screens.strategy_window_manager import StrategyWindowManager`
- [ ] In `StrategyUI.__init__`, create window manager:
  ```python
  ...
  ```
- [ ] Replace all `open_*` method bodies with delegation:
...
- [ ] Replace close callback bodies with delegation:
...
- [ ] Update `handle_resize` to call `self._window_manager.handle_resize(width, height)`
- [ ] Remove now-unused imports from strategy_ui.py: `PlanetSelectionWindow`, `PlanetListWindow`, `FleetOrdersWindow`, `FleetReportWindow`, `BuildQueueListWindow`, `EmpireBuildQueueWindow`, `EventLogWindow`, `TransferDialog` (check each is not used elsewhere)
...
- [ ] Update `_has_modal_open` to check window references via `self._window_manager` instead of `self`:
  ```python
  ...
  ```
- [ ] Update `handle_event` UI_WINDOW_CLOSE handling to reference `self._window_manager.*`
...
- [ ] Verify line count of `strategy_ui.py` decreased by ~150+ lines

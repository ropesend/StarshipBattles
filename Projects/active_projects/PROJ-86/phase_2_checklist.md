...
- [ ] Create new file `game/ui/screens/test_lab/validation_manager.py`
- [ ] Create `class TestLabValidationManager` with constructor accepting:
...
- [ ] Move `_validate_all_scenarios` logic (lines 329-393) into `ValidationManager.validate_all(self)` method
- [ ] Move `_build_validation_context_from_files` logic (lines 395-446) into `ValidationManager.build_context_from_files(self, test_id, metadata)` method
- [ ] Move `_handle_update_expected_values` logic (lines 476-523) into `ValidationManager.handle_update_expected_values(self, selected_test_id, ui_manager, screen_width, screen_height, on_confirm_callback)` method
...
- [ ] Move `_apply_metadata_updates` logic (lines 525-613) into `ValidationManager.apply_metadata_updates(self, changes)` method
- [ ] Ensure imports: `json`, `os`, `simulation_tests.logging_config.get_logger`, `.dialogs.ConfirmationDialog`
...
- [ ] Add import: `from .validation_manager import TestLabValidationManager`
- [ ] In `TestLabScreen.__init__`, create `self._validation_manager = TestLabValidationManager(self.registry, self._data_extractor, lambda: self.all_scenarios)`
- [ ] Replace `_validate_all_scenarios` method body with delegation: `self._validation_manager.validate_all()`
- [ ] Replace `_build_validation_context_from_files` method body with delegation: `return self._validation_manager.build_context_from_files(test_id, metadata)`
- [ ] Replace `_handle_update_expected_values` method body with delegation, passing UI params:
  ```python
  ...
  ```
- [ ] Replace `_apply_metadata_updates` method body with delegation: `self._validation_manager.apply_metadata_updates(changes)`
- [ ] Remove now-unused imports from screen.py (check: `Validator` import at line 338, `json` if only used here)
...
- [ ] Verify line count of `screen.py` decreased by ~200+ lines

...
**Active Phase:** Phase 9 (_CarriedItemsProxy final cutover) — Phase 8 complete on branch `proj/PROJ-436/phase_8` (not yet merged to `main`)
...
**Next Action:** Phase 8 COMPLETE on branch `proj/PROJ-436/phase_8` — see the 2026-05-18 Phase 8 decisions rows. Deleted the 3 `context_type` storage-dispatch sites in `ProductionEngine._check_affordability` / `_log_resource_shortage` / `_apply_resource_consumption` by defining `IProductionResourceSource` (runtime_checkable Protocol) in `production_engine.py` and adding 3 thin polymorphic delegators on Planet (over `has_stockpile` / `get_stockpile` / `consume_from_stockpile`) and 3 on Fleet (over `has_cargo_resources` / `get_cargo_resource` / `consume_cargo_resource`). Entity-specific names retained because `transfer_branches.py` and `IStockpileHolder` use them. The Phase 5 `ValueError`-on-unknown-owner contract is gone — an owner without `production_*` methods now raises the natural `AttributeError`. AST guard at `tests/static_guards/test_no_legacy_storage_fields.py` extended with 3 new tests (1 pinning absence of `getattr(colony_or_fleet, 'context_type')` in production_engine.py source, 2 pinning the unified-protocol methods exist on Planet / Fleet). New integration suite at `tests/integration/test_production_engine_container_unified.py` (17 tests across 5 classes). 6 engine-side test files migrated. PLANET_LOC_CEILING raised 405 -> 425 (3 protocol-conformance methods + 4-line header comment). Codex consult substituted with OpenCode `pre-final-check` `--allow-tests` (`AgentCoordination/Scratchpad/Consult/20260518T072020Z_proj436-phase8-cutover/`, exit_status ok). All 7 verification checks PASS; 1 finding (residual `context_type` test-mock assignments) remediated this phase. Sharded suite: **23205/23207** (+20 from 23185 Phase 7 baseline, net of new integration tests + AST guards + adjusted test scoping). Branch awaits merge to `main`.

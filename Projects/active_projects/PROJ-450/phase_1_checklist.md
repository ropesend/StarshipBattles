...
- [x] Create the new test file. Cover:
...
- [x] Run; expect 7+ failures (methods don't exist yet — RED is correct)
...
- [x] Move `_is_carried_vehicle_dict` from `transfer_branches.py:41-52` to `planet.py` (module-level private helper)
- [x] Move `_pod_from_dict` from `transfer_branches.py:55-73` to `planet.py`
- [x] Move `_staging_yard_carried_vehicle` from `transfer_branches.py:76-87` to `planet.py`
- [x] In `transfer_branches.py`, remove the 3 helper definitions; replace internal call sites with imports from `planet.py` (or, where Phase 1.3 deletes them, drop entirely)
...
- [x] Locate `add_to_staging_yard` at lines 316-322 (post-PROJ-449 Phase 3). Change signature from:
  ```python
  ...
  ```
- [x] In the body, normalize the input to dict (since the substrate is still dict-typed in Phase 1):
  ```python
  ...
  return True
  ```
- [x] Add `pop_staging_yard_typed(self, index: int) -> CarriedVehicle | DropPod | None`:
  ```python
  ...
  ```
- [x] Run Task 1.1's tests; they should all pass
...
- [x] At line 412, change:
  ...
  ```
- [x] At lines 454-460, drop the `pod_dict = dict(pod.payload); pod_dict["design_id"] = ...; planet.add_to_staging_yard(pod_dict)` flatten block; change to:
  ```python
  if planet.add_to_staging_yard(pod):
  ```
- [x] In `_dispatch_drop_pod_load` at lines 200-250, replace `_pod_from_dict(removed)` calls with `planet.pop_staging_yard_typed(i)` — change the loop so it pops via the typed API rather than calling `remove_from_staging_yard` then `_pod_from_dict`
- [x] Similar migration in `_dispatch_carried_vehicle_load` at lines 340-380 — pop via `pop_staging_yard_typed` instead of `_staging_yard_carried_vehicle(item)` after `remove_from_staging_yard`
- [x] Run focused tests; expect them to pass (transparent change to engine semantics)
...
- [x] Create the integration test that verifies the PROJ-445 Phase 2 fleet-to-fleet pod path continues to work after Phase 1's engine cleanup
- [x] Fleet A unloads a `DropPod` to planet; planet `pop_staging_yard_typed(0)` returns a `DropPod` instance with original `design_id` / `mass` / `payload`
- [x] Run; verify green

### Task 1.7: Full sharded suite + commit [Medium]
**Tests:** `python Tools/test_sharded/test_sharded.py`

- [x] Sharded suite green at the pre-phase count + new test count (Task 1.1 added 7+ tests; Task 1.6 added 1+)
- [x] Commit message: `PROJ-450 Phase 1: Path A engine API cleanup — typed accept + typed pop (centralizes dict↔typed helpers in Planet)`

---

## Phase Completion Checklist
- [x] 3 helpers moved from transfer_branches.py to planet.py
- [x] `Planet.add_to_staging_yard` accepts typed inputs
- [x] `Planet.pop_staging_yard_typed` exists and returns typed views
- [x] `transfer_branches.py` flatten blocks at 412 + 454-460 dropped
- [x] `issuer_adapter.py:363` flatten dropped
- [x] New unit + integration tests green
- [x] Sharded suite green
- [x] Plan.md Quick Status → Complete; Current State updated
- [x] **Note in plan.md**: substrate is still `List[Dict[str, Any]]` — Phase 2 changes the type

...
- [ ] Open `Projects/active_projects/PROJ-449/plan.md`
- [ ] Confirm Phase 3 row shows `Complete` in the Quick Status table
- [ ] Verify that `_planet_init_with_legacy_kwargs` is GONE from `game/strategy/data/planet.py` (use `Grep` to confirm absence)
- [ ] Verify that `Planet.staging_yard` @property/@setter pair is GONE (use `Grep` for `@staging_yard.setter` — should return zero matches)
- [ ] If any of the above fail, **PAUSE**: PROJ-450 Phase 1 cannot start. Update plan.md Current State with "Blocked on PROJ-449 Phase 3" and surface to user.
...
- [ ] Run (PowerShell-friendly):
  ...
  ```
- [ ] Compare against Stage 3 preflight §1.2 numbers (310 across 49 files, broken down 82 game + 228 tests)
- [ ] Pre-audit at 2026-05-19 confirmed matching counts; verify still matches at the time of execution
- [ ] If count is dramatically different (>10% delta in either direction), capture the delta in `findings/phase_0_audit.md` and verify before proceeding
...
- [ ] **BLOCKER #2** — Integration test mutations. Run:
  ```bash
...
- [ ] **BLOCKER #3** — Validator + write service probes:
...
- [ ] At lines 165-191, verify the `_ship_container_snapshot` projects bay capacity from the real `_cargo_mgr.get_vehicle_bay_capacity()` (per PROJ-444 Phase 2 Task 2.4 completion)
- [ ] Document in `findings/phase_0_audit.md`: "F-A-013 confirmed complete; no PROJ-450 action required"
...
- [ ] Open `Projects/active_projects/PROJ-454/plan.md`. Confirm the Quick Status table shows ALL phases marked `Complete` AND the Current State block reads "Project complete" (or equivalent). If still in progress, record the current phase in `findings/phase_0_audit.md` and PAUSE.
- [ ] Open `Projects/active_projects/PROJ-456/plan.md`. Same check.
- [ ] **If either is in progress:** STOP. Update PROJ-450 plan.md Current State with: "Blocked on sync gate — waiting on PROJ-454=<phase>, PROJ-456=<phase>". DO NOT proceed to Phase 1. Resume only when both gates clear.
- [ ] **If both are complete:** record SHAs (or final-state references) in `findings/phase_0_audit.md` and proceed to Phase 1.
- [ ] **Optional fill work while blocked:** read codex audit artifacts for context, do read-only verification of upstream code state, refine PROJ-450/decisions.md housekeeping. Do not start any Phase 1+ task.
...
- [ ] `findings/phase_0_audit.md` exists with all 5 sections populated
- [ ] PROJ-449 Phase 3 confirmed landed via direct code inspection (no SHA signal mechanism — that was removed in Bucket A fix)
- [ ] **Task 0.6 cross-group sync gate cleared** (PROJ-454 + PROJ-456 both Complete)
- [ ] Plan.md Quick Status row for Phase 0 → Complete
- [ ] Plan.md Current State updated with audit summary numbers + sync-gate status

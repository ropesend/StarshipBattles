...
**Next Action:** Execute Phase 3 — UI reader migration + DTO + validator + write-service tightening. Delete the Phase-2 dict-projection bridge and replace with a permanent typed read-only property `Planet.staging_yard -> Tuple[CarriedVehicle | DropPod, ...]`. Migrate `strategy_detail_fmt.py:289` to typed attribute access. Migrate the 5 dict-injection fixture tests at `test_strategy_detail_fmt.py:915-1009`. Drop validator runtime shape probes. Also migrate `issuer_adapter.py:335` direct `_staging_yard = remaining` site to use the typed write API.

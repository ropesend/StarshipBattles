...
**Next Action:** Execute Phase 5 — tighten `tests/static_guards/test_no_legacy_storage_fields.py` to additionally pin "every entry in `_staging_yard` is `CarriedVehicle` or `DropPod`, no dict entries" so a future regression that re-introduces dicts is caught at static-guard time.

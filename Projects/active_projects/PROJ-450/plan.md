...
**Next Action:** Execute Phase 4 — migrate the 9+ direct `.append`/`.extend` mutations in `tests/integration/test_fms_planet_*` (4 files) + `tests/integration/test_fms_a_e2e.py` (whatever survives). Replace dict-literal mutations with `planet.add_to_staging_yard(typed_instance)` or direct typed construction.

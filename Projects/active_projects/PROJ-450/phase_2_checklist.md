...
- [x] At line 305, replace `planet.staging_yard.clear()` with an explicit removal call that survives the Phase-2 read-only projection:
  ```python
...
- [x] Run the focused test; verify it passes (the substrate is still dict-typed at this point — the migration is mechanical)
- [x] Confirm no other `.clear()` / `.append(` / `.extend(` against `planet.staging_yard` survives in the production / integration test surface (Phase 4 covers the `.append(` / `.extend(` sites; Phase 4's substrate-aware migration handles them after the typed substrate lands)
...
- [x] Run; expect 3 failures (substrate is still dict-typed)
...
- [x] Change the dataclass field annotation:
  ```python
  ...
  ```
- [x] Update `add_to_staging_yard`: when accepting a dict, promote to typed via the existing internal helpers BEFORE appending. Replace the Phase-1 normalization that produced a dict with:
  ```python
  ...
  return True
  ```
- [x] Update `get_staging_mass`:
  ```python
  def get_staging_mass(self) -> float:
      return sum(getattr(item, 'mass', 0.0) for item in self._staging_yard)
  ```
- [x] Update `pop_staging_yard_typed`: now trivially returns the typed entry from the list (no dict→typed promotion required); keep the method as the typed-pop API
- [x] Update `remove_from_staging_yard`: returns the typed entry (no longer a dict). **Note**: this is a semantic change — callers that expected a dict need to adapt. Per Phase 0 audit, the only production caller of `remove_from_staging_yard` is `pop_staging_yard_typed`, so no change at the call site (it just gets a typed thing now).
- [x] **Add the temporary dict-projection property** for the UI reader (Phase 3 deletes this):
  ```python
  @property
...
- [x] Read the current `IStagingYardHolder.add_to_staging_yard` and `remove_from_staging_yard` signatures at lines 189-196. Currently `Dict[str, Any]` and `Optional[Dict[str, Any]]`.
- [x] Now that the substrate is typed (Task 2.2 widened `_staging_yard` to `List[CarriedVehicle | DropPod]` and `add_to_staging_yard` accepts the union per Phase 1), tighten the protocol annotations to match:
  ```python
  ...
  def remove_from_staging_yard(
      ...
      ...
  ```
- [x] The `Dict[str, Any]` union member stays in `add_to_staging_yard` because Task 2.2 widened the implementation to accept dicts too (back-compat for legacy save load). The `remove_from_staging_yard` return tightens to typed-only because the substrate now returns typed entries.
- [x] Drop unused `Optional` / `Dict` imports if no other signature in the file needs them
- [x] Run focused tests: `pytest tests/unit/strategy/data/ -k "galaxy_protocols or planet" -n 4 -q`
...
- [x] Add a new module-level helper:
  ```python
  def _normalize_to_typed(
      ...
      return result
  ```
- [x] Update `planet_from_dict_kwargs` at line 159 (post-PROJ-449 Phase 2 rewrite) to call the normalizer:
  ```python
  _staging_yard=_normalize_to_typed(data.get("staging_yard", [])),
  ```
- [x] Update `planet_to_dict` to serialize each typed entry to its dict form. Find the line that emits `"staging_yard": planet._staging_yard` (or similar) and change to:
  ```python
  "staging_yard": [item.to_dict() for item in planet._staging_yard],
  ```
- [x] Run focused save-load round-trip test; verify dict-on-disk + typed-in-memory invariant holds
...
- [x] Locate `IStagingYardHolder` in `galaxy_protocols.py` (Phase 0 audit showed 4 staging_yard refs in this file)
- [x] Widen any `staging_yard: List[Dict[str, Any]]` annotation to `staging_yard: List[CarriedVehicle | DropPod]`
- [x] Verify the matching test file `tests/unit/strategy/data/test_galaxy_protocols.py` still pins the right shape
...
- [x] At lines 347-360 (the dict-construction site flagged by Stage 3 preflight §3.1), replace dict-based construction with typed:
  ```python
  # OLD:
  pod_dict = {
      "design_id": ...,
...
- [x] Sharded suite green (count + 3 new tests from Task 2.1)
- [x] Save-load round-trip integration test green
- [x] Commit message: `PROJ-450 Phase 2: widen Planet._staging_yard to List[CarriedVehicle | DropPod]; serde normalizes on load (closes F-B-013 substrate half)`

---

## Phase Completion Checklist
- [x] `_staging_yard` substrate is typed `List[CarriedVehicle | DropPod]`
- [x] `planet_serde._normalize_to_typed` converts dict → typed on load
- [x] `planet_to_dict` serializes typed → dict on save
- [x] `IStagingYardHolder` protocol annotation widened
- [x] `production_spawner.py` constructs typed objects directly
- [x] **Temporary dict-projection property `staging_yard` ALIVE** (deleted in Phase 3)
- [x] Save-load round-trip green against `galaxy_proj372_populated.json`

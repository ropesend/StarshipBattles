...
- [ ] Locate the battle-event emitter (likely in
      `ConflictResolutionEngine._resolve_combat_at_hex` or its
      post-resolution callback).
- [ ] After the battle outcome is applied, fetch the just-captured
      `replay_id` from the engine (set in Phase 3 Task 3.2 as
      `engine.replay_id`).
- [ ] Add `replay_id: Optional[str]` to the event row's data shape /
...
- [ ] Update `event_log_data_source.py` to surface `replay_id` per row.
...
- [ ] In the event log row factory, when an event row has a non-None
      `replay_id`, render a small "Replay" button at the right edge.
- [ ] Button is disabled (greyed) when the replay file is missing on disk
      (use `ReplayStore.load(replay_id)` returning None as the signal —
      cache the result so we don't re-stat every frame).
- [ ] Tooltip on hover: "Replay this battle" or, when missing, "Replay file
      not found (may have been evicted by ring buffer)".
- [ ] Click handler: see Task 6.3.
...
- [ ] Click resolves `replay_id` → `ReplayStore.load(replay_id)` →
      `ReplayRecord`.
- [ ] If load returns None (missing/corrupt/version-mismatch): show a
      toast / banner "Replay unavailable: <reason>" and abort.
- [ ] If `record.components_registry_hash` differs from the current
...
- [ ] Convert `record.spec` → `BattleSpec` via the Phase 2 helper
      `to_battle_spec(replay_spec, *, registries)`.
- [ ] Build a `BattleConfig` with `replay_mode=True`, `replay_id=...`,
      `captured_telemetry_level=record.spec.telemetry_level`.
- [ ] Stash a "return-to" pointer so Phase 5 Task 5.4's Exit Replay can
      restore the Event Log window state.
- [ ] Transition to the BattleScreen via the standard scene-transition
      pathway.
...
- [ ] Battle event rows show participant context: turn number, sector,
...
- [ ] Newest replays sort first (matches the existing event-log sort).
...
- [ ] Add a short section to `docs/systems/strategy_layer.md` describing
      ...
      the key files.
- [ ] Optionally add a new pattern entry to `docs/02_PATTERNS.md`
...
- [ ] Update `> **Last verified:**` blockquotes on touched docs.
...
- [ ] Test: event row with `replay_id` pointing at a missing file → button
      visible-but-disabled with the missing tooltip; click does nothing
      with no crash.
- [ ] Test: event row with `replay_id` pointing at a corrupt JSON file →
      same fallback behavior.
- [ ] Test: event row with `replay_id` pointing at a version-mismatched
      replay → silent skip with debug log; UI behaves as if file is
      missing.
- [ ] Test: event row with `replay_id` pointing at a registry-hash-drifted
...
- [ ] Headless UI test (uses pygame_gui's existing test harness) that
...
- [ ] Full sharded suite passes. Record final post-PROJ-312 test count
      against the 15672 baseline; estimate is ~+31 tests.
- [ ] Manual smoke: full happy path — start save → run battle in strategy
...
- [ ] Manual smoke: 3-team end-turn battle → Replay button works → playback
      shows all three teams correctly.
- [ ] Manual smoke: delete the save via in-game UI → confirm replays/
...
- [ ] Replay button is visible on every battle event row that has a
      captured replay
- [ ] Graceful degradation tested for missing / corrupt / version /
      registry-drift cases
- [ ] End-to-end smoke (capture → save → load → replay → exit) passes
      manually
- [ ] Documentation updated in `docs/systems/strategy_layer.md`
- [ ] Update status at top of this file to `Complete`
- [ ] Update plan.md phase table row to `Complete`
- [ ] Update plan.md Current State to: project complete, all phases verified
...
- [ ] All 6 phase checklists complete and validated
- [ ] AST guard against unseeded `random.*` is green and visible in CI
- [ ] Round-trip serialization tests cover every Replay DTO
- [ ] End-to-end determinism test (capture → save → load → replay → outcome
      hash matches) is green for 2-team, 3-team, ErraticBehavior, and
      DETAILED-telemetry cases
- [ ] Sharded test suite green at expected count (15672 + ~31 new tests)
- [ ] Manual smoke: full capture → playback flow works in strategy AND
      Combat Lab AND Battle Setup contexts
- [ ] Save delete cascade verified
- [ ] Ring buffer cap honoured (write 51 replays, oldest one is gone)
- [ ] User has run through the manual smokes and confirmed UX

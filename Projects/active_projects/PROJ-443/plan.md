...
**Active Phase:** Phase 3 complete; Phase 4 ready to start
...
**Next Action:** Phase 4 — flip `pytest.ini`: remove `data`, `combat_lab`, `Assets` from `norecursedirs` (keep `ShipThemes` per `decisions.md` 2026-05-18 row). Add `tests/static_guards/test_no_hidden_test_files.py` regression guard (every on-disk `test_*.py` under `tests/` is in the `pytest tests/ --collect-only` output set). Update `docs/guides/testing_infrastructure.md` snippet around lines 187-194 with the new `norecursedirs` value + rationale. Document `.testmondata` one-time rebuild in `decisions.md`. Verify post-flip sharded count jumps from 21233 to ~23185 (band 23110–23185 per Phase 0 projection). Address any flake / test-isolation issue exposed by the wider collection.
**Blockers:** None.

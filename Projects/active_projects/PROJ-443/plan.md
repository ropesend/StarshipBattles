...
| 6. Codex consult + verified-finding remediation | Complete | [phase_6_checklist.md](phase_6_checklist.md) |

## Current State
**Last Updated:** 2026-05-18
**Active Phase:** Phase 6 complete; project ready for user `verified` label
...
**Next Action:** User applies the `verified` label and closes the project per the CLAUDE.md authority constraint. All 6 phases complete; Codex consult landed with 1 MUST-FIX-NOW remediated (`movement_phase_collaborator._prune_destroyed_fleet_contents` now triggers speed recalculation per the PROJ-320 fleet-speed invariant — pinned by a new RED test) and 6 SHOULD-FIX-FOLLOWUP items logged in `decisions.md` 2026-05-18 row for future projects to triage. Sharded suite green at 23186 tests / 23184 passed / 0 failed / 0 errors / 2 intentional skips. **Verification on the user's side**: confirm sharded green locally (`python Tools/test_sharded/test_sharded.py`), spot-check that direct invocation of all 6 previously-hidden directories matches the sharded count, and apply the `verified` label.
**Blockers:** None.

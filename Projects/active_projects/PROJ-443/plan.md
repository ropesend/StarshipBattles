...
**Last Action:** Phase 1 (`test_cargo_tracking.py` triage) complete. All 30 failures classified as category (b) — test contracts stale, no production bugs surfaced. Two underlying root causes: **Cluster A** (29/30) — `ShipInstance(...)` direct-constructor fixtures predate PROJ-211's mandatory-registry contract; `ShipStatsCache.calculate` raises `ValueError` because `_registries is None`; the fixtures' `mock_registries` parameter was never actually wired. **Cluster B** (5/30, overlapping with A) — tests called `fleet.get_fleet_cargo_X(...)` directly; per `Fleet:281-282` PROJ-210 Phase 2 removed these pass-through methods in favor of `fleet.resources.get_fleet_cargo_X(...)`. **Fix:** added module-level `_prime_stats(ship)` helper that hydrates `ship._cached_stats` from `ship.design_data['expected_stats']` (so `ShipStatsCache.get_or_compute` returns cached and skips `calculate`); updated all 3 fixtures + 2 inline ships to prime stats; re-primed the `from_dict`-restored and `clone()`-returned instances; mechanical sweep of 5 Fleet method calls to add `.resources.` prefix; removed unused `unittest.mock` import. **Result:** `pytest tests/unit/strategy/data/test_cargo_tracking.py` → 32 passed in 1.40s (30 newly-fixed + 2 pre-existing pass). No production code touched. Sharded suite re-verified green at 21233/21233 (in progress / completed pending commit).
**Next Action:** Phase 2 — triage 4 failures in `tests/unit/strategy/data/test_mutator_boundary_ast_guard.py` (`test_mutator_boundary[ShipInstance|Fleet|Planet|Empire]`). Plan estimated ~9 failures but actual is 4 — PROJ-436's protocol consolidation already eliminated most drift. These are AST-static-guard tests checking that data classes don't violate allowlist policies (likely "no public mutator methods on these entities outside their canonical mutator interface"). Run with `--tb=long` to inspect the guard's assertion form; decide per-failure whether (i) the data class genuinely violates the invariant → fix the class, (ii) the guard's allowlist needs updating to reflect post-PROJ-436 architecture → update the guard.
**Blockers:** None.

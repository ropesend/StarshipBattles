...
- [ ] Read `water_target_editor.py` end-to-end. Note water-specific state fields, widget construction, callback shapes, validation rules (water-specific: total + per-form / liquid / ice / vapor breakdown if applicable).
...
- [ ] Mirror the Phase 2/3 test classes. Adjust to water-specific surface.
- [ ] **Test class: TestConstruction** — bypass-init + production + window-manager-required.
- [ ] **Test class: TestWaterSliderInteraction** — initial values from `planet.water_target`; slider→label updates.
- [ ] **Test class: TestSpeciesIdealPreset** — set-to-ideal populates from `race_config.water_preference`.
- [ ] **Test class: TestApplyCallback** — Apply emits `(planet_id, water_dict)`.
...
- [ ] Move widget construction into `DefaultWaterTargetEditorUiBuilder`.
...
- [ ] Update `findings/PROJ-458_findings.md` to mark F-C-017 (WaterTargetEditor row) `Status: resolved`.
...
- [ ] F-C-017 (WaterTargetEditor) flipped to `Status: resolved` in `findings/PROJ-458_findings.md`.
- [ ] Run `python Tools/test_sharded/test_sharded.py` — full sharded suite green.
- [ ] Run `python Projects/scripts/validate_phase.py PROJ-458 4` — PASSED.
- [ ] Update status at top of this file to `Complete`.
- [ ] Update plan.md phase table row to `Complete`.
- [ ] Update plan.md Current State to point to Phase 5.
- [ ] Commit message: `PROJ-458 Phase 4: retrofit WaterTargetEditor to two-stage bypass-init + dedicated characterization tests`.

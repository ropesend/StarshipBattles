...
| 2026-05-04 | **D-008:** Phase 2 scope reduced from 5 → 3 retrofits after pre-flight | `MoveChoiceWindow` and `PlanetTargetEditor` (base) both lack their own `__init__` — the bypass shell from `StrategyModalWindow` (PROJ-324 Phase 1) already covers them. Marked `no-retrofit-needed` in inventory matrix. Per Decision D-006 from PROJ-327, "no work needed" is a valid outcome and must be documented (not silently dropped). Documented in `docs/known-issues.md`. |
| 2026-05-04 | **D-009:** 4 PlanetTargetEditor concrete subclasses surfaced + deferred | Original `UIWindow`/`StrategyModalWindow` grep missed `AtmosphereTargetEditor`, `GravityTargetEditor`, `WaterTargetEditor`, `RadiationShieldEditor` (transitively-extending classes). They have no UI tests — same rubric as SettingsWindow (D-003): defer until coverage exists. Documented in `docs/known-issues.md`. The inventory grep was extended to include `class.*\(.*PlanetTargetEditor\)` to catch this in future audits. |

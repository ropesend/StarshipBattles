...
- [x] Locate the inline `pygame_gui.UIWindow(...)` construction at line 48
- [x] Extract a class definition above the function:
  ```python
...
- [x] Replace the inline `pygame_gui.UIWindow(...)` call with `MoveChoiceWindow(window_manager=..., ...)` passing the existing kwargs plus the new `window_manager` keyword
- [x] Existing button callbacks (`lambda: (on_move_sector(), win.kill())`) work unchanged — `kill()` now also handles deregistration via the base class
**Notes:**
...
- [x] Delete `move_choice_window: Optional[UIWindow] = None` slot field on `StrategyWindowManager`
- [x] Delete the slot's clause from `has_modal_open()`
- [x] Delete the slot's clause from `_is_blocking_ui_element_at()`
- [x] Delete the `event.ui_element == wm.move_choice_window` branch from `_handle_window_close` (around line 437)
- [x] Update any direct assignments to `wm.move_choice_window = win` in `move_choice_dialog.py` — they're no longer needed (the base class handles registration)
**Notes:**

### Task 6.3: Phase verification [Simple]
**Tests:** `python Tools/test_sharded/test_sharded.py`

- [x] `MoveChoiceWindow` is a `StrategyModalWindow` subclass and registers itself
- [x] Router scans no longer reference `move_choice_window`
- [x] `_handle_window_close` no longer has a branch for it
- [x] Full sharded suite still 15893 passing
- [x] Manual smoke: trigger a move-choice scenario in-game (right-click a sector with multiple targets), verify the dialog opens, click an option, verify the dialog closes and `has_modal_open()` returns False
**Notes:**
...
- [x] Update plan.md Current State to point to Phase 7 (Migrate untracked editor windows)

...
- [ ] Change class declaration to `class OrdersWindow(StrategyModalWindow):`
- [ ] Add import for `StrategyModalWindow`
- [ ] Update `__init__` signature to accept `window_manager: "StrategyWindowManager"` as keyword-only param
- [ ] Pass `window_manager=window_manager` to `super().__init__(...)`
- [ ] In `strategy_event_router.py` find the `OrdersWindow(...)` spawn site (around line 105 of `orders_window_ctrl.py` per audit) and add `window_manager=self.window_manager` argument
- [ ] In `strategy_window_manager.py`, delete the `fleet_orders_window: Optional[UIWindow] = None` slot field
- [ ] In `strategy_event_router.py`, delete the `if self.window_manager.fleet_orders_window is not None:` clause from `has_modal_open()`
- [ ] In `strategy_event_router.py`, delete the same slot from `_is_blocking_ui_element_at()`
- [ ] In `strategy_event_router.py`, delete the `event.ui_element == wm.fleet_orders_window` branch from `_handle_window_close` (around line 413-446)
- [ ] Run `pytest tests/unit/ui/screens/test_orders_window.py` — pass
- [ ] Run `python Tools/test_sharded/test_sharded.py` — 15893 baseline preserved
...
- [ ] Same migration steps as Task 3.1, applied to `TransferDialog`
- [ ] Spawn site: `transfer_dialogs.py:43` per audit
- [ ] Run targeted tests — pass
- [ ] Run full sharded — 15893 preserved
**Notes:**
...
- [ ] Spawn site: `transfer_dialogs.py:69`
- [ ] Run targeted tests — pass
- [ ] Run full sharded — 15893 preserved
**Notes:**
...
- [ ] Same migration steps
- [ ] Spawn site: `selection_prompts.py:45`
...
- [ ] Same migration steps
- [ ] Spawn site: `selection_prompts.py:63`
- [ ] Run targeted tests — pass
- [ ] Run full sharded — 15893 preserved
**Notes:**
...
- [ ] Same migration steps
- [ ] Spawn site: `selection_prompts.py:80`
- [ ] Run targeted tests — pass
- [ ] Run full sharded — 15893 preserved
**Notes:**
...
- [ ] All 6 windows successfully migrated
- [ ] `has_modal_open()` and `_is_blocking_ui_element_at()` chains have shrunk by 6 clauses each
- [ ] `_handle_window_close` has shrunk by 6 branches
- [ ] Full sharded suite still 15893 passing
- [ ] Manual smoke: open and close one of the migrated windows (e.g. transfer dialog) — confirm `has_modal_open()` returns False after close
**Notes:**
...
- [ ] Update plan.md Current State to point to Phase 4 (Migrate dual-cleanup windows)

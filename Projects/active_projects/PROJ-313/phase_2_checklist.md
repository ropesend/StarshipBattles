...
- [ ] Locate `has_modal_open()` (around line 47)
- [ ] At the END of the existing chain (after the last `is not None` check), add a final clause:
  ```python
...
- [ ] Verify no behavioural change — run targeted test before/after Task 2.1, assert same result.
...
- [ ] Locate `_is_blocking_ui_element_at(self, point)` (around line 499)
- [ ] At the END of the existing chain, add the modal-list walk:
  ```python
...
- [ ] Test `test_has_modal_open_returns_true_when_only_modal_list_populated` — populate `_modals` directly with a stub window (no slot fields used), assert `has_modal_open() == True`
- [ ] Test `test_has_modal_open_returns_true_when_only_slot_populated` — populate one slot field directly, assert `has_modal_open() == True`
- [ ] Test `test_has_modal_open_returns_false_when_both_empty` — assert `has_modal_open() == False` when no slots and no modals
- [ ] Mirror the same three tests for `_is_blocking_ui_element_at(point)` with appropriate rect setup
...
- [ ] Run full sharded suite — assert 15893 + N tests passing (N = baseline + Phase 1 new + Phase 2 new)
...
- [ ] Update plan.md Current State to point to Phase 3 (Migrate event-listener-only windows)

...
- [ ] Subclass migration steps
- [ ] Delete `kill()` override at line 554
- [ ] Delete `on_close_callback` param from `__init__`
- [ ] Delete registrar `_on_closed` method
- [ ] Delete slot field, `has_modal_open` clause, `_is_blocking_ui_element_at` clause, `_handle_window_close` clause
...
- [ ] Delete `kill()` override at line 378
- [ ] Run targeted tests — pass
- [ ] Run full sharded — 15893 preserved
**Notes:**
...
- [ ] All 3 windows migrated
- [ ] Router scans shrunk by 3 clauses each; `_handle_window_close` shrunk by 3
- [ ] `settings_window` slot untouched (the only non-modal slot remaining at this point)
- [ ] Full sharded suite still 15893 passing
- [ ] Manual smoke: open Empire Panel, then open Settings from inside it; close both; verify `has_modal_open()` returns False after each
**Notes:**
...
- [ ] Update plan.md Current State to point to Phase 5 (Migrate registrar-callback-only windows)

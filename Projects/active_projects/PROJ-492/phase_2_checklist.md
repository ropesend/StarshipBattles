...
- [ ] Record the canonical signature in this checklist:
  ```
  def _make_mock_fleet(<params>) -> <return type>:
  ```
- [ ] Note all optional kwargs and their defaults.

### Task 2.2: Triage pass — classify all 37 files (exact match only)
**Output:** `findings/triage_results.md` (new)

- [ ] For each file in manifest.md Phase 2 list (37 files, exact match), open it, compare local signature to canonical, classify as A/B/C/D.
- [ ] For each file in manifest.md "Excluded from Phase 2" table, verify the helper is truly sibling (different name suffix or different shape). If a file is actually exact-match `_make_fleet`/`make_fleet`/`_make_mock_fleet`, move to the included list.
- [ ] Record results in `findings/triage_results.md` with the format:
  ```
...
- [ ] Migrate each Category A file: delete local, add canonical import, verify.
- [ ] Work in batches of ~5 files; run tests after each batch.

### Task 2.4: Category B migrations (canonical-superset signature)
**Tests:** `pytest <affected files>` after each batch

- [ ] Same as Task 2.3 — local omitted optional kwargs are filled by canonical defaults.

### Task 2.5: Category C migrations (divergent signature, rewrite call sites)
**Tests:** `pytest <affected files>` after each batch

- [ ] For each Category C file, rewrite call sites in tests to use canonical kwarg names/defaults, then delete local.
- [ ] Verify after each file (don't batch — higher regression risk).

### Task 2.6: Category D — rename for clarity
**Tests:** `pytest <affected files>` after each batch

- [ ] For each Category D file, rename `_make_fleet` → `_make_<purpose>_fleet` (e.g. `_make_cargo_fleet`, `_make_order_fleet`).
- [ ] Add docstring explaining why this isn't the canonical.
- [ ] Update call sites within the file.
- [ ] Verify: tests pass.

### Task 2.7: Final sweep
**Tests:** `python Tools/test_sharded/test_sharded.py`

- [ ] `grep -rl "def _make_fleet\b\|def make_fleet\b\|def _make_mock_fleet\b" tests/` should return only `tests/conftest.py` (canonical) — Category D renames removed the exact-match collision.
...
- [ ] LOC reclaimed recorded in plan.md (expect ~250-350 LOC across 37 files)

...
| `Projects/active_projects/PROJ-313/plan.md` | Tracking | Mark deferred goals; bump `Last verified:` |
...
| File | Type | Notes |
|------|------|-------|
| `game/ui/screens/strategy_modal_window.py` | Production | Remove `= None` default from `window_manager` parameter; keep `Optional` typing |
...
| `game/ui/screens/strategy_windows/move_choice_dialog.py` | Production | `MoveChoiceWindow`: remove `= None` default |
...
| `game/ui/screens/planet_selection_window.py` | Production | **NO CHANGE** — keeps `= None` default (legitimate dual-caller via `BuildQueueScreen`) |
...
| Tests across `tests/unit/ui/screens/` and `tests/integration/ui/` | Test | Surfaced by signature change; add explicit `window_manager=None` or real fixture as appropriate |
...
| `Projects/active_projects/PROJ-316/decisions.md` | Tracking | Document any in-flight decisions made during implementation |

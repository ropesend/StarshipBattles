...
**Active Phase:** Phase 4 complete — moving to Phase 5 (Codex consult).
**Last Action:** Phase 4 legacy retirement landed. Deleted from `transfer_view_model.py`: `get_amounts(info_obj)`, `build_row_data(source_obj, target_obj)`, `_build_pod_rows`, `all_pod_names` constructor param + field. Deleted from `transfer_dialog.py`: `_all_pod_names` property, `_get_amounts` method, `_add_pod_rows` method, the `discover_pod_designs` call in `__init__`. Retired pinning tests: `TestTransferViewModelRows::test_worker_i_transfer_vm_species_key_ordering`, `TestGetAmounts` block, `TestAddPodRows` block (5 tests total). AST guard already pre-existing in `test_no_legacy_storage_fields.py` (PROJ-436 Phase 7) — manifest's `test_no_resource_types_constant.py` skipped as redundant. View model down to 325 LOC (well under ceiling); dialog at 523 LOC (down from 546; remaining 23 LOC over is the additional shim cluster — deferred follow-up). Full suite green: 23307 passed, 2 skipped.
**Next Action:** Phase 5 — end-of-project Codex consult. Run `claude-consult` skill with Codex, mode `pre-final-check`, `--allow-tests`. Verify findings against current code; remediation sub-phase if verified findings warrant. Mark PROJ-437 complete only after consult + verified remediation.

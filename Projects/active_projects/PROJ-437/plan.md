...
| 3. Mixed content display (resources/items/population) through one screen model | In Progress (3a substrate complete; 3b cutover not started) | [phase_3_checklist.md](phase_3_checklist.md) |
...
**Active Phase:** Phase 3 — 3a substrate complete; 3b cutover not started.
**Last Action:** Phase 3a substrate landed: new `build_row_data_from_containers(source_containers, target_containers, *, filter_empty=False) -> list[dict]` classmethod on `TransferViewModel` (delegating to [game/ui/screens/transfer_container_rows.py](../../../game/ui/screens/transfer_container_rows.py)). Row dicts gain an additive `kind: ContainableKind` field. Resources always emit the canonical 8 (matches legacy `build_row_data` UX); population alphabetical; items alphabetical with `drop_pod:<design_id>` prefix (vehicle vs drop-pod discrimination needs `ItemRef.state` on the snapshot — Phase 3b / PROJ-436 substrate change). 9 new tests at [tests/unit/ui/screens/test_transfer_mixed_content.py](../../../tests/unit/ui/screens/test_transfer_mixed_content.py). Dialog still calls the legacy DTO path; cutover is Phase 3b. Module-extraction kept view model at 453 LOC.
**Next Action:** Phase 3b cutover — `TransferDialog._build_grid` switches to call `build_row_data_from_containers` when both sides have container snapshots; renderer `_add_row` learns kind dispatch (still renders identically for Phase 3 — per-kind icon / damage / species visuals are out-of-scope polish). Retire `all_pod_names` + `_build_pod_rows` + back-compat property shims; update legacy DTO tests. Mirror in `strategy_windows/transfer_dialogs.py` if constructor signature changes (it shouldn't).

...
| 3. Mixed content display (resources/items/population) through one screen model | Complete (3a substrate + 3b minimal cutover; legacy retirement is Phase 4) | [phase_3_checklist.md](phase_3_checklist.md) |
...
**Active Phase:** Phase 3 complete (3a substrate + 3b minimal cutover) — moving to Phase 4.
**Last Action:** Phase 3b minimal cutover: `TransferDialog._build_grid` now reads source/target `containers` off the Phase-1b dropdown entries and calls `build_row_data_from_containers(source_containers, target_containers, filter_empty=vm.filter_empty)`. The legacy `vm.build_row_data(source_obj, target_obj)` DTO path is dead code from this commit forward — Phase 4 owns the deletion of `get_amounts(info_obj)`, `build_row_data`, `_build_pod_rows`, `all_pod_names`, plus the dialog's back-compat property shims (`_get_amounts`, `_add_pod_rows`, `_all_pod_names`, `_row_data`, `_current_source`, `_current_target`, `_filter_empty`) and their pinning characterization tests. MagicMock-friendly cutover: MagicMock iterates as empty by default, so characterization tests that don't wire facade.get_containers explicitly see canonical-8 zero rows (same UX as the legacy DTO path with empty DTOs).
**Next Action:** Phase 4 — final UI cutover. Delete the legacy DTO row-builder path + `all_pod_names` + `_build_pod_rows` + `get_amounts(info_obj)` from the view model; delete the dialog back-compat shims; retire the pinning tests in `test_transfer_view_model.py::test_worker_i_transfer_vm_species_key_ordering` and the `TestGetAmounts` / `TestAddPodRows` blocks in `test_transfer_dialog_characterization.py`. Add the AST guard test `tests/static_guards/test_no_resource_types_constant.py` per the manifest (PROJ-436 Phase 7 deleted `RESOURCE_TYPES` already — the guard pins continued absence). Final sharded green at the close.

...
- [x] Add new constructor parameters: `hex_coord=None`, `galaxy=None`, `empire=None`
- [x] When hex context provided, call `collect_build_queues_at_hex(hex_coord, galaxy, empire)` to populate `self.queue_sources: List[BuildQueueSource]`
- [x] When only `build_context` provided (backward compat during transition), create a single BuildQueueSource from it
...
- [x] Default: select first queue source (index 0)
- [x] Remove `self.planet = build_context` backward-compat alias (line 59)
...
- [x] Add `_create_queue_selector_panel()` method
- [x] Position: to the right of context report, full height from top to bottom bar
  - `x = 10 + context_report_width + 10` (= ~500)
  - `y = 10`
  - `width = 200`
  - `height = screen_height - 10 - 80` (same as build queue panel)
- [x] Header: "Build Queues" (UITextBox, 30px height)
- [x] Scrollable container below header for queue entries
- [x] For each `BuildQueueSource` in `self.queue_sources`, create a row:
...
- [x] Store queue selector UI elements for event handling
...
- [x] Reduce context report width: `report_width = 480` (was 580, line 130)
- [x] Queue selector panel: `x = 500, width = 200, full height`
- [x] Adjust filter panel left to remain at `x = 10` (unchanged)
- [x] Adjust filter panel width to `200` (unchanged)
- [x] Adjust available designs panel:
  - `panel_left = 220` (unchanged: 10 + 200 + 10)
  - `panel_width = 280` (was 360, reduced to fit)
- [x] Adjust build queue panel:
...
- [x] Design report: keep at `screen_width - 410` (unchanged, 400w)
- [x] Bottom bar: unchanged
- [x] Verify layout fits at 1920x1080 (710 + 250min + 420 = 1380 < 1920)
- [x] Update all `_create_*_panel` methods with new coordinates
...
- [x] Add `_on_queue_selected(index: int)` method:
...
- [x] Add `_on_queue_toggled(index: int)` method (ctrl+click or checkbox):
...
- [x] Update `_refresh_queue_display()`:
...
- [x] Add `_refresh_queue_selector()` method to update visual selection state of queue list items
- [x] Update queue item count badges when queue contents change
...
- [x] Manual visual test: queue selector appears and responds to clicks (deferred to user verification)

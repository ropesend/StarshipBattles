...
- [x] Write tests for EventLogDataSource:
...
- [x] Define EVENT_LOG_COLUMNS:
  ...
  ```
- [x] Create `EventLogDataSource(ITableDataSource)`:
...
- [x] Verify: `pytest tests/unit/ui/screens/test_event_log_data_source.py -v` passes (40 tests)
...
- [x] Keep filter tab buttons (All, Combat, Production, Colonies)
- [x] Update `_rebuild_list()`:
...
- [x] Keep `get_filtered_events()` as delegate to DataSource for backward compatibility
- [x] Update existing tests (mock structures may need adjustment)
- [x] Verify: `pytest tests/unit/ui/screens/test_event_log_window.py -v` passes (27 tests)
...
- [x] Run `pytest tests/unit/ui/ -v` — all UI tests pass (3192 tests)
- [x] Run `pytest tests/ -n 12` — 12,667 passed, 1 skipped (pre-existing flaky hash test)
- [x] Event log now has virtual scrolling
- [x] Event log now has sortable column headers
...
- [x] EventLogWindow uses VirtualTable + EventLogDataSource + NoSelect
- [x] Event log has virtual scrolling
- [x] Event log has column headers with sort indicators
- [x] Filter tabs still work (All/Combat/Production/Colonies)
- [x] All existing event log tests pass
- [x] No regressions: 12,667 passed, 1 skipped (pre-existing)

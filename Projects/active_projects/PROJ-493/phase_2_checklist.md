...
- [ ] Add module-level `StubValidator` class:
  ```python
  class StubValidator:
  ...
  ```
- [ ] Verify file imports parse and tests still pass (no migrations yet).
...
- [ ] For each of the 3 sites: identify the containing test method.
- [ ] Replace `patch('...SuperweaponValidator.find_ship_with_ability')` block with `processor = SuperweaponOrderProcessor(..., validator=StubValidator(return_ship=...))`.
- [ ] Update assertions: replace `mock.assert_called_with(...)` with `stub.calls == [...]`.
- [ ] Verify each migrated test passes individually.
...
- [ ] `grep -n "patch.*SuperweaponValidator.find_ship_with_ability" tests/unit/strategy/engine/test_superweapon_order_processor.py` should return zero lines.
- [ ] Full test file passes.
- [ ] Run broader engine test suite to catch any cross-file regression.
...
- [ ] No remaining `patch('...SuperweaponValidator.find_ship_with_ability')` in the file
- [ ] Full test suite passes
- [ ] Update status at top of this file to `Complete`
- [ ] Update plan.md phase table row to `Complete`
- [ ] Update plan.md Current State to indicate PROJ-493 ready for audit

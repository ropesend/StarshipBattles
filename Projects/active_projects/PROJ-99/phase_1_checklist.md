...
- [ ] Create new file `game/strategy/engine/empire_economy_calculator.py`
- [ ] Import `dataclass`, `field`, `Dict` from typing, `PLANET_RESOURCES` from `game.core.constants`
- [ ] Define `EmpireEconomySnapshot` dataclass with fields:
...
- [ ] Verify: snapshot can be instantiated with no args and all fields default to empty dict
...
- [ ] Define `EmpireEconomyCalculator` class with `MAINTENANCE_RATE = 0.05`
- [ ] Implement `calculate(self, empire) -> EmpireEconomySnapshot`:
...
- [ ] Implement `_aggregate_colony_production(self, empire) -> Dict[str, float]`:
...
- [ ] Implement `_aggregate_maintenance(self, empire) -> Dict[str, float]`:
  - Initialize zeros
...
- [ ] Implement `_calculate_maintenance_cost(self, design_data) -> Dict[str, float]`:
...
- [ ] Test: empty empire (no colonies, no fleets) → all values are 0.0 for each resource
- [ ] Test: single colony with one facility having `ResourceHarvester` → correct colony_production
- [ ] Test: facility maintenance cost → 5% of resource_cost
- [ ] Test: ship maintenance cost → 5% of resource_cost
- [ ] Test: net_resources = production - expenses
- [ ] Test: current_storage and max_storage copied from empire
- [ ] Test: non-operational facility is skipped
- [ ] Test: both layer formats (dict with `components` key AND direct list)
...
- [ ] All tests pass: `pytest tests/unit/strategy/engine/test_empire_economy_calculator.py -v`
- [ ] No pygame imports in the calculator module

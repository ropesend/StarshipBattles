...
- [ ] Read the file in full. Build a per-method LOC + dependency table:
...
- [ ] Determine which methods truly bind to `self` (i.e., need `self._image_registry`) vs. which are static-like and could become module-level helpers.
- [ ] Pick the cleanest axis or declare "no clean cut" with reasoning.
...
- [ ] Update `findings/PROJ-459_findings.md`: F-A-009 status → "deferred with concrete next-touch criterion (see decisions.md 2026-MM-DD)".
- [ ] Skip Tasks 2.3 + 2.4 + 2.5; proceed directly to Task 2.6 (commit).
...
- [ ] Create the sibling module(s). Naming convention: `planet_gen_<axis>.py` (e.g., `planet_gen_surface.py`, `planet_gen_moons.py`, `planet_gen_orbits.py`).
- [ ] Move the methods. Convert them from `PlanetGenerator` methods to module-level functions taking explicit arguments (the registry, the RNG, the input state). Drop the `self.` references.
- [ ] Update `PlanetGenerator` to call into the new module(s).
- [ ] Preserve seed-determinism: any RNG state passed in must reach the same calls in the same order, or the seeded test fixtures break.
- [ ] Verify imports update cleanly.
...
- [ ] All planet_gen unit tests green.
- [ ] All strategy generation tests green.
- [ ] Sharded suite green; same count as Phase 1.
- [ ] Seed-determinism preserved: a fixed-seed galaxy gen produces the same bodies before and after the split. (Verified by the existing test suite; spot-check one specific seed if needed.)

### Task 2.5: Verify LOC target met [Simple]

- [ ] Re-measure `planet_gen.py`. Target: under 500 LOC.
- [ ] If still over: was the split too small? Pick another axis from Task 2.1. Iterate.
- [ ] Update `findings/PROJ-459_findings.md`: F-A-009 status → "closed via Phase 2 split into planet_gen_<axis>.py".

### Task 2.6: Commit [Simple]

- [ ] Commit message (split branch): `PROJ-459 Phase 2: split planet_gen.py into planet_gen_<axis>.py (closes F-A-009; ~XXX LOC drop)`
- [ ] Commit message (defer branch): `PROJ-459 Phase 2: defer planet_gen.py split with rationale (F-A-009 deferred per decisions.md)`
- [ ] Update `plan.md` Current State.

---

## Phase Completion Checklist
- [ ] planet_gen.py read end-to-end; per-method LOC + dependency table built
- [ ] Decision recorded (split or defer)
- [ ] If split: planet_gen.py under 500 LOC; sibling module(s) created
- [ ] If defer: concrete next-touch criterion in decisions.md
- [ ] Tests green (`pytest tests/unit/strategy/data/test_planet_gen.py tests/unit/strategy/generation/`)
- [ ] Sharded suite green
- [ ] F-A-009 status updated in findings file (closed-via-split or deferred-with-rationale)
- [ ] No behavior change (seed-determinism verified)

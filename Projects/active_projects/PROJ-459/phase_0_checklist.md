...
- [ ] `git log --oneline -50` to see recent commits; confirm a "PROJ-451" merge commit is present (HARD gate). Note whether a "PROJ-449" merge commit is present (advisory for Phases 1-2, hard for Phase 3).
- [ ] Check `Projects/archived_projects/` for archived PROJ-449 and PROJ-451 directories (or active with all phases complete).
- [ ] If **PROJ-451** is still in flight: stop. Phase 0 cannot complete. Update `plan.md` Current State with a blocker note and exit.
- [ ] If only **PROJ-449** is still in flight: Phases 1 and 2 can proceed; Phase 3 must wait. Record this disposition in `findings/phase_0_remeasurement.md` so the downstream agent knows to pause before Phase 3.
...
- [ ] Compare against the 2026-05-19 baseline:
  - fleet.py: 686 LOC (baseline)
  - planet_gen.py: 610 LOC (baseline)
  - ship_instance.py: 839 LOC (baseline)
- [ ] Note any drift (PROJ-449 / PROJ-451 may have grown or shrunk fleet.py independently of the wrapper work; ship_instance.py is expected to DROP).

### Task 0.3: Verify Phase 1 extraction target locations [Simple]

**File:** `game/strategy/data/fleet.py`

- [ ] Grep for `def to_dict` and `def from_dict` in fleet.py; confirm both still exist on `Fleet`.
- [ ] Confirm `Fleet.to_dict` body is still ~30 LOC (per 2026-05-19 sample: fleet.py:520-557 = 37 LOC).
- [ ] Confirm `Fleet.from_dict` body is still ~80 LOC (per 2026-05-19 sample: fleet.py:558-655 = 98 LOC).
- [ ] Confirm `Fleet.resolve_order_references` still exists (per 2026-05-19 sample: fleet.py:657).
- [ ] Record in `findings/phase_0_remeasurement.md` whether the extraction surface matches expectations or has drifted.

### Task 0.4: Verify Phase 2 split candidates in planet_gen.py [Simple]

**File:** `game/strategy/data/planet_gen.py`

- [ ] Grep `^    def ` in planet_gen.py; record the method list and approximate per-method LOC bands.
- [ ] Confirm the 4 candidate split axes from `findings/PROJ-459_findings.md` (orbital arrangement / moon generation / body construction / surface-type-resource) still match the structure.
- [ ] Note if any new methods have appeared.

### Task 0.5: Re-measure ship_instance.py and inspect post-PROJ-449 surface [Simple]

**File:** `game/strategy/data/ship_instance.py`

- [ ] Re-measure LOC (this is the gate for Phase 3).
- [ ] Read `ship_instance.py` end-to-end to confirm which shims PROJ-449 actually retired vs. which survive:
...
- [ ] Estimate the LOC contribution of any surviving shims (block of lines + comment narration).
- [ ] Provisional Phase 3 verdict:
...
- [ ] Commit the findings file. No production code touched.
- [ ] Update `plan.md` Current State.

---

## Phase Completion Checklist
- [ ] PROJ-451 confirmed merged into main (HARD gate)
- [ ] PROJ-449 status recorded: merged (Phase 3 can proceed), or in-flight (Phases 1-2 still OK; Phase 3 pauses)
- [ ] LOC re-measured for fleet.py, planet_gen.py, ship_instance.py
- [ ] Extraction targets confirmed at expected locations (Phase 1)
- [ ] Split candidates confirmed in planet_gen.py (Phase 2)
- [ ] Phase 3 provisional verdict recorded (or "pending PROJ-449" if PROJ-449 not yet merged)
- [ ] `findings/phase_0_remeasurement.md` committed

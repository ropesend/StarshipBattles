...
- [ ] Create the test file. Add `test_fleet_to_dict_byte_identical_pre_extraction` that:
...
- [ ] Add `test_fleet_round_trip` that:
...
- [ ] Run the test. It should pass against current code (pre-extraction baseline). This is the **characterization step**, not a RED step — the test capturing current behavior is exactly the point.
- [ ] Capture the to_dict output string into the test file as a frozen comparison constant for the post-extraction assertion.
...
- [ ] Note the public surface: `planet_to_dict(planet) -> Dict[str, Any]`, `planet_from_dict_kwargs(data: dict) -> Dict[str, Any]`, `_deserialize_planet_orders(orders_data, planet_name)` (module-private helper).
- [ ] Note the import pattern: `TYPE_CHECKING` block at the top; lazy imports inside `planet_from_dict_kwargs` for runtime dependencies (`PersistenceException`, `ErrorCode`, `deserialize_list`, `PlanetType`, `PlanetaryFacility`, `SpeciesPopulation`).
- [ ] Note the use of `validation_helpers` (`require_keys`, `validate_enum`, `validate_positive`, `validate_non_negative`).
- [ ] Confirm `Planet.to_dict` / `Planet.from_dict` are 1-line facades calling into this module. (Quick read of `game/strategy/data/planet.py` to confirm the wrapper shape.)
...
- [ ] Create the new file with a module docstring modeled on `planet_serde.py`'s:
  ```python
  ...
  """
  ```
- [ ] Add `TYPE_CHECKING` block importing `Fleet`, `ShipInstance`, `GameRegistries` (and any other type-only refs).
- [ ] Write `fleet_to_dict(fleet: "Fleet") -> Dict[str, Any]`:
...
- [ ] Write `fleet_from_dict_kwargs(data: dict, registries: "GameRegistries") -> Dict[str, Any]`:
...
- [ ] Write sibling helpers used by `Fleet.from_dict` for post-construction hydration. At minimum:
...
- [ ] Add `__all__` exporting `fleet_to_dict`, `fleet_from_dict_kwargs`, and any helper functions `Fleet.from_dict` needs (e.g., `_deserialize_fleet_ships`).
...
- [ ] Replace `Fleet.to_dict` body (fleet.py:520) with:
  ```python
  ...
  ```
- [ ] Replace `Fleet.from_dict` body (fleet.py:558) with the split-call shape (matches the Task 1.3 design):
  ```python
  @classmethod
...
- [ ] Audit whether `resolve_order_references` (fleet.py:657) belongs in fleet_serde.py or stays on Fleet. The planet_serde precedent keeps deserialization helpers in the serde module; order references resolved at load time are part of the same conceptual surface.
- [ ] Re-run the byte-identical save-output test from Task 1.1. It MUST pass — same dict before and after extraction.
...
- [ ] Save-load tests green.
- [ ] Sharded suite green; same count as Phase 0 baseline.
- [ ] Re-measure fleet.py LOC (PowerShell): `(Get-Content game/strategy/data/fleet.py | Measure-Object -Line).Lines`. Target: ~545 LOC (down from 686). Acceptable: under 600.
- [ ] Measure fleet_serde.py LOC: `(Get-Content game/strategy/data/fleet_serde.py | Measure-Object -Line).Lines`. Target: ~150 LOC (in the same ballpark as planet_serde.py's 219).
- [ ] Update `findings/PROJ-459_findings.md`: F-A-008 status → "closed via Phase 1 extraction; fleet_serde.py created; save-format byte-identical".

### Task 1.6: Docs sync [Simple]

**Files:** `docs/02_PATTERNS.md`, `docs/01_ARCHITECTURE.md`

- [ ] Update `docs/02_PATTERNS.md` if it references the planet_serde pattern as a "single instance"; now there are two. (Check first; the file may already describe the pattern generically.)
- [ ] Update `docs/01_ARCHITECTURE.md` strategy/data/ listing to include `fleet_serde.py` alongside `planet_serde.py`.
- [ ] Verify the docs reflect current state.

### Task 1.7: Commit [Simple]

- [ ] Commit message: `PROJ-459 Phase 1: extract Fleet.to_dict/from_dict to fleet_serde.py (closes F-A-008; ~140 LOC drop)`
- [ ] Update `plan.md` Current State.

---

## Phase Completion Checklist
- [ ] fleet_serde.py created (~150 LOC) modeled on planet_serde.py
- [ ] fleet.py drops to under 600 LOC (target ~545)
- [ ] `Fleet.to_dict` / `Fleet.from_dict` are 1-line facades
- [ ] Save-load round-trip byte-identical
- [ ] `pytest tests/integration/save_load/ tests/unit/strategy/data/test_fleet.py` green
- [ ] Sharded suite green
- [ ] F-A-008 marked closed in findings file
- [ ] Docs updated

...
| 2. Same recipe for `planet_list_window.py` (862 → under 500) | Not Started | [phase_2_checklist.md](phase_2_checklist.md) |
...
**Active Phase:** Phase 1 (build_queue_screen.py extraction)
**Last Action:** Phase 0 complete: re-measured all 12 F-C-027 files post-PROJ-456 merge (`244c1fa16`). All 12 remain OVER 500 LOC; no top-3 rescope needed. Findings doc at `findings/post_proj_456_loc_remeasure.md`. User decision 2026-05-19: Phase 4 DROPPED (exceptions.py is already under 500 LOC; the re-export aggregator would be a compat shim disfavoured by CLAUDE.md Rule 4; large blast radius for no functional change).
**Next Action:** Phase 1 — extract one responsibility from `build_queue_screen.py` (958 → under 500).

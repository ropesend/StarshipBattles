...
- [ ] Check `Projects/active_projects/PROJ-456/plan.md` Quick Status table — all 5 phases marked `Complete`.
- [ ] Check `Projects/projects_index.md` — PROJ-456 row marked `Complete` and `verified` (or at least `awaiting-confirmation`).
- [ ] Run `python Tools/test_sharded/test_sharded.py` — sharded suite green at the post-PROJ-456 baseline.
- [ ] If PROJ-456 has not yet completed, STOP: PROJ-457 cannot start until PROJ-456 is in.
...
- [ ] Run (PowerShell, PS 5.1 compatible — this repo's shell is Windows PowerShell):
  ```powershell
  $files = @(
...
- [ ] Record results in `findings/post_proj_456_loc_remeasure.md` with a table comparing pre-PROJ-456 (2026-05-19) LOC to post-PROJ-456 LOC and the delta.
- [ ] Identify which files are now under 500 LOC (naturally compliant — no longer in F-C-027 scope) and which still over-run.

### Task 0.3: Rescope Phases 1-3 if needed [Simple]
**File:** `Projects/active_projects/PROJ-457/plan.md` Quick Status table.

- [ ] **ESCALATION GATE**: If ANY of the top-3 target files (`build_queue_screen.py`, `planet_list_window.py`, `test_lab/screen.py`) is already under 500 LOC at this re-measurement, **STOP and surface to the user** before proceeding with that phase's extraction. The "ceiling enforcement" rationale is gone; force-splitting a file just because it was scoped is the exact anti-pattern Codex r4 warned against ("F-A-007 should not be smuggled in as a side quest"). Document the user's decision in `decisions.md`: (a) drop that phase entirely, (b) replace the target with a next-worst offender from the remaining 9, or (c) proceed with an explicit architectural-cleanup rationale documented in plan.md.
- [ ] (For reference — same pattern as F-C-028 in Phase 4: that file is already 411 LOC at HEAD, under the 500 ceiling, and Phase 4 is PENDING USER DECISION. Apply the same escalation discipline here.)
- [ ] If `build_queue_screen.py` < 500: STOP per the escalation gate above. If user approves replacement: pick the next-worst-offender from the remaining 9.
- [ ] If `planet_list_window.py` < 500: STOP per the escalation gate above.
- [ ] If `test_lab/screen.py` < 500: STOP per the escalation gate above.
- [ ] If all 12 are now under 500 LOC: close PROJ-457 except for Phase 4 (exceptions split, which is independent and ALSO under the ceiling — pending user decision) and the Phase 5 "next-touch" rule documentation (which can also be skipped if no files remain over-ceiling).
- [ ] Document the rescope rationale in `decisions.md` with a 2026-05-XX row.
- [ ] Update `plan.md` Quick Status, Current State, and the per-phase target file references.
...
- [ ] For each of the (possibly rescoped) Phase 1 / 2 / 3 targets, do a read-only audit: identify 2-3 candidate cohesive responsibilities that could be extracted to a sibling module. Record candidates in `decisions.md` along with the expected post-extraction LOC for each candidate.
- [ ] The actual choice is made at Phase 1 / 2 / 3 start; this audit just gives the executing agent a head start.
...
- [ ] `findings/post_proj_456_loc_remeasure.md` committed with 12-file LOC table + deltas.
- [ ] `plan.md` Quick Status table updated if any rescope happened.
- [ ] `decisions.md` carries 1-2 rows documenting the rescope rationale + candidate-extraction audit.
- [ ] Run `python Projects/scripts/validate_phase.py PROJ-457 0` — PASSED.
- [ ] Update status at top of this file to `Complete`.
- [ ] Update plan.md phase table row to `Complete`.
- [ ] Update plan.md Current State to point to Phase 1.
- [ ] Commit message: `PROJ-457 Phase 0: re-measure 12 F-C-027 files post-PROJ-456 (rescope decisions documented)`.

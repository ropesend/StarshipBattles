...
- [ ] Run (PowerShell, PS 5.1 compatible — `wc.exe` from Git-for-Windows crashes on this checkout per the codex r5 audit; see also `phase_0_checklist.md` Task 0.2 for the same pattern):
  ```powershell
  $files = @(
  ...
  ```
- [ ] Cross-check against the 2026-05-19 reference values. After PROJ-456 ships and Phases 1-3 land, some of these may also have shrunk incidentally (e.g. `event_log_window.py` had the F-C-012 empire_name fix in PROJ-456 Phase 1 — small change).
- [ ] If any are now under 500 LOC, drop them from the "next-touch" list. Document the drop in `decisions.md`.

### Task 5.2: Append the "next-touch" rule to decisions.md [Simple]
**File:** `Projects/active_projects/PROJ-457/decisions.md`

- [ ] Append the decision row using this template (adjust LOC values to the Task 5.1 re-measurement):
  ```
  ...
  ```
- [ ] Include the actual LOC values from Task 5.1.

### Task 5.3: Decide whether CLAUDE.md / AGENTS.md need the cross-reference [Simple]
**Files:** `CLAUDE.md`, `AGENTS.md`.

- [ ] Read the "Key Conventions" section of `CLAUDE.md` and the "Critical Conventions" section of `AGENTS.md`. Both list the 500-LOC ceiling rule.
- [ ] **Decision**: option (a) add a one-line cross-reference in both docs naming the `decisions.md` entry; option (b) leave the docs alone and rely on agents to discover the rule when they touch a flagged file.
- [ ] **Recommendation**: (a) if the rule is repo-wide (apply to all files, not just F-C-027 ones), but (b) if the rule is bounded to the 9 named files. Given Codex r4's framing ("next touch"), this is a file-specific rule — (b) is the cleaner choice.
- [ ] Record the decision in `decisions.md`. If (a), edit both docs and bump their "Last verified" dates.
...
- [ ] If Task 5.1 found any of the 9 were already under 500, mark those as `Status: resolved (naturally; no extraction required)`.

### Task 5.5: PROJ-457 project-completion verification [Simple]

- [ ] All 2 owned findings (F-C-027, F-C-028) closed in `findings/PROJ-457_findings.md`.
- [ ] All 5 phases (0-5) marked `Complete` in `plan.md` Quick Status table.
- [ ] Run `python Tools/test_sharded/test_sharded.py` — full sharded suite green.
- [ ] Update `Projects/projects_index.md` PROJ-457 row to `Complete`.
...
- [ ] F-C-027 / F-C-028 final disposition recorded in `findings/PROJ-457_findings.md`.
- [ ] `decisions.md` carries the "next-touch" rule decision row.
- [ ] `CLAUDE.md` / `AGENTS.md` updated if Task 5.3 chose option (a).
- [ ] Run `python Projects/scripts/validate_phase.py PROJ-457 5` — PASSED.
- [ ] Update status at top of this file to `Complete`.
- [ ] Update plan.md phase table row to `Complete` (and mark PROJ-457 as Project Complete in `plan.md` Current State).
- [ ] Update `Projects/projects_index.md` PROJ-457 row to `Complete`.
- [ ] Commit message: `PROJ-457 Phase 5: document 9 remaining over-ceiling UI files as next-touch rule; PROJ-457 complete`.
...
- [ ] All 6 phase checklists (0-5) complete.
- [ ] Final sharded-suite green run.
- [ ] (PowerShell-safe) `(Get-Content game/ui/screens/build_queue_screen.py | Measure-Object -Line).Lines` < 500.
- [ ] (PowerShell-safe) `(Get-Content game/ui/screens/planet_list_window.py | Measure-Object -Line).Lines` < 500.
- [ ] (PowerShell-safe) `(Get-Content game/ui/screens/test_lab/screen.py | Measure-Object -Line).Lines` < 500.
- [ ] (PowerShell-safe) `(Get-Content game/core/exceptions.py | Measure-Object -Line).Lines` < 80.
- [ ] 5 new exceptions submodules each under 200 LOC.
- [ ] All 250+ existing `from game.core.exceptions import ...` callers still working.
- [ ] "Next-touch" rule documented and referenced if appropriate.

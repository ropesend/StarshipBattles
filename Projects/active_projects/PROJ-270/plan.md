...
| **10. Visual-mode contract completion + shim eradication** | **Mostly complete** — 10.1/10.2/10.5/10.6/10.7 done; 10.3/10.4 scope-trimmed | [phase_10_checklist.md](phase_10_checklist.md) |
| **11. Test + doc hardening** | **Substantially complete** — 11.1/11.2/11.3/11.4/11.7/11.8/11.9/11.10 done; 11.5/11.6 covered by earlier | [phase_11_checklist.md](phase_11_checklist.md) |
| **12. Dead scaffolding + type-model cleanup** | **Partial** — 12.5/12.6 done; 12.4 documented; 12.1/12.2/12.3 deferred (wide fanout) | [phase_12_checklist.md](phase_12_checklist.md) |

## Current State
**Last Updated:** 2026-04-12 — Phases 9-12 substantially complete; only manual launcher smoke + archival remain
**Active Phase:** Phase 8 — Final cleanup + acceptance audit (awaiting manual smoke)
**Last Action:** Second `/proj-continue` autonomous session powered through Phases 10-12. Delivered: **Phase 10** — `BattleController.start_from_spec` + BattleService.adopt_started_engine helper, migrated 3 visual call sites (app.py, test_lab/screen.py, test_execution_service.py) to route through `start_engine_from_spec`, eliminating duplicated engine.boundary/modifier_stack plumbing; deleted `ReturnDestination` re-export from battle_config.py; fixed `self.engine.update()` bypass in `BattleScreen._run_single_tick` (now raises StateException if controller is None); added `TestNoDirectEngineTickLoop` regression guard. **Phase 11** — strengthened `test_outcome_emission` with real end-to-end outcome content assertion; converted `TestNoLegacyScenarioSetup` from brittle regex to AST walker; widened `TestNoLegacyCompatibleComments` pattern + scope with `NOQA: legacy-retained` escape hatch; added `TestStrategyCompilerBehavioralStatKeys` (3 behavioral tests); added `TestCircleBoundaryOriginConvention` + `TestRectBoundaryCenterDeterminism` for origin-ambiguity cases; fixed critical doc drift (simulation_testing.md, COMBAT_LAB_DOCUMENTATION.md, 01_ARCHITECTURE.md). **Phase 12** — added FORBIDDEN_FIELDS sunset dates to test_battle_config; fixed stale docstring in test_executor.py; documented BoundaryRegion type-split as intentionally deferred. Regression: pytest 14637+ passed, Combat Lab 162 fast + 170 full green, 17+ regression guards all green. 71 BattleScreen.start callers + 43 AIPolicy refs + 51 TaskForceOutcome refs intentionally deferred per scope constraints.
**Next Action:** Manual launcher smoke (Combat Lab visual+headless+batch, Battle Setup 2v2 complex toggles, Strategy fleet conflict). Then archive PROJ-270 via protocol 05.
**Blockers:** Only manual launcher smoke remains.

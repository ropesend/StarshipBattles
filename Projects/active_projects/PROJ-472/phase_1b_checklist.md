...
- [ ] Write a FAILING unit test asserting `BuildQueueSourceDTO.from_domain(...)`
...
- [ ] Add `is_paused: bool` to the frozen DTO and populate it in `from_domain`
      from `source.is_paused`
- [ ] Add the **exact** owner-derived SCALAR fields below. Do NOT add a live
...
- [ ] Confirm the DTO stays `frozen=True` and keeps deep-copying
...
- [ ] Verify the new test passes; existing facade DTO tests stay green
...
- [ ] Remove the runtime import `from game.strategy.data.build_queue_source import
      BuildQueueSource, collect_build_queues_at_hex` (`build_queue_screen.py:23`)
- [ ] Replace `collect_build_queues_at_hex(...)` calls (`:214-217`, `:333-336`)
      with `facade.empires.hex_build_queues(empire_id, hex_coord)` returning
      `BuildQueueSourceDTO`
- [ ] Migrate `empire_build_queue_window.py` `owner_entity` reads to the enriched
...
- [ ] Remove these files' TEMPORARY allowlist entries from the 1A import guard
- [ ] Verify the import guard now passes WITHOUT those allowlist entries
...
- [ ] `build_queue_controller.py`: migrate the live-`BuildQueueSource` class
...
- [ ] `build_queue_input_router.py`: `source.owner_entity` (`:128`,`:164`),
      `source.is_paused` (`:174`), `source.construction_queue` (`:83-84`) → DTO fields
- [ ] `empire_build_queue_formatter.py`: migrate the `owner_entity`-derived
...
- [ ] Migrate the remaining viewmodels/data-sources/selector/factory/
      renderer to consume `BuildQueueSourceDTO` instead of domain `BuildQueueSource`
- [ ] `strategy_build_queue_manager.py`: migrate ONLY its BQS-import usage here;
...
- [ ] Remove each migrated file's TEMPORARY 1A import-guard allowlist entry
- [ ] Verify NO file in the cluster still imports domain `BuildQueueSource` /
...
- [ ] Do NOT migrate these in 1B. Keep them as `FleetCapabilityCalculator`
      late-imports.
- [ ] Add/keep BOTH files on the 1A import-guard allowlist as
...
- [ ] Record the deferral in PROJ-475's plan (remaining live-reader tail) so the
...
- [ ] `pytest tests/ --testmon` green for touched files
- [ ] Import guard passes with the build-queue CLUSTER allowlist entries REMOVED
...
- [ ] No domain `BuildQueueSource`/collectors remain in the cluster; no mixed
      domain+DTO representation left in the feature
- [ ] Caller read-set fully satisfied by the enriched DTO. Verified against the
...
- [ ] Determinism/save-compat unchanged (projection-only; no serialized changes)
...
- [ ] Update plan.md phase table row 1B to `Complete`
- [ ] Update plan.md Current State to point to Phase 1C

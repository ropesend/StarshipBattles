...
| 2026-05-17 | Phase IDs follow source TD-04 numbering verbatim (phase_0..phase_5) | The earlier renumbering to phase_1..phase_6 was reverted after scaffold review. The 03c protocol does not require 1-indexed phase IDs (`Projects/protocols/03c_phase_aware_execution.md`); sibling projects in the same batch (PROJ-422, PROJ-426, PROJ-429) use phase_0. Convention: phase IDs follow the source plan's phase numbers; no renumbering for convenience. |
| 2026-05-16 | Phase 0 is test-only | Characterization tests must fail for the intended reason and pass against the current code before any extraction begins. No production changes in Phase 0. |
| 2026-05-17 | Add Phase 6 to harden the registry-purity AST guard | Codex-consult finding: the Phase 4 guard inspected only `tree.body` top-level `Import` / `ImportFrom` nodes, so class-body imports and dynamic `importlib.import_module(...)` / `__import__(...)` calls at module load were bypass vectors. Hardening mirrors PROJ-424 Phase 7's lazy-import guard pattern at `tests/unit/strategy/engine/commands/test_order_metadata_view.py`. Tests-only phase, lightweight review. |

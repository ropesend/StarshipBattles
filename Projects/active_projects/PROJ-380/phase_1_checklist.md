...
- [x] Remove unused `IControllableShip` import (line 56) from the `if TYPE_CHECKING:` block — symbol does not exist in `game/simulation/interfaces/ai_controller.py` (mypy: `attr-defined` error)
- [x] Update the stale string annotation `ship: 'IControllableShip'` (line 86) to `ship: 'ShipControllableAdapter'` (already imported at line 69 and is the runtime type passed by callers)
- [x] Confirm `mypy game/ai/controller.py` no longer reports the `attr-defined` error (IDE diagnostic confirmed cleared after annotation update)
- [x] Verify: focused tests pass (62/62 in tests/unit/ai/ -k controller); no new mypy/import errors elsewhere; LOC delta ≈ −1 line

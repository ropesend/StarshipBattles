...
- [x] Add constructor parameters with backward-compatible defaults:
  ```python
  ...
  ```
- [x] Line 15: Replace hardcoded string with `window_title` parameter:
  ```python
  super().__init__(rect, manager, window_display_title=window_title)
  ```
- [x] Line 29: Replace hardcoded label with `list_label` parameter:
  ```python
  self.label = UILabel(..., text=list_label, ...)
  ```
- [x] Conditionally create `btn_any` based on `show_any_button`:
  ```python
  self.btn_any = None
  if show_any_button:
      self.btn_any = UIButton(..., "Any Planet", ...)
  ```
- [x] Guard `btn_any.check_pressed()` (line 131):
  ```python
  if self.btn_any and self.btn_any.check_pressed():
  ```
- [x] Verify: Existing colonization calls still work (defaults match current behavior)

**Notes:** 4 tests added for backward compatibility verification.
...
- [x] Verify `prompt_planet_selection()` call in strategy_ui.py uses PlanetSelectionWindow with default params
- [x] Run colonization-related tests to ensure no breakage
- [x] Verify: Colony ship at multi-planet hex still shows planet selection window correctly

**Notes:** 177 colonization tests pass.
...
- [x] Add `on_planet_selection_needed: Optional[Callable] = None` parameter to `__init__`
- [x] Add `hex_coord` and `galaxy` parameters to `__init__` (needed for planet lookup):
  ```python
  def __init__(self, ..., hex_coord=None, galaxy=None, empire=None,
               on_planet_selection_needed=None):
  ```
- [x] Add helper `_needs_planet_selection(self, source, category) -> bool`:
  ```python
  ...
  ```
- [x] In `_add_to_single_queue()`: when `_needs_planet_selection()` returns True:
...
- [x] When only 1 planet at hex, auto-set `target_planet_id` without prompting
- [x] For planet-based sources (planet_id is set), auto-set `target_planet_id=source.planet_id`
- [x] Verify: Fleet shipyard + complex + multi-colony = callback triggered

**Notes:** Added `_needs_planet_selection()`, `_get_target_planet_id()`, and `_add_item_with_target_planet()` helper methods. 7 new tests.
...
- [x] Pass `hex_coord`, `galaxy`, `empire` to `BuildQueueController.__init__()` (line 138-144)
- [x] Add method `_prompt_target_planet(self, planets, on_selected)`:
  ```python
  ...
  ```
- [x] Pass `on_planet_selection_needed=self._prompt_target_planet` to controller
- [x] Add import for `PlanetSelectionWindow` at top of file
- [x] Verify: Adding complex to fleet shipyard at multi-colony hex opens planet selection

**Notes:** Also added cleanup in `_close()` to kill planet selection window if open.
...
- [x] In `_spawn_fleet_complex()` (line 460-523):
...
- [x] Verify: Complex spawns on correct planet when target_planet_id is set

**Notes:** Updated both call sites to pass target_planet_id. 3 new tests.
...
- [x] Test PlanetSelectionWindow with `window_title="Test"` and `show_any_button=False`
- [x] Test controller `_needs_planet_selection()` returns True for fleet+complex+multi-colony
- [x] Test controller `_needs_planet_selection()` returns False for planet-based source
- [x] Test controller triggers `on_planet_selection_needed` callback when needed
- [x] Test controller auto-sets `target_planet_id` for single-colony hex
- [x] Test `_spawn_fleet_complex()` uses `target_planet_id` when present
- [x] Test `_spawn_fleet_complex()` falls back to first planet when absent (legacy)
- [x] Run: `pytest tests/ --testmon`

**Notes:** 14 new tests total: 4 for PlanetSelectionWindow, 7 for controller, 3 for ProductionEngine.
...
- [x] Manual test: Fleet shipyard at multi-colony hex, add complex -> planet selection popup (deferred to user)
- [x] Manual test: Colonization planet selection still works correctly (177 tests pass)

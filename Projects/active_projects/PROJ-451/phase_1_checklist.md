...
- [x] Add test:
  ...
  ```
- [x] Fixture setup: use `tests/fixtures/strategy_entities.py` helpers to build a minimal empire + fleet + cargo state
- [x] Run; verify RED — the test fails because no RESOURCE_SHORTAGE event is emitted today
...
- [x] **Step 1 — Targeted raw RED run.** Run the focused pytest invocation above WITHOUT any xfail markers on either test. Confirm BOTH tests fail (this is the literal RED proof; without this step, an xfail-on-passing-test could silently hide an already-green contract).
- [x] **Step 2 — Add xfail markers.** Once RED is proven, add `@pytest.mark.xfail(reason="PROJ-451 Phase 1 RED — fixed by Phase 2")` to both tests. This is the standard pattern for committing failing tests on `main` while keeping the sharded suite green.
- [x] **Step 3 — Targeted xfail run.** Re-run the focused pytest; both tests now show `xfailed`.
- [x] **Step 4 — Sharded suite green.** Run sharded; the suite passes at +2 xfail-passing tests.
- [x] **Step 5 — Commit.** Commit message: `PROJ-451 Phase 1 RED: reproduction tests for DI-006 engine UX gap + zero-consume detection (xfail-marked; Phase 2 GREEN removes markers)`. The commit captures both the raw-failing test bodies AND the xfail markers; the targeted-raw-RED step is recorded in the commit message body for the audit trail.
...
- [x] `test_fractional_cost_rounds_to_zero_emits_resource_shortage` exists and is RED (`xfail` preferred)
- [x] `test_apply_resource_consumption_emits_shortage_on_zero_consume` exists and is RED (`xfail` preferred)
- [x] Sharded suite green (with the 2 new tests as `xfail`-passing)
- [x] Plan.md Quick Status → Complete; Current State updated → ready for Phase 2 GREEN

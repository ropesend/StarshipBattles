...
| 2. GREEN — emit RESOURCE_SHORTAGE when affordability passes but consumption charges 0 | Complete | [phase_2_checklist.md](phase_2_checklist.md) |
| 3. Decide: `_apply_resource_consumption` bool-return handling | Complete (option B — strict assertion) | [phase_3_checklist.md](phase_3_checklist.md) |
| 4. Stocked-fleet ratchet tests for `IProductionResourceSource` implementers | Complete | [phase_4_checklist.md](phase_4_checklist.md) |

## Current State
**Last Updated:** 2026-05-18
**Active Phase:** All phases complete; codex audit next
**Last Action:** Phase 4 complete. New ratchet at `tests/unit/strategy/data/test_production_resource_source_ratchet.py` (17 tests) pins the affordability→consumption symmetry contract at the implementer level for both Planet (float stockpile) and Fleet (integer cargo with int(round(...)) rounding). Defense-in-depth alongside Phase 3's engine-side assertion. Sharded 23395/23395 GREEN.

Phase 3 complete — option B (strict assertion) chosen per Codex r4 + CLAUDE.md "Capability validation is hard, not soft." `_apply_resource_consumption` now asserts `production_consume_resource` returns True; failure surfaces a programmer error in the implementer rather than silently burning tick_capacity. New test `test_apply_resource_consumption_raises_on_contract_breach`. Side fix: `_make_colony` in `tests/unit/strategy/production_engine/test_paused_queue.py` now mirrors real `Planet.consume_from_stockpile` bool return. Sharded 23378/23378 GREEN. Closes DI-007 + F-B-019.

Phase 2 GREEN complete. Task 2.0: rounded the `Fleet.consume_cargo_resource` gate at `fleet.py:285` to match `has_cargo_resources`; closes the DI-006 data-half asymmetry. New ratchet at `tests/unit/strategy/data/test_fleet_consume_cargo_symmetry.py`. Task 2.1: added `_log_zero_consume_shortage` sibling emit path; `_apply_resource_consumption` now collects `actually_consumed == 0` resources and routes to the new emit. Task 2.2: removed `@pytest.mark.xfail` from all 3 Phase 1 RED tests — all GREEN unconditionally. Task 2.5: module docstring updated to drop "empire pool" framing; now points at `IProductionResourceSource`. Side fix: `tests/unit/strategy/engine/test_production_refactor.py::colony` fixture's `production_consume_resource` now actually mutates the stockpile dict (previously a MagicMock no-op caused the new zero-consume detection to fire spuriously). Sharded 23377/23377 GREEN.

Phase 1 RED complete. Added 3 xfail-strict reproduction tests: (1) `TestFractionalCostRoundedToZeroEmitsResourceShortage::test_engine_level_zero_consume_emits_resource_shortage` and (2) `TestQueueLoopRoundedToZeroEmitsResourceShortage::test_fractional_per_step_cost_against_int_cargo_emits_resource_shortage` in `tests/integration/test_production_engine_fractional_fleet_cost.py`; (3) `test_apply_resource_consumption_emits_shortage_on_zero_consume` in `tests/unit/strategy/engine/test_production_engine_consumption.py`. All three confirmed RED via `--runxfail`, then xfail-strict markers added. Sharded 23373 tests | 23370 passed | 0 failed | 0 errors | 3 xfailed.

...
**Next Action:** Execute Phase 4 — add stocked-fleet ratchet tests for `IProductionResourceSource` implementers (Planet, Fleet) to enforce the affordability→consumption symmetry contract at the implementer level. The engine's assertion (Phase 3) is then defense-in-depth, not the primary enforcement.

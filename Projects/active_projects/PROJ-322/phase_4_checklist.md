...
**Status:** Partial (4/9 done — os.utime/doc; 5 mocked-clock tasks deferred)
...
- [ ] S12-CAT7-001: **Replace the polling sleep loops with a mocked clock** (e.g., `freezegun.freeze_time` or a manual `time.monotonic` patch). Do NOT modify `game/services/llm/background.py` to add a `threading.Event` — that's a production change out of P1 test-cleanup scope. Keep deadlines as safety nets where they remain meaningful under a mocked clock. _(deferred — requires freezegun mocked clock or threading.Event helper; out of safe-pass scope)_
- [ ] Fallback: if mocked clock is infeasible (e.g., the polling logic uses a non-monotonic clock), skip the test with `@pytest.mark.skip(reason="Polling-loop test pending production-side Event API; tracked in PROJ-XXX")` and create a follow-up project for the production change. _(deferred — requires freezegun mocked clock or threading.Event helper; out of safe-pass scope)_
- [ ] Verify: `pytest tests/unit/services/llm/test_background.py` passes; LOC delta approximately -20 _(deferred — requires freezegun mocked clock or threading.Event helper; out of safe-pass scope)_
...
- [ ] S03-CAT7-001: replace `time.sleep(0.02)` (line 142) and the `duration_ms > 15` assertion with a mocked clock or `freezegun`. _(deferred — requires freezegun mocked clock or threading.Event helper; out of safe-pass scope)_
- [ ] Verify: `pytest tests/unit/services/llm/test_decorators.py` passes; LOC delta approximately -2 _(deferred — requires freezegun mocked clock or threading.Event helper; out of safe-pass scope)_
...
- [ ] S03-CAT7-002: replace `time.sleep(0.05)` (line 96) and the `45 < duration < 100` two-bound assertion with a mocked clock; both bounds can fail under heavy CI load or Windows 15.6ms timer resolution. _(deferred — requires freezegun mocked clock or threading.Event helper; out of safe-pass scope)_
- [ ] Verify: `pytest tests/unit/services/llm/test_persistence.py` passes; LOC delta approximately -2 _(deferred — requires freezegun mocked clock or threading.Event helper; out of safe-pass scope)_
...
- [ ] S08-CAT7-002: replace the 4 `time.sleep(0.02)` calls (lines 135, 139, 325, 343) with **Event-based synchronization** in the test (test-side helper, not a production change). The `_wait_until` spin-loop using `time.sleep(0.01)` should use the same helper. _(deferred — requires freezegun mocked clock or threading.Event helper; out of safe-pass scope)_
- [ ] Verify: `pytest tests/unit/services/llm/test_race_description_llm_controller.py` passes; LOC delta approximately -5 _(deferred — requires freezegun mocked clock or threading.Event helper; out of safe-pass scope)_
...
- [ ] S08-CAT7-003: replace the `_BlockingProvider.complete` polling (lines 82-91) — `while time.monotonic() < end` with `time.sleep(0.005)` — with a **mocked clock** (`freezegun` or a manual `time.monotonic`/`time.sleep` patch). Current shape can take up to 5 seconds. If mocked-clock is infeasible, fall back to the M-005 skip pattern with a follow-up project tracker. _(deferred — requires freezegun mocked clock or threading.Event helper; out of safe-pass scope)_
- [ ] Verify: `pytest tests/unit/services/llm/test_race_description_llm_controller.py` passes; LOC delta approximately -3 _(deferred — requires freezegun mocked clock or threading.Event helper; out of safe-pass scope)_

...
- [x] Replace the `__new__` bypass + manual wiring (or `MagicMock(spec=Class)`) for OrdersWindow, BuildQueueScreen, TransferDialog, BuildQueueListWindow (~350 LOC) with `make_ui_widget(...)`; or refactor hotkey logic into a separately testable module. Coordinate with Task 3.26 in Phase 3. **PARTIAL RESOLVED IN PROJ-328 Phase A** — OrdersWindow migrated in Task A.3 (commit 00874c571), BuildQueueListWindow migrated in Task A.2 (commit 7859d652c), and the corresponding hotkey clusters in this file rewritten to bypass_init in Task A.6 (commit 2252a6ef3). BuildQueueScreen cluster unchanged (not a UIWindow subclass — bypass_init machinery doesn't apply). TransferDialog cluster left as-is — Phase C scope will rewrite it alongside the deep TransferDialog split. _(original deferral: all four target classes inherit from pygame_gui.elements.UIWindow (or StrategyModalWindow); the factory cannot patch through super().__init__() chains. PROJ-325 PoC + PROJ-328 Phase A unblocked the UIWindow path.)_
- [x] Verify: `pytest tests/unit/ui/screens/test_sub_window_hotkeys.py` passes; LOC delta approximately -175 — actual: 23 tests pass; OrdersWindow + BuildQueueListWindow clusters now use bypass_init helpers (~10 LOC each instead of inline mock wiring).

...
- [ ] Add `format_fleet_info` to the import from `strategy_detail_fmt` (line 25-27):
  ```python
  ...
  ```
- [ ] Remove `from game.strategy.data.fleet import OrderType` (line 21) - no longer needed
- [ ] Update module docstring (lines 1-6): remove "OrderType" from the cross-layer imports note
...
- [ ] Replace the inline fleet text generation block (lines 562-583) with:
  ```python
  text = format_fleet_info(obj)
  ```
- [ ] Keep ALL button-showing logic (lines 586-600) unchanged - it stays in strategy_ui.py because it depends on UI state (`self.scene`, `current_empire_id`)
- [ ] Verify `pytest tests/ --testmon` passes

**Notes:** The `elif is_fleet(obj):` block structure stays the same - only the text generation changes. Buttons, colonize validation, and visibility logic remain untouched.

### Task 3.3: Final Verification [Simple]
**Tests:** `pytest tests/ -n 12`

- [ ] Run full test suite: `pytest tests/ -n 12`
- [ ] Verify test count is >= 6519 (baseline) + new tests
- [ ] No regressions

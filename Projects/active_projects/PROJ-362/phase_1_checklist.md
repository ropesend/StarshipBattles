...
- [x] Module docstring referencing PROJ-362 Phase 1 and review finding #2.
- [x] Reuse existing fixtures from `test_system_effects_collector.py`: `_make_facility`, `_make_planet`, `_make_system`, `_stabilizer_design`, `_harvest_booster_design`.

**Notes:** Used hand-rolled `_FakeSource` dataclass directly instead of MagicMock-based fixtures because the tests bypass the iterator and call `_aggregate` directly. Cleaner control over `affects_hex` exception path. Reused fixture patterns conceptually but expressed them as IAbilitySource adapters. 10 tests added (more than 9 baseline; the +1 is the `affects_hex_false_return_skips_source` companion case).

### Task 1.2: get_abilities() exception path [Simple]
**File:** Same as 1.1
**Tests:** Per-test pytest

- [x] `test_get_abilities_exception_skips_source_and_logs`:
...
- [x] `test_affects_hex_exception_skips_source_and_logs`:
...
- [x] `test_deactivating_ability_shows_progress_remaining`:
...
- [x] `test_any_active_overrides_activating_and_deactivating`:
...
- [x] `test_activating_when_no_active_uses_first_activating_status`:
...
- [x] `test_deactivating_when_no_active_or_activating`:
...
- [x] `test_owned_source_skipped_when_empire_id_mismatches_query`:
  - Source with `owner_id=2`. Query with `empire_id=1`.
  - Assert source contributes nothing to the result.
- [x] `test_ownerless_source_contributes_to_any_empire_query`:
...
- [x] `test_improvement_rate_field_used_when_rate_and_multiplier_missing`:
...
- [x] All tests pass against the current pre-refactor code.
- [x] If a test fails, do NOT change production code in this phase — instead, the test was wrong (current behavior is the spec). Adjust the test to reflect actual behavior, document the deviation in the test docstring as "characterization of existing behavior", and update decisions.md.
- [x] Verify count: 9 tests added, all green.
...
- [x] 9 new tests, all green against current code

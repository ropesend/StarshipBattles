...
- [x] Update star rendering formula (line 528):
  ```python
...
- [x] Update comments on lines 689, 695, 705
...
- [x] Update click detection (line 339): remove `* 0.5` factor, use `radius_hexes`
- [x] Update second rendering location (line 517): same change
- [x] Update UI display label (line 391): `"Diameter: {star.diameter_hexes:.1f} hexes"` → `"Radius: {star.radius_hexes} hexes"`
...
- [x] `strategy_detail_fmt.py` line 196: `"<b>Diam:</b> {star.diameter_hexes:.1f} Hex"` → `"<b>Radius:</b> {star.radius_hexes} Hex"`
- [x] `strategy_detail_formatter.py` line 271: same change
...
- [x] Update star rendering formula (~line 222) to use `radius_hexes`
...
- [x] No `diameter_hexes` references remain in `game/ui/` or `scripts/`

...
- [ ] In `update_design()` (line 99), after creating `DesignStatsPanel` at line 126-132, add `self._stats_panel.update_stats(ship)` to populate stat values
- [ ] This is the same fix as BUG-04 in `game/ui/screens/builder/right_panel.py:57-59`
- [ ] Verify: Select a design in Available Designs - right panel should show actual stat values, not "--"
...
- [ ] Create a helper method `_get_design_cost(self, design_id: str) -> Dict[str, int]` that:
...
- [ ] Update `_calculate_build_turns()` (line 170) to use `_get_design_cost(design_id)` instead of `design.resource_cost`
- [ ] Update `_build_cost_tracking()` (line 191) to use `_get_design_cost(design_id)` instead of `design.resource_cost`
- [ ] Verify: Add a design that costs 4500 metals to a shipyard (3000 build rate) - should show 2 turns, not 1
- [ ] Verify: Queue items should now have populated `total_cost` dict
...
- [ ] Stats display correctly when design selected (not "--")
- [ ] Production time shows correct turns based on cost
- [ ] Queue items have total_cost populated

...
- [ ] Keep title creation at top (lines 339-345), unchanged
- [ ] Define layout constants after title:
  ```python
  ...
  ```
- [ ] Update `RaceThemeGallery` creation (lines 349-357) to pass new position and `height`:
  ```python
  ...
  ```
- [ ] Remove old `theme_gallery_height = 200` estimation (line 361)
- [ ] Remove old `y += theme_gallery_height + 10` (line 362)
- [ ] Create ship preview scroll container to the right:
  ```python
  ...
  ```
- [ ] Remove `self.ship_preview_container = self.ship_preview_scroll` alias (line 375) - unused
- [ ] Verify: Visual test - theme list on left, preview area on right
...
- [ ] Visual test: theme list appears on left, ship previews on right

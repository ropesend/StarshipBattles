...
- [x] Module docstring referencing PROJ-365 Phase 1.
- [x] Build a `TurnEngine` with mock engines (reuse `mock_engines.py` fixtures).
- [x] Wrap `_time_phase` to capture `(phase_name, callable_repr, args_signature)` tuples in order.
- [x] Run `_process_tick(tick=1, ...)` and `_process_tick(tick=5, ...)` (covering tick==1-only branches).
- [x] Build the expected golden list — extract from `turn_engine.py:703-782` exactly. Format:
  ```python
  GOLDEN_PHASE_ORDER = [
      'harvesting',
      'resources',
  ...
  ```
- [x] Assert captured order == GOLDEN_PHASE_ORDER.
...
- [x] `test_log_empire_state_called_only_on_tick_1`:
...
- [x] These are characterization for the `tick_gating='only_tick_1'` descriptor field that Phase 2 will introduce.
...
- [x] `test_combat_phase_receives_moved_fleet_ids_from_movement_diff`:
...
- [x] This is redundant with `test_turn_engine_phase_320_movement_diff.py` but pins the invariant inside the same module that documents the new descriptor design.

**Notes:** _(filled during implementation)_

### Task 1.4: Verify all tests green against current code [Simple]
- [x] Run the file. All tests pass against the current imperative `_process_tick`. This is characterization — production code is unchanged.
...
- [x] tick==1 gating characterization green
- [x] PROJ-320 invariant assertion green

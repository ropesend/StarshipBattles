...
- [ ] Line 20: Rename `class BattleResult:` to `class BattleServiceResult:`
- [ ] Update all references within battle_service.py (return types, method bodies)
- [ ] Grep entire codebase for `BattleResult` (NOT `BattleResults`) to find all import sites:
  - Search for `from game.simulation.services.battle_service import.*BattleResult`
  - Search for `BattleResult` in type hints
- [ ] Update each import site to use `BattleServiceResult`
- [ ] Update each usage site to use `BattleServiceResult`
- [ ] Verify: `pytest tests/ -n 12` passes

**Notes:** This is Complex because it touches every caller of BattleService. Do thorough grep before and after.
...
Current state:
- `BattleEngine.get_winner()` returns `int` (0, 1, or -1) - line 655
- `BattleService.get_winner()` returns `Optional[int]` (None when no engine) - line 262
...
- [ ] Line 655: Verify `get_winner(self) -> int:` already has correct annotation (it does)
- [ ] Update get_winner() docstring to explicitly state: "Never returns None. Returns -1 for draw."
- [ ] In BattleService.get_winner(), update docstring to explain: "Returns None only when no battle engine is active. Otherwise delegates to BattleEngine.get_winner() which returns 0, 1, or -1."
- [ ] Verify: `pytest tests/ -n 12 -k "winner"` passes

**Notes:** The types are actually correct for their respective layers. The issue is documentation, not code.
...
- [ ] Review `get_resource(self, name: str) -> Optional[ResourceState]` at line 114 - this is a single-value lookup, Optional is correct here (returns None for missing resource)
- [ ] Review `get_all_resources(self) -> List[ResourceState]` at line 200 - already returns list, correct
- [ ] Review `get_resource_names(self) -> List[str]` at line 196 - already returns list, correct
- [ ] Document the convention as a class-level docstring addition:
  ```
  Return Convention:
...
**Notes:** After review, ResourceRegistry already follows the correct convention. The task is to document it explicitly.
...
- [ ] Line 612: Change `raise ValueError("apply_results_to_fleets only valid in STRATEGY mode")` to `raise StateException("apply_results_to_fleets only valid in STRATEGY mode", code=ErrorCode.INVALID_STATE.value)`
- [ ] Line 622: Change `raise ValueError("No source fleets configured")` to `raise StateException("No source fleets configured", code=ErrorCode.INVALID_STATE.value)`
- [ ] Add imports: `from game.core.exceptions import StateException` and `from game.core.error_codes import ErrorCode`
- [ ] Grep for tests that catch `ValueError` from these methods and update to catch `StateException`
- [ ] Verify: `pytest tests/ -n 12 -k "battle_controller"` passes

**Notes:** Only changing battle_controller.py in this phase. battle_state_manager.py and battle_mode_handler.py can be addressed in a future phase if needed.

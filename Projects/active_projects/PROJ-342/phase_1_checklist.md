...
- [ ] Create test file with imports: `pygame`, `types.SimpleNamespace`, `pytest`, `Mock` from `unittest.mock`, and `from game.ui.screens.test_lab.screen import TestLabScreen`
- [ ] Define test function that:
...
- [ ] Run the test against unmodified production code; **MUST FAIL** with `AttributeError: 'SimpleNamespace' object has no attribute 'screen'`
- [ ] Document the expected failure mode in a comment so future readers understand it's a regression test
...
- [ ] Run against unmodified production code; **MUST FAIL** with `TypeError` (current `__init__` signature is `(game, scene_callback)`)
- [ ] Use the same display-surface mocking pattern as Task 1.1 to keep `tests/unit/test_lab/conftest.py` unchanged (it does not auto-create a display per `tests/unit/ui/conftest.py:41-44` comparison)
...
- [ ] Run the new test file against current (pre-Phase-2) code
- [ ] Confirm exactly the failure modes documented above
- [ ] If a test fails for an unexpected reason, fix the test before proceeding — do not weaken assertions to make it pass
...
- [ ] Both new tests exist and are RED on unmodified production code
- [ ] Test file imports are clean (no unused imports)
- [ ] Test names clearly describe the regression they pin

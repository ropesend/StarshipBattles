...
**Last Action:** Phase 2 (F-D-011 partial — battle_controller spec-in extraction) complete on `group-c`. `BattleController.start_from_spec`'s ~103-LOC body extracted to the new `game/simulation/battle_controller_spec.py` (155 LOC) as `build_controller_from_spec(controller, spec, *, ...)` (Option A — free function taking controller as first arg); the method is now a 1-line facade. `battle_controller.py` dropped 831 → 728 LOC (plan claimed stale 682). No imports removed (BattleConfig/RetreatManager/logger all still used elsewhere; grep-verified). Body byte-faithful (`self.` → `controller.`); deferred imports preserved. Targeted gate `pytest tests/integration/replay/ tests/integration/save_load/ tests/unit/simulation/battle_controller/` = 404 passed (includes test_headless_visual_equivalence + test_start_from_spec). Task 2.4 manual UI smoke SUPERSEDED by the automated replay gate per the Group C prompt. Phase 2 doc entry staged to `_doc_consolidation/PROJ-460_pending.md`. Phase 2 sharded gate running. Also note: Phase 1 surfaced + fixed a pre-existing sharding flake in `test_settings_window.py` (PROJ-458 test I authored; `pygame.event.custom_type()` collision with `UI_BUTTON_PRESSED` under heavy sharding) via a separate hygiene commit.
**Next Action:** Confirm Phase 2 sharded green; commit + push; then Phase 3 (split `replay_serialization.py` into `replay_capture_serde.py` + `replay_outcome_serde.py` + `replay_serde_helpers.py`). Phase 3 Task 3.0: re-measure replay_serialization.py LOC + re-derive the spec/outcome boundary first.

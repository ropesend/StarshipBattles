...
**File:** `game/strategy/data/movement.py`
**Tests:** `pytest tests/ -k movement`

- [ ] Replace `galaxy._global_hex_*` / `galaxy._planet_to_system` / `galaxy._zone_to_system` reads with the canonical `galaxy.state.<public_name>` accessors (LEG-03-022)
- [ ] Verify: file no longer references underscore-prefixed forwarders
...
- [ ] Replace forwarder reads with public `GalaxyState` accessors (LEG-03-022)
- [ ] Verify: file no longer references underscore-prefixed forwarders
...
- [ ] Delete the 5 property forwarders at lines 97-131 (`_global_hex_<five>`, `_planet_to_system`, `_zone_to_system`) plus the "backwards-compat under-prefixed forwarders" docstring (LEG-03-022)
- [ ] Verify: `grep -rn -E "galaxy\._(global_hex|planet_to_system|zone_to_system)" .` returns zero hits

...
|-------|--------|-----------|
| 1. Fix UI Encapsulation & Extract Bridge Helper | Not Started | [phase_1_checklist.md](phase_1_checklist.md) |
...
**Last Action:** Project plan fully specified
**Next Action:** Begin Phase 1 — fix UI encapsulation violations and extract bridge helper
**Blockers:** None
**Context for Next Agent:** This project cleans up remaining findings from the resource/state duplication audit that PROJ-91 left behind. PROJ-91 removed type-specific methods from ShipInstance but left them in extracted managers (ShipResourceManager, FleetResourceAggregator, Fleet). Additionally, 2 UI files access private `._resources` internals, and `IPostBattleShip.resources` is typed as `Any`. All changes are low-risk dead code deletion and protocol cleanup. Baseline: **7616 tests passing**.

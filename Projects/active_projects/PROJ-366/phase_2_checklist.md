# Phase 2: Coordinator construction + start + run-loop shutdown
...
**Status:** Not Started
**Objective:** Construct and start the `ReplayVerificationCoordinator` in `bootstrap()`; expose both store and coordinator on `BootstrapResult`; wire `shutdown_all_coordinators(timeout=5.0)` into `RunLoop.run()` immediately after `shutdown_all_calls(timeout=5.0)` and before `pygame.quit()`. Phase 1 must be complete before this phase starts.

See `plan.md` Phase 2 for context.
...
- [ ] Update the `BootstrapResult` dataclass (around lines 73-90) to add (alongside the existing fields):
  ...
  ```
  Use forward-string types so module-level imports stay minimal. Add a TYPE_CHECKING block for the typing-only imports if needed.
- [ ] **Verify:** existing bootstrap tests still pass (constructor signature widened, not narrowed).

**Notes:**

### Task 2.3: Construct + start coordinator in `bootstrap()` [Medium]
**File:** `game/app_bootstrap.py`
**Tests:** Task 2.1 should now pass.

- [ ] Add lazy imports inside `bootstrap()` body (after Phase 1's imports):
  - `from game.strategy.services.replay_store import load_replay_settings`
  - `from game.strategy.services.replay_verification_coordinator import ReplayVerificationCoordinator`
  - `from game.ai.ai_factory import AIControllerFactory`
  - `from game.combat_lab.design_loader import load_combat_lab_design`
- [ ] AFTER the Phase 1 `replay.construct_store` block, add:
  ```python
  with _timed_phase("replay.start_coordinator", ctx.profiler):
      replay_verification_coordinator = ReplayVerificationCoordinator(
          replay_store=replay_store,
          ai_factory=AIControllerFactory(),
          registry_provider=provider,  # the local from earlier in bootstrap()
          settings=load_replay_settings(),
          fallback_ship_builder=load_combat_lab_design,
      )
...
**File:** Add a new test under `tests/unit/test_run_loop_shutdown_ordering.py` OR extend an existing run-loop test if one matches.
**Tests:** Same file

- [ ] Search `tests/unit/` for existing run-loop tests (e.g., `test_run_loop.py`).
- [ ] Add a test that monkeypatches `shutdown_all_calls` AND `shutdown_all_coordinators` (importing both into the run-loop module) to record call order; constructs a `RunLoop` with `running=False` so `run()` exits immediately after the loop; runs it; asserts both were called and `shutdown_all_calls` happened BEFORE `shutdown_all_coordinators` BEFORE `pygame.quit()`.
...
- [ ] In `plan.md` Current State:
  ...
  - **Active Phase:** 3
  - **Last Action:** Phase 2 complete; coordinator constructed + started in bootstrap, exposed on BootstrapResult; shutdown wired in run loop.

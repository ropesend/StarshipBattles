...
- [ ] Phase 2 outcomes — list of migrated and deferred sites
- [ ] PROJ-377 [decisions.md](decisions.md) — confirm the deferred-site pinning rows exist
- [ ] `game/strategy/data/pathfinding.py` — current module shape (top-level FunctionDefs)
- [ ] `tests/unit/strategy/data/test_galaxy_planet_star_loc_ceilings.py` — reference for AST-walk test style (PROJ-372 Phase 5 added this; mirror its conventions)
...
- [ ] Walk through PROJ-377 plan.md Phase 2 scope table. Functions imported by Class B (deferred) + Class C (stays) + the helpers `_pathfinder_for` / `_intercept_for` form the surviving set. Likely:
...
- [ ] Functions Phase 3 is allowed to drop (no production caller AND no test patch):
...
- [ ] Document the final surviving list in PROJ-377 `decisions.md` as a pinned row.
...
- [ ] **Skip if Task 3.1 found all functions still have callers or test patches.**
- [ ] For each function the analysis says is unused: delete its `def` block.
- [ ] **Verify:** sharded green.
- [ ] **Verify:** focused pathfinding tests still green: `pytest tests/unit/strategy/pathfinding/ -v`.
...
- [ ] Module docstring: explain the test's role (AST walk over `pathfinding.py`; assert top-level `FunctionDef` names ≡ pinned set; prevents silent regrowth of the shim). Direct future agents to update the pinned list **deliberately**, not silently, with a decisions.md row when they add or remove a function.
- [ ] Test function `test_pathfinding_shim_function_set_pinned()`:
...
- [ ] Add a small acceptance test `test_helpers_present()` asserting `_pathfinder_for` and `_intercept_for` are in the set (since they're the test-patch transparency infrastructure).
- [ ] Run focused: `pytest tests/unit/strategy/data/test_pathfinding_shim_scope.py -v`. Expect green.
- [ ] **Verify:** test passes.
...
- [ ] Replace the current docstring (lines 1-18) with a new docstring that:
...
- [ ] **Verify:** the file LOC count stays roughly the same (no drift in code, just docstring rewrite).
...
- [ ] Append a new row dated 2026-XX-XX (Phase 3 commit date):
...
- [ ] **Verify:** PROJ-372 plan.md and decisions.md are mutually consistent re: Phase 5 leftovers status (both should now reference PROJ-377 as the resolution).
...
- [ ] If the doc references the shim's "deprecated" status or a "PROJ-376 follow-up", update to point at PROJ-377 closeout.
- [ ] If no such reference exists, skip (no update needed).
...
- [ ] **Verify:** pass count = baseline + 3 (2 golden-fixture tests from Phase 1 + 1 AST shim-scope test from Phase 3); zero new failures.
...
- [ ] `Grep` for `from game\.strategy\.data\.pathfinding import|from game\.strategy\.data import pathfinding` under `game/`. Confirm count matches the deferred-site list pinned in decisions.md.
- [ ] `Grep` for `galaxy\._intercept\b` under `game/`. Confirm zero hits (PROJ-372 review MIN-001 remediation preserved).
- [ ] `Grep` for `from game\.strategy\.data\.pathfinding import` under `tests/`. Pin the count in decisions.md as the "test-patch surface dependency count" — for documentation only, no migration this phase.
...
- [ ] `git add tests/unit/strategy/data/test_pathfinding_shim_scope.py game/strategy/data/pathfinding.py Projects/active_projects/PROJ-372/decisions.md Projects/active_projects/PROJ-377/decisions.md` (and `docs/systems/strategy_layer.md` if Task 3.6 modified it).
- [ ] Commit message: `PROJ-377 phase 3: AST shim-scope guard + docstring update + PROJ-372 cross-link (closes MIN-002)`
...
- [ ] AST shim-scope guard committed and green
- [ ] Shim docstring reflects PROJ-377 closeout
- [ ] PROJ-372 cross-link backfilled
- [ ] Sharded suite green; pass count = baseline + 3
- [ ] Update status at top of this file to `Complete`
- [ ] Update plan.md phase table row to `Complete`
- [ ] Update plan.md Current State to "Awaiting user verification"
- [ ] Final audit log entry in plan.md

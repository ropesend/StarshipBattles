...
- [x] Read the three method shims at transfer_dialog.py:279-286. Confirm each delegates trivially:
...
- [x] Find characterization-test sites: `rg -n "dialog\._extract_dropdown_value|dialog\._format_pending|dialog\._discover_pod_designs" tests` (expect 2-3 hits per finding).
- [x] **RED**: Migrate each test site to call the underlying canonical surface directly. Example: `dialog._format_pending(amount)` → `dialog.view_model.format_pending(amount)`. Run; tests pass with same semantics.
- [x] **GREEN**: Delete the three method shims + the comment block at lines 274-277 (~13 lines total).
- [x] **Verify**: `rg -n "_extract_dropdown_value|_format_pending|_discover_pod_designs" game tests` returns hits only on the canonical-surface definitions (TransferGridRenderer / TransferViewModel / TransferController), not on the dialog.
- [x] Check `_on_source_changed` at line 292 — it uses `self._extract_dropdown_value(label)`. Migrate that internal call to `TransferGridRenderer.extract_dropdown_value(label)`.
...
- [x] Read sentinel re-exports at transfer_dialog.py:58-62 (`MAX_LOAD`, `MAX_DROP`) and the 18 layout-constant re-exports at lines 64-86 (`ROW_HEIGHT` through `TARGET_AMT_W`).
- [x] Confirm canonical homes by reading:
...
- [x] Audit importers: `rg -n "TransferDialog\.(MAX_LOAD|MAX_DROP|ROW_HEIGHT|NAME_X|NAME_W|SOURCE_AMT_X|MAX_LOAD_X|LOAD_ARROWS_X|PENDING_X|ZERO_BTN_X|DROP_ARROWS_X|MAX_DROP_X|TARGET_AMT_X)" game tests`. For each, migrate to the canonical-home class (`TransferViewModel.MAX_LOAD`, `TransferGridRenderer.ROW_HEIGHT`, etc.).
- [x] **GREEN**: Delete the sentinel + layout-constant blocks at lines 58-86 (~29 lines including comments and gaps).
- [x] Run targeted tests; sharded suite green for the touched files.
...
- [x] Locate the 6 dialog-level property shims in transfer_dialog.py. Use `rg -n "Back-compat property shims" game/ui/screens/transfer_dialog.py` to find the comment header. The shims cover:
...
- [x] **RED**: For each test file, do the mechanical sweep (sed-style, but verify by hand each block):
...
- [x] **GREEN**: Delete the 6 dialog-level property shim block + its comment header in transfer_dialog.py.
- [x] **Verify (PowerShell-safe)**: `rg -n "dialog\.(_current_source|_current_target|_row_data|_filter_empty|available_sources|available_targets|pending_transfers)" tests` returns 0 hits.
- [x] Run all 3 transfer_dialog test files; sharded suite green.
...
- [x] F-C-003, F-C-011, F-C-029, DI-2026-05-18-002 flipped to `Status: resolved` in `findings/PROJ-456_findings.md`.
- [x] DI-2026-05-18-002 marked resolved in `AgentCoordination/discovered_issues/log.jsonl` (or noted as still-open with a documented reason).
- [x] `transfer_dialog.py` under 500 LOC.
- [x] Run `python Tools/test_sharded/test_sharded.py` — full sharded suite green.
- [x] Run `python Projects/scripts/validate_phase.py PROJ-456 4` — PASSED.
- [x] Update status at top of this file to `Complete`.
- [x] Update plan.md phase table row to `Complete`.
- [x] Update plan.md Current State to point to Phase 5.
- [x] Commit message: `PROJ-456 Phase 4: retire transfer_dialog shim cluster (F-C-003, F-C-011, F-C-029); LOC 448 -> <FINAL>; DI-2026-05-18-002 closed (shim-residue framing, not LOC overflow which was already stale)`.

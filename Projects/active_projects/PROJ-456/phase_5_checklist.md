...
- [ ] Read the property block at strategy_renderer.py:107-130 — 6 read-only `@property` shims delegating to `self._background.<attr>` and `self._hex_outlines.<attr>`.
- [ ] **Verify** (PowerShell-safe; already done at 2026-05-19, re-check at phase start): `rg -n "_bg_image|_bg_scaled|_bg_brightness|_hex_outline_cache" tests` returns 0 hits. No test reads of these names.
- [ ] **GREEN**: Delete the property block at lines 107-130 (~24 lines including the `# -- Back-compat shims for tests that read these as instance attributes --` header comment).
- [ ] Audit production-side reads (PowerShell-safe): `rg -n "renderer\._bg_image|renderer\._bg_scaled|renderer\._bg_brightness|renderer\._hex_outline_cache" game` — should return 0 hits. If any do exist, migrate them to `renderer._background.<attr>` / `renderer._hex_outlines.<attr>`.
...
- [ ] Read property block at new_game_setup_screen.py:272-321 — 6 read/write property shims:
...
- [ ] **RED**: For each of the 3 test/fixture files, replace `screen.<name>` with `screen._view_model.<name>` (read & write). 34 refs total + 1 self-reference in the screen file (inside the property block being deleted).
- [ ] **RED — Fixture migration**: `tests/fixtures/new_game_setup_ui_builder.py` likely sets these as part of bypass-init builder behavior. The 4 refs should migrate to `screen._view_model.<name> = ...`. Verify the builder still constructs `_view_model` first; if not, the fixture may need adjustment.
- [ ] Run each test file individually after migration.
- [ ] **GREEN**: Delete property block at new_game_setup_screen.py:272-321 (~50 lines including the comment header at 272-273).
- [ ] **Verify (PowerShell-safe)**: `rg -n "screen\.(player_count|galaxy_type|system_count|player_races|active_race_modal|race_modal_player_index)" game tests` returns 0 hits in non-VM context.
- [ ] Run targeted tests; sharded suite green.
...
- [ ] Read property block at battle_setup/screen.py:93-205 — 11 property shims:
...
- [ ] **RED — Production panel migration**:
...
- [ ] Run targeted tests after each file migration.
- [ ] **GREEN**: Delete property block at battle_setup/screen.py:93-205 (~113 lines including comment headers at 93-95 and 145-146).
- [ ] **Verify (PowerShell-safe)**: For each shim name, `rg -n "screen\.<name>" game/ui/screens/battle_setup tests` returns 0 hits in non-VM/non-controller context.
- [ ] **Re-measure LOC (PowerShell-safe)**: `(Get-Content game/ui/screens/battle_setup/screen.py | Measure-Object -Line).Lines`. The file was 189 LOC at HEAD before this phase (per the 2026-05-19 re-measurement); after deleting ~113 lines of property block, expect ~75-85 LOC. Document in `decisions.md`.
- [ ] Run targeted tests; sharded suite green.
...
- [ ] F-C-004, F-C-008, F-C-009 flipped to `Status: resolved` in `findings/PROJ-456_findings.md`.
- [ ] Run `python Tools/test_sharded/test_sharded.py` — full sharded suite green.
- [ ] Run `python Projects/scripts/validate_phase.py PROJ-456 5` — PASSED.
- [ ] Update status at top of this file to `Complete`.
- [ ] Update plan.md phase table row to `Complete` (and mark PROJ-456 as Project Complete in `plan.md` Current State).
- [ ] Update `Projects/projects_index.md` PROJ-456 row to `Complete`.
- [ ] Commit message: `PROJ-456 Phase 5: retire big-three UI shim clusters (F-C-004 strategy_renderer, F-C-008 new_game_setup, F-C-009 battle_setup); PROJ-456 complete`.
...
- [ ] Final sharded-suite green run on file with the expected new baseline.
- [ ] `transfer_dialog.py` LOC < 500.
- [ ] All 14 owned-finding entries in `findings/PROJ-456_findings.md` carry `Status: resolved` with closure dates.
- [ ] Codex end-of-project consult invoked + remediation landed if it surfaced any MUST-FIX-NOW items.
- [ ] User applies the `verified` label and closes the project per the CLAUDE.md authority constraint.

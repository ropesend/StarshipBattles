...
**Active Phase:** Phase 1 (in progress — sub-phases 1a + 1b + 1c + 1d + 1e complete)
**Last Action:** Sub-phase 1e complete. The UI/DTO/AI read surfaces no longer touch the legacy `ship.carried_items` substrate or call `CarriedVehicle.from_any()`. Production migrations: `game/ai/carrier_controller.py` (`_pop_cvs`, `_pop_cvs_within_budget`) walks `carrier.bay_inventory.bay` and writes back via `carrier.set_bay_inventory(BayInventory(bay=remaining, pods=inventory.pods))`; `game/strategy/facade/dto/fleet_dto.py` (`_aggregate_carried_items`, `_aggregate_carried_vehicles_by_type`) aggregates over `ship.bay_inventory.bay` (CarriedVehicle entries — name pulled from `design_data['name']`) and `ship.bay_inventory.pods` (DropPod entries — name pulled from `payload['name']` because the legacy projection tucked dict-shaped pod metadata into payload); `game/ui/screens/fleet_menu_items.py::_fleet_has_carried_vehicle` walks `ship.bay_inventory.bay`; `game/ui/screens/strategy_detail_fmt.py::_format_cargo_summary` walks both `bay_inventory.bay` and `bay_inventory.pods`. No new BayInventory API extensions were needed. AST guard test landed: `tests/unit/strategy/test_ui_dto_ai_readers_no_legacy_substrate.py` (8 tests: two per file × four files) pins all four sources' typed-substrate-only invariant. Test stubs that previously set `ship.carried_items = [...]` directly were updated to wire the typed view (`tests/unit/ai/test_carrier_controller.py`, `tests/integration/test_fms_c_carrier_ai_launch.py`, `tests/unit/ui/screens/test_fleet_menu_items.py`, `tests/unit/ui/screens/test_strategy_detail_fmt.py`, `tests/unit/strategy/facade/test_fleet_dto.py`). Focused tests: 173 pass for the four migrated files + AST guard; broader sanity (`tests/unit/strategy/facade/`, `tests/unit/ui/screens/`, `tests/unit/ai/`) 4141/4141 green. Out-of-scope files (serializer, `CarriedVehicle.from_any()`, `ShipInstance.carried_items` field) untouched.
**Next Action:** Sub-phase 1f — delete `ShipInstance.carried_items` field + `CarriedVehicle.from_any()`; migrate the serializer; run zero-hit grep gates.
**Blockers:** None. Phase 1 still requires sub-phase 1f and the final delete-substrate commit per decisions.md.

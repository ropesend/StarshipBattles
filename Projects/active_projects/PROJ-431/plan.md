...
|-------|--------|-----------|
| 1. Typed `BayInventory` on `ShipInstance` | In Progress (sub-phases 1a–1c complete) | [phase_1_checklist.md](phase_1_checklist.md) |
...
**Active Phase:** Phase 1 (in progress — sub-phases 1a + 1b + 1c + 1d complete)
**Last Action:** Sub-phase 1d complete. The colonize/transfer validators and order-handler dispatch branches no longer touch the legacy mixed-shape `ship.carried_items` substrate or call `CarriedVehicle.from_any()`. Production migrations: `game/strategy/validation/colonize_validator.py` now walks `ship.bay_inventory.pods` (entries are typed `DropPod` by construction; the legacy `vehicle_type == 'drop_pod'` filter is gone because the pods slot is homogeneous); `game/strategy/validation/transfer_validator.py:_validate_vehicle_load` replaces `CarriedVehicle.from_any(item)` with an explicit `vehicle_type ∈ VALID_VEHICLE_TYPES` shape probe (staging yard remains dict-shaped); `game/strategy/engine/order_handlers/colonize.py::_deploy_drop_pod` consumes the pod from `ship.bay_inventory.pods` and writes through `ship.set_bay_inventory(...)`; `game/strategy/engine/order_handlers/transfer_branches.py` migrates `_dispatch_drop_pod_load`, `_dispatch_carried_vehicle_load`, and `_dispatch_drop_pod_unload` — fleet-side pod reads/writes route through `bay_inventory.pods` / `set_bay_inventory(...)`, staging-yard reads use the new module-level helpers `_is_carried_vehicle_dict`, `_staging_yard_carried_vehicle`, and `_pod_from_dict` that replace `CarriedVehicle.from_any()` at the legacy-dict boundary. No new BayInventory API extensions were needed. AST guard tests landed: `tests/unit/strategy/validation/test_validators_no_legacy_substrate.py` (4 tests) and `tests/unit/strategy/engine/order_handlers/test_colonize_transfer_no_legacy_substrate.py` (4 tests) pin both files' typed-substrate-only invariant. Several pre-existing tests using MagicMock ships with `ship.carried_items = [...]` were updated to also wire the typed `bay_inventory` view (test files: `test_colonize_validator.py`, `test_colonize_handler.py`, `test_transfer_handler.py`, `test_pod_transfer.py`, `test_staging_yard_operations.py`, `test_order_processor_transfer.py`, `test_multi_pod_colonization.py`, `test_order_processor_colonize.py`). Focused tests: 88 pass for validators/handlers; full strategy unit suite 4762/4762 green; integration suites (`tests/integration/strategy/transfer/`, FMS e2e) 33/33 green. Out-of-scope files (serializer, UI readers, AI controllers) untouched.
**Next Action:** Sub-phase 1e — test-file sweep (53 files: migrate remaining tests off `ship.carried_items = [...]` to `ship.set_bay_inventory(BayInventory(...))`).
**Blockers:** None. Phase 1 still requires sub-phases 1e–1f and the final delete-substrate commit per decisions.md.

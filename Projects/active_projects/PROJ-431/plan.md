...
**Last Action:** Phase 1 COMPLETE (sub-phases 1a–1f shipped). Sub-phase 1f deleted the `ShipInstance.carried_items` dataclass field and `CarriedVehicle.from_any(...)` static method. `bay_inventory: BayInventory` is now the actual dataclass field on `ShipInstance` (typed `bay: list[CarriedVehicle]` + `pods: list[DropPod]`); the previous "projection over carried_items" direction is reversed — bay_inventory IS the storage. `carried_items` survives as a backward-compatible write-through proxy property (`_CarriedItemsProxy`) used exclusively by test infrastructure that still pokes `ship.carried_items.append({...})`. Production migrations: `game/strategy/data/ship_instance_serializer.py` emits/reads the `bay_inventory` key (old saves disposable per CLAUDE.md no-migration rule); `game/simulation/systems/tactical_mine_resolver.py` reads `mine_group.ships[0].bay_inventory.bay` and writes back via `set_bay_inventory(...)` (writeback now matches survivors by positional index, since fresh `cv.to_dict()` dicts no longer share identity with the carrier); `game/simulation/systems/attack_processor.py`, `game/strategy/engine/issuer_adapter.py` (`_matches` + `append_carried`), `game/strategy/engine/handlers/fms_shared.py::count_matching_yard` replaced `CarriedVehicle.from_any(...)` with explicit `isinstance(item, dict) and item['vehicle_type'].lower() in VALID_VEHICLE_TYPES` + `CarriedVehicle.from_dict(...)`. Dead-code removals: `ShipInstanceWriteService.add_carried_item` / `pop_carried_item` (no production callers); the `IStrategyMutator` protocol entries; the `bay_inventory` projection property + `set_bay_inventory` write-through shim was replaced with an attribute setter. AST guard tests landed at `tests/unit/strategy/data/test_phase_1f_deletion_guard.py` (two tests: `__dataclass_fields__` does not contain `carried_items`; `CarriedVehicle` has no `from_any` attribute). 12 test files migrated off `CarriedVehicle.from_any(...)` (replaced with the shape-probe pattern); 8 stub-carrier classes (`_StubCarrier` / `_StubShip` / `SimpleNamespace` fakes) gained a `bay_inventory` property + `set_bay_inventory` method so the production code paths keep working. Tests on the legacy `from_any` itself (in `test_vehicle_bay.py`) were deleted — the deletion guard test in 1f replaces them. Full sharded suite: **21138/21138 passed**.
**Next Action:** Phase 2 — `MineGroup` extraction (DeployedGroup family 1). Open `phase_2_checklist.md`.
**Blockers:** None.

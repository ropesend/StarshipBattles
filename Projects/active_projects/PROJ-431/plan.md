...
**Active Phase:** Phase 1 (in progress — sub-phases 1a + 1b + 1c complete)
**Last Action:** Sub-phase 1c complete. The five FMS order handlers (`lay_mines.py`, `launch_fighters.py`, `launch_satellites.py`, `recover_fighters.py`, `recover_satellites.py`) and the three FMS command handlers under `game/strategy/engine/handlers/` (`lay_mines.py`, `launch_fighters.py`, `launch_satellites.py`) are now off `carried_items` / `CarriedVehicle.from_any()` on the fleet path. Recover handlers already routed bay writes through `issuer.append_recovered(...)` — no changes were needed there. Highlights: `fms_shared.count_matching` was split into `count_matching_bay` (typed, consumes `BayInventory.bay`) and `count_matching_yard` (legacy dict shape, still uses `from_any` since the planet staging yard is out of scope until 1d); `FleetShipIssuerAdapter.pop_carried` now returns typed `list[CarriedVehicle]` instead of dict-roundtripped items so the launch handlers consume the typed payload directly; the lay-mines order handler deposits into the synthetic mine-carrier via `ship.bay_inventory` / `ship.set_bay_inventory(...)` instead of the old `mine_group.ships[0].carried_items.append(...)`. The fleet adapter's `pop_carried` typed-return is a contract change consumed by the three migrated order handlers (which accept both dict and typed shapes so the planet adapter, still on the dict substrate, remains compatible). Focused tests: 53/53 in `tests/unit/strategy/engine/order_handlers/` + `test_issuer_adapter.py` green; all 4754 strategy unit tests green. Out-of-scope files (`validators`, `colonize`, `transfer`, `serializer`, UI readers, AI controllers) untouched.
**Next Action:** Sub-phase 1d — migrate `colonize.py` + `transfer_branches.py` + `ship_instance_serializer.py` (planet staging yard / pod-drop / serializer paths).
**Blockers:** None. Phase 1 still requires sub-phases 1d–1f and the final delete-substrate commit per decisions.md.

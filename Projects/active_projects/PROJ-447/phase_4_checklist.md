...
- [x] Read the existing assertion message: "Add an entry in game/strategy/engine/commands/specs.py."
- [x] Confirm `commands/specs.py` is truly gone: `ls game/strategy/engine/commands/specs.py` should fail. The guard test `tests/unit/strategy/engine/test_no_specs_tuple_literal.py:3-7` confirms re-introduction is forbidden.
- [x] Read the canonical extension path at `docs/systems/orders_system.md:130-137` and `:418-422` — uses `@command_spec(...)` decorator + per-module `register(registry)` calls
- [x] **GREEN**: Rewrite the assertion message: "Add a `@command_spec(...)` decorator on the Command DTO and a `register(registry)` call in the owning handler module (see `docs/systems/orders_system.md`)."
- [x] Verify the test still passes (the message change doesn't affect the assertion logic).
...
- [x] Read the existing try/except wallpaper:
  ```python
  try:
  ...
  ```
- [x] Note: PROJ-446 F-C-021 originally cited this finding but used the wrong filename (`tech_tree.json` doesn't exist; the real file is `data/techtree.json`). F-D-020 closes the actual finding.
- [x] Confirm `data/techtree.json` exists: `ls data/techtree.json`
- [x] **GREEN**: Drop the try/except. Let `FileNotFoundError` propagate naturally — if the production data file is genuinely missing, the test should fail loudly.
- [x] Run targeted test; should pass (the file exists, so no exception is raised).
...
- [x] Read existing static guards for the canonical pattern. Codex r3 verified paths (the original phase-4 references were wrong — both files live elsewhere):
...
- [x] **GREEN**: Create the new guard. Use a runtime `Path.exists()` check rather than an AST scan (the asserted invariant is "the file does not exist"):
  ```python
  ...
  ```
- [x] **RED-check**: Temporarily create an empty `game/strategy/engine/commands/specs.py`; confirm the guard fails. Delete the temp file; the guard now passes.
- [x] Add a module docstring explaining the pairing: this guard + `tests/unit/strategy/engine/test_no_specs_tuple_literal.py` together prevent both ways of re-introducing the deleted module (file existence + tuple-literal anti-pattern).
...
- [x] Test assertion message at `test_command_specs_contract.py:85-87` updated
- [x] Wallpaper at `test_workflow.py:188-192` removed
- [x] Static guard `test_no_commands_specs_module.py` in place and green
- [x] Run `python Tools/test_sharded/test_sharded.py` — full sharded suite green
- [x] Run `python Projects/scripts/validate_phase.py PROJ-447 4` — PASSED
- [x] Update status to `Complete`; plan.md phase table + Current State → Phase 5

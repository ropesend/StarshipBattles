...
- [ ] Read existing `TechTree.__init__(self):` at line 26 and `load_from_json(cls, file_path: str = None)` at line 31
- [ ] **GREEN — F-D-014**: Add `-> None` return to `__init__`
- [ ] **GREEN — F-D-013**: Change `file_path: str = None` → `file_path: str | None = None` on `load_from_json`. Add return annotation if absent.
- [ ] Sweep sibling sites in the research module flagged by the finding text:
...
- [ ] Read the AssetManager class — confirm `from __future__ import annotations` is at line 1 (per the finding)
- [ ] **GREEN — F-D-016**: Replace `_default_asset_manager: Optional['AssetManager'] = None` with `_default_asset_manager: "AssetManager | None" = None`. Drop the `from typing import Optional` import if no longer needed.
- [ ] **GREEN — F-D-015**: Annotate the 5-method cluster:
  - `__init__(self) -> None:`
...
- [ ] Run targeted tests (if any) + sharded suite.
...
- [ ] Read `CollisionSystem.__init__(self, rng: 'random.Random' = None)` at collision.py:68
- [ ] **GREEN**: Drop the string-quoted forward ref. Change to `(self, rng: random.Random | None = None) -> None`. Verify `import random` is at the top of the file; add it if not.
- [ ] Read `PhysicsBody.__init__(self, x, y, angle=0)` at physics.py:53
- [ ] **GREEN**: Annotate as `(self, x: float, y: float, angle: float = 0.0) -> None`. (Verify the angle's natural unit — likely radians as float.)
- [ ] Run targeted tests.
...
- [ ] Read `load_components_data(file_path: str = None, ...)` at line 78 and `load_modifiers_data(file_path: str = None) -> dict` at line 186
- [ ] **GREEN**: Change `file_path: str = None` → `file_path: str | None = None` on both. Verify or add the return type annotations (`-> dict | None` if the function can return None on error).
- [ ] Run targeted tests.
...
- [ ] Same recipe as F-D-018 — two public module-level loaders with `file_path: str = None`. Update both.
- [ ] Run targeted tests.
...
- [ ] All 6 annotation-sweep tasks complete
- [ ] No remaining `file_path: str = None` patterns in the touched files (run `rg -n "str\s*=\s*None|Dict\[.*\]\s*=\s*None" game/research/ game/assets/ game/engine/ game/simulation/components/component_loader.py game/simulation/entities/ship_loader.py` — should return 0 hits)
- [ ] Run `python Tools/test_sharded/test_sharded.py` — full sharded suite green
- [ ] Run `python Projects/scripts/validate_phase.py PROJ-447 3` — PASSED

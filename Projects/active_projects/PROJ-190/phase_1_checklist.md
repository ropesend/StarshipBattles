...
- [ ] Create file with module docstring explaining purpose and portability goals
- [ ] Import: `typing.Protocol`, `runtime_checkable`, `TYPE_CHECKING`, `TypeGuard`, `Optional`, `List`, `Dict`, `Any`, `Set`
- [ ] Define `IAbility` protocol:
...
- [ ] Define `IResourceConsumptionAbility(IAbility)` protocol:
  - `resource_type: str` (property)
  - `amount: float` (property)
  - `check_available() -> bool` (method)
- [ ] Define `IResourceStorageAbility(IAbility)` protocol:
  - `resource_type: str` (property)
  - `max_amount: float` (property)
- [ ] Define `IResourceGenerationAbility(IAbility)` protocol:
  - `resource_type: str` (property)
  - `rate: float` (property)
- [ ] Define `IWeaponAbility(IAbility)` protocol:
  - `damage: float`, `range: float`, `reload_time: float`, `firing_arc: float` (properties)
  - `get_damage(range_to_target: float = 0) -> float` (method)
- [ ] Define `IBeamWeaponAbility(IWeaponAbility)` protocol:
  - `base_accuracy: float`, `accuracy_falloff: float` (properties)
- [ ] Define `ISeekerWeaponAbility(IWeaponAbility)` protocol:
  - `projectile_speed: float`, `endurance: float`, `turn_rate: float`, `projectile_damage: float`, `projectile_hp: float` (properties)
- [ ] Define `IProjectileWeaponAbility(IWeaponAbility)` protocol:
  - `projectile_speed: float` (property)
- [ ] Define `IWarpJumpAbility(IAbility)` protocol:
  - `max_tonnage: float`, `energy_cost: float` (properties)
- [ ] Add TypeGuard helper functions: `is_resource_consumption()`, `is_resource_storage()`, `is_resource_generation()`, `is_weapon()`, `is_beam_weapon()`, `is_seeker_weapon()`, `is_projectile_weapon()`, `is_warp_jump()`
- [ ] Verify: `python -c "from game.simulation.interfaces.ability_protocols import IAbility"` succeeds
...
- [ ] Add `is_component()` TypeGuard function
- [ ] Verify: `python -c "from game.simulation.interfaces.component_protocols import IComponent"` succeeds
...
- [ ] Create file with module docstring
- [ ] Import Vector2 type under TYPE_CHECKING
- [ ] Define `ICombatShip` protocol:
...
- [ ] Define `IPhysicsShip` protocol:
  - `is_thrusting: bool`, `engine_throttle: float`, `turn_throttle: float`
  - `mass: float`, `turn_speed: float`, `acceleration_rate: float`
- [ ] Define `IFormationHost` protocol:
  - `formation: Any`
- [ ] Define `ISerializableShip` protocol:
  - `total_strategic_movement: float`, `warp_max_tonnage: float`, `warp_energy_cost: float`
  - `ship_class: str`, `vehicle_type: str`, `theme_id: str`
- [ ] Add TypeGuard helpers: `is_combat_ship()`, `is_projectile()`, `is_physics_ship()`, `is_formation_host()`, `is_serializable_ship()`
- [ ] Verify: `python -c "from game.simulation.interfaces.entity_protocols import ICombatShip"` succeeds
...
- [ ] Import and re-export all protocols from `ability_protocols`, `component_protocols`, `entity_protocols`
- [ ] Import and re-export all TypeGuard functions
- [ ] Update `__all__` list
- [ ] Verify: `python -c "from game.simulation.interfaces import ICombatShip, IComponent, IAbility"` succeeds
- [ ] Run full simulation unit tests: `pytest tests/unit/simulation/ -n 12` — all pass
...
- [ ] `pytest tests/unit/simulation/ -n 12` — all pass (no regressions)
- [ ] All 15 protocols importable from `game.simulation.interfaces`

...
- [x] Narrow `handle_command` (line 403) from `-> Any` to `-> ValidationResult` (TYPE_CHECKING import of `Command`). Docstring already states "Returns: ValidationResult"
...
- [x] Narrow `_get_planet_mutator` (line 196) to `-> PlanetWriteService`
- [x] Narrow `_get_empire_mutator` (line 205) to `-> EmpireWriteService`
- [x] Verify: tests pass; `mypy` clean
...
- [x] Narrow `_get_planet_mutator` (line 143) to `-> PlanetWriteService`
- [x] Narrow `_get_ship_mutator` (line 152) to `-> ShipInstanceWriteService`
- [x] Verify: tests pass; `mypy` clean
...
- [x] Narrow `_get_ship_mutator` (line 65) to `-> IShipInstanceMutator`
- [x] Verify: tests pass; `mypy` clean
...
- [x] Narrow `_get_planet_mutator` (line 34) to `-> IPlanetMutator`
- [x] Verify: tests pass; `mypy` clean
...
- [x] Narrow `_get_planet_mutator` (line 103) to `-> IPlanetMutator` (or `-> PlanetWriteService` — match concrete return)
- [x] Verify: tests pass; `mypy` clean
...
- [x] Narrow `_get_empire_mutator` (line 77) to `-> IEmpireMutator`
- [x] Verify: tests pass; `mypy` clean
...
- [x] Narrow `_resolve_build_entity` (line 323 / current ~336) to `-> Planet | Fleet | None`
- [x] Narrow `_resolve_queue_owner` (line 377 / current ~390) to `-> Planet | Fleet | PlanetaryFacility | None`
- [x] Narrow `_build_colonize_target` (line 419 / current ~432) to `-> Planet | dict[str, Any]`
- [x] Verify: tests pass; `mypy` clean
...
- [x] Add `-> Iterator[Fleet]` to `_at_hex` nested generator (line 157)
- [x] Add `-> Iterator[Fleet]` to `_in_system` nested generator (line 163)
- [x] Verify: tests pass; `mypy` clean
...
- [x] Add `-> Generator[tuple[str, dict[str, Any]], None, None]` to `_walk_strategic_abilities` (line 128) — generator yielding `(ability_name, ability_data)` tuples
- [x] Verify: tests pass; `mypy` clean
...
- [x] Add `-> Ship` to nested closure `_replay_combat_lab_fallback` (line 310)
- [x] Verify: tests pass; `mypy` clean
...
- [x] Verify PROJ-481 owns the `_button_handlers` annotation; remove this task if confirmed

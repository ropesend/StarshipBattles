...
- [ ] Run the canonical discovery commands from the findings file:
  ```bash
  ...
  ```
- [ ] Diff the 2026-05-19 caller list in `findings/PROJ-454_findings.md` against the current output. Note any callers added since the audit. If the diff is non-zero, update `findings/PROJ-454_findings.md` "F-B-005 caller list" sections in place.
- [ ] **Sanity check the symbol map**: for each of the 16 symbols listed in `findings/PROJ-454_findings.md` "F-B-005 symbol map (canonical)", verify the destination module per `component_inspector.py:28-47`. If a symbol has moved or been renamed, document in `decisions.md`.
...
- [ ] For each file, replace the import path on the affected lines. Per-line edits:
...
- [ ] For each file, repoint the import to `component_abilities` (all engine callers consume Surface A symbols per the 2026-05-19 audit — verify against the caller list before editing).
- [ ] Run targeted tests; confirm green.
...
- [ ] Repoint each import to `component_abilities` (Surface A).
- [ ] Run targeted tests; confirm green.
...
- [ ] **DISCIPLINE GATE**: re-read the "UI behaviour preservation" section in `findings/PROJ-454_findings.md` before starting. Edit only the import statement. If you notice UI residue while doing the touch, log it via `/claude-di-log` — do NOT fix inline.
- [ ] For each file, repoint the import to `component_abilities` (all 6 UI callers consume Surface A symbols).
- [ ] Run targeted tests; confirm green.
...
- [ ] Repoint imports per the symbol map.
- [ ] **Decide file renames**:
...
- [ ] Run the renamed tests; confirm green.
...
- [ ] For each `patch('game.strategy.services.component_inspector.X', ...)` site, repoint to the canonical module path:
...
- [ ] **Critical**: a `patch()` target is path-based, not symbol-based. Once `component_inspector.py` is deleted, any surviving `patch('game.strategy.services.component_inspector.X')` will raise `AttributeError` at test setup. This is the bug class Task 2.7 prevents.
- [ ] Run targeted tests; confirm green.
...
- [ ] Same recipe as Task 2.7, but for **11 sites in a single file** — the largest single-file test migration in PROJ-454.
- [ ] Walk through each site; identify the symbol being patched; repoint to `component_abilities` (all 11 are Surface A per the 2026-05-19 audit).
- [ ] Lines 345-380 (6 sites) — `has_warp_capability` patches; lines 863, 1079, 1105, 1155, 1178 (5 sites) — `ship_has_ability` patches.
- [ ] Run targeted tests; confirm green.
...
- [ ] Read the full file. It's a static drift gate against the shim's export surface.
- [ ] **Decision**:
...
- [ ] Apply the decision; document in `decisions.md`.
...
- [ ] **Pre-delete sanity check**: `git grep -n "component_inspector" game/ tests/` should return only `Projects/archived_projects/` matches (historical narration) and the `component_abilities.py:11` / `component_layers.py:14` docstring references (which describe the retired shim).
- [ ] If the sanity check passes, delete: `git rm game/strategy/services/component_inspector.py`.
- [ ] **Post-delete sanity check**: `python -c "from game.strategy.services import component_inspector"` should fail with `ModuleNotFoundError`.
...
- [ ] `git grep -n "component_inspector" game/ tests/` returns zero matches (excluding archived projects + the canonical-module docstring references documenting the shim's historical existence).
- [ ] `game/strategy/services/component_inspector.py` no longer exists.
...
- [ ] F-B-005 closed (documented in `decisions.md`)
- [ ] `component_inspector.py` deleted
- [ ] All ~68 caller migrations landed green (52 `import` + 16 `patch` — count corrected by codex audit 2026-05-19)
- [ ] All `patch(...)` targets repointed
- [ ] Static drift-gate test deleted or refactored into a re-emergence guard
- [ ] `pytest tests/unit/ tests/integration/ -q` green
- [ ] Full sharded suite green (`python Tools/test_sharded/test_sharded.py`)
- [ ] Run `python Projects/scripts/validate_phase.py PROJ-454 2` — PASSED

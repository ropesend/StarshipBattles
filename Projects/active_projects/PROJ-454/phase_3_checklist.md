...
- [ ] Run the canonical discovery commands:
  ```bash
  ...
  ```
- [ ] Diff against the 2026-05-19 caller list in `findings/PROJ-454_findings.md`. Note any new callers since the audit.
- [ ] **Verify**: there should be **zero production callers** of the three legacy facade methods (per the 2026-05-19 audit). If any production caller surfaces, surface for user decision — that changes the project's risk profile.
...
- [ ] Confirm `OrderExecutionResult` carries all the fields the test callers will need after migration:
...
- [ ] The migration is field-by-field equivalent — every property that the legacy result types carried is already on `OrderExecutionResult`. Phase 3 is a thin remapping, not a behaviour change.
...
- [ ] For each `processor.process_colonize(...)` call, replace with the handler-direct path. Recipe:
  ```python
  # Before (line 286-ish):
  ...
  ```
- [ ] Update any assertion that reads `result.colonized` / `result.planet_name` — these attributes already exist on `OrderExecutionResult`, so the assertion text doesn't change.
- [ ] Run targeted tests; confirm green.
- [ ] **Verify**: `git grep -n "process_colonize" tests/integration/colonization/test_planet_specific_colonization.py` returns zero matches after this task.
...
- [ ] For each `processor.process_transfer(...)` call (3 sites), replace with the handler-direct path:
  ```python
...
- [ ] Replace the single `processor.process_join_fleet(...)` call with:
  ```python
...
- [ ] Line 22: drop the `ColonizeResult` import. The import line currently reads `from game.strategy.engine.order_processor import OrderProcessor, ColonizeResult`. Change to `from game.strategy.engine.order_processor import OrderProcessor`.
- [ ] Lines 180, 211, 245, 280, 310, 341 (6 call sites — corrected count from codex audit 2026-05-19; original cited 2 sites): migrate each `processor.process_colonize(...)` call per the Task 3.3 recipe.
- [ ] Run targeted tests; confirm green.
...
- [ ] Line 15: drop the `TransferResult` import.
- [ ] Strict call-site count (2026-05-19 audit): **7 `processor.process_transfer(...)` call sites** at lines 196, 230, 262, 293, 327, 372, 414. Line 103 docstring is narration only — update or drop. Migrate every call per the Task 3.4 recipe.
- [ ] Run targeted tests; confirm green.
...
- [ ] Line 102 (docstring): mentions `ColonizeResult(colonized=False)`. Update the docstring to reflect `OrderExecutionResult(colonized=False)` (or whatever the test actually asserts after migration).
- [ ] Migrate all 7 `proc.process_colonize(...)` calls in the file body (lines 105, 124, 157, 181, 213, 245, 283 — codex audit 2026-05-19 confirmed via live grep) per the Task 3.3 recipe.
- [ ] Line 304: comment references "the legacy process_colonize delegate" — drop or update.
- [ ] Run targeted tests; confirm green.
...
- [ ] Added by codex audit 2026-05-19 — file was MISSING from the original Phase 3 inventory.
- [ ] Strict call-site count (2026-05-19 audit): 3 `processor.process_transfer(...)` sites at lines 97, 106, 115. (Line 91 docstring is narration only.)
- [ ] Migrate each call per the Task 3.4 recipe (use `processor.get_handler(OrderType.TRANSFER).execute_action_order(...)`).
- [ ] Run targeted tests; confirm green.
...
- [ ] Added by codex audit 2026-05-19 — file was MISSING from the original Phase 3 inventory.
- [ ] Migrate both `proc.process_join_fleet(...)` calls (lines 247, 268) per the Task 3.5 recipe (use `processor.get_handler(OrderType.JOIN_FLEET).execute_action_order(...)`).
- [ ] Update the file docstring (lines 8, 234) and test-name narration referring to `process_join_fleet` — switch to the unified handler vocabulary.
- [ ] Run targeted tests; confirm green.
...
- [ ] Added by codex audit 2026-05-19 — file was MISSING from the original Phase 3 inventory. **Second-largest single-file migration in Phase 3.**
- [ ] Walk every site; migrate each `proc.process_transfer(...)` to the handler-direct path per Task 3.4 recipe.
- [ ] Drop / update the section-header comments at lines 63 ("`process_transfer`: invalid params") and any other narration referring to the facade method.
- [ ] Run targeted tests; confirm green.
...
- [ ] Added by codex audit 2026-05-19 — file was MISSING from the original Phase 3 inventory.
- [ ] Strict call-site count (2026-05-19 audit): 6 `processor.process_colonize(...)` sites at lines 201, 234, 271, 307, 386, 420. (Lines 2-6 and 176, 320 are docstring/comment narration referring to `process_colonize()`; not call sites.)
- [ ] Migrate each call per the Task 3.3 recipe.
- [ ] Update the file docstring (lines 2-6) and section-header docstrings at lines 176, 320 — they narrate `process_colonize()` as the unit under test; rewrite to reference the handler.
- [ ] Run targeted tests; confirm green.
...
- [ ] Added by codex audit 2026-05-19 — file was MISSING from the original Phase 3 inventory.
- [ ] Strict call-site count (2026-05-19 audit): 5 `processor.process_colonize(...)` sites at lines 496, 528, 592, 991, 1047. (Line 478 docstring is narration only.)
- [ ] Migrate each call per the Task 3.3 recipe.
- [ ] Update test docstrings at lines 477-478 (and any other narration that references `process_colonize`) — switch to the unified-handler vocabulary.
- [ ] Run targeted tests; confirm green.
...
- [ ] Added by codex audit 2026-05-19 — file was MISSING from the original Phase 3 inventory. **Largest single-file migration in Phase 3 (11 sites).**
- [ ] The file mixes `process_join_fleet` (sites at 82, 102, 116, 129) and `process_colonize` (sites at 196, 224, 245, 269, 520, 549, 577). Migrate each to the matching handler:
...
- [ ] Update test docstrings (lines 68, 88, 107, 122-123, 177, 205, 232, 253, 501, 531, 558) referencing `process_join_fleet` / `process_colonize` as the unit under test.
- [ ] Run targeted tests; confirm green.
...
- [ ] Read the file end-to-end. Per the docstring at lines 33, 37, 53-54, this file specifically tests the **facade** — i.e., the surface PROJ-454 is deleting.
- [ ] **Decision**: delete the file outright (the surface is gone) OR rewrite to test the unified `execute_action_order` contract (which is then duplicated coverage of the per-handler tests). Recommendation: **delete**, document in `decisions.md`.
- [ ] If deleting: `git rm tests/unit/strategy/engine/order_handlers/test_order_processor_facade.py`.
...
- [ ] **Pre-delete sanity check**: `git grep -nE "\.process_(join_fleet|colonize|transfer)\(" game/ tests/` should return ZERO call sites outside `game/strategy/engine/order_processor.py` (the lines being deleted). The 68 call sites enumerated in the "Live caller inventory" block at the top of this checklist should all be migrated by Tasks 3.3 through 3.8f. If any test caller remains, identify the file and migrate before deleting.
- [ ] Also run `git grep -nE "process_join_fleet|process_colonize|process_transfer" game/ tests/` (no word boundary, captures docstring/comment references). Expected survivors after migration:
...
- [ ] Also `git grep -nE "JoinFleetResult|ColonizeResult|TransferResult" game/ tests/` should return only internal references + archived.
- [ ] Delete `order_processor.py:39-58` (the three dataclasses) and `:97-143` (the three facade methods).
- [ ] Update the module docstring at `order_processor.py:1-20` — drop the narration about "Public surface kept for backward compatibility" + the dataclass listing.
- [ ] Update any internal references to the deleted symbols (e.g., docstrings in nearby methods).
...
- [ ] `git grep -nE "\b(process_join_fleet|process_colonize|process_transfer)\(" game/ tests/` returns zero matches (excluding archived projects). This is the call-site grep — the strict signal that every caller migrated.
- [ ] `git grep -nE "JoinFleetResult|ColonizeResult|TransferResult" game/ tests/` returns zero matches (excluding archived projects).
- [ ] `OrderProcessor` class no longer has `process_join_fleet` / `process_colonize` / `process_transfer` methods.
- [ ] Document closure in `decisions.md`: `2026-XX-XX | F-B-017 closed | Migrated 68 test call sites across 12 test files (count corrected by codex audit 2026-05-19 from the original ~15 sites / 7 files) to processor.get_handler(...).execute_action_order(...). Deleted 3 legacy facade methods + 3 typed result dataclasses (~50 LOC). All readers now consume OrderExecutionResult directly. | PROJ-454 Phase 3.`
...
- [ ] F-B-017 closed (documented in `decisions.md`)
- [ ] Three legacy facade methods deleted from `order_processor.py`
- [ ] Three legacy result dataclasses deleted from `order_processor.py`
- [ ] All 68 test call sites across 12 test files migrated to the unified result path (count corrected by codex audit 2026-05-19 from the original `~15 sites / 7 files`)
- [ ] `pytest tests/integration/colonization/ tests/integration/strategy/ tests/unit/strategy/ tests/unit/strategy/engine/ -q` green
- [ ] Full sharded suite green (`python Tools/test_sharded/test_sharded.py`)
- [ ] Run `python Projects/scripts/validate_phase.py PROJ-454 3` — PASSED

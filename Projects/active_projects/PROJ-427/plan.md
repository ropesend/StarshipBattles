...
**Active Phase:** Phase 5 complete; resuming with Phase 6.
**Last Action:** Phase 5 — Replay-store wiring converted from module-global to instance-owned. Module-level names `_replay_store`, `set_replay_store`, `get_replay_store`, `_notify_replay_store_save_or_load`, `_notify_replay_store_save_deleted` REMOVED from `game/strategy/systems/save_game_service.py`. `SaveGameService.__init__(replay_store=None)` stores the hook on `self._replay_store`. Instance methods `set_replay_store(store)`, `get_replay_store()`, `_notify_replay_store_save_or_load(path)`, `_notify_replay_store_save_deleted()` own the wiring. A class-level `default()` classmethod returns the process-level singleton; static `save_game`/`load_game`/`delete_save` consult `SaveGameService.default()` for hook notifications. All 9 call sites (`game/app_bootstrap.py`, `game/ui/screens/strategy_windows/event_log_window_ctrl.py`, 5 integration test files, 3 unit test files) migrated to `SaveGameService.default().set_replay_store(...)` / `.get_replay_store()`. Phase 0 characterization tests in `test_save_load_ops.py` flipped from module-global shape to default-singleton shape. New tests: `tests/unit/strategy/save_game_service/test_replay_store_instance.py` (6 tests pinning module-globals are gone, instance round-trip, default-singleton stability, save/load/delete notifications, no-store safe). Focused suites: 97 + 98 = 195 passed across `save_game_service/` + `replay/` + `bootstrap-invariants` + `main-integration` + `replay-capture-e2e`.
**Next Action:** Begin Phase 6 (UI migration — `FacadeSessionState.designs_by_empire` to catalog — and `DesignLibrary` deletion gate).
**Blockers:** None.

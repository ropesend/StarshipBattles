...
| 6. Migrate UI callers and delete the old `DesignLibrary` shim | Deferred to PROJ-434 ([plan](../PROJ-434/plan.md)) | [phase_6_checklist.md](phase_6_checklist.md) | Phase 5 |

## Current State
**Last Updated:** 2026-05-17
**Active Phase:** Phases 0-5 complete; Phase 6 spun off to PROJ-434.
**Note on Phase 6 status:** `phase_state.json` records `phase_6.status` as `partial` (the committed dependency inversion + 1-UI migration). The remaining deletion work is owned by [PROJ-434](../PROJ-434/plan.md) — PROJ-427 itself closes with Phase 6 marked as deferred-to-spinoff rather than `verified`.

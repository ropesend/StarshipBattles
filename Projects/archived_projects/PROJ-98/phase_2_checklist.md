...
- [x] Add `get_resource_rate_text(source: BuildQueueSource, resource_name: str) -> str`:
  - Empty queue -> "-"
...
- [x] Add `get_resource_total_text(source: BuildQueueSource, resource_name: str) -> str`:
  - Empty queue -> "-"
...
- [x] Add `TestGetResourceRateText` class:
...
- [x] Add `TestGetResourceTotalText` class:
  - `test_empty_queue_returns_dash` - no queue items -> "-"
...
- [x] Append 5 rate columns to `DEFAULT_COLUMNS`:
  ```python
...
- [x] Add module-level constants (after imports):
  ```python
  RESOURCE_RATE_COLS = {
  ...
  ```
- [x] Import `get_resource_rate_text`, `get_resource_total_text` from formatter module (line 27-34)
- [x] Extend `_get_column_value()` (line 524) with:
  ```python
...
- [x] `test_empire_build_queue_window.py` line 411: change `assert len(visible) == 8` to `assert len(visible) == 18`
- [x] `TestColumnConfiguration.test_expected_column_ids`: add all 10 new IDs to `expected` set
- [x] `test_empire_build_queue_filter_manager.py` `test_columns_list_populated_with_expected_ids`: add 10 new IDs to expected list
- [x] `test_empire_build_queue_filter_manager.py` `test_all_columns_visible_by_default`: update count to `len(DEFAULT_COLUMNS)` (18) - already uses len(DEFAULT_COLUMNS)
- [x] Add tests in `TestGetColumnValue` for at least 2 resource rate and 2 resource total column IDs
...
- [x] Verify: no regressions (7636 passed full suite)

...
- **Reference value over production call** for stable expecteds: see `tests/unit/strategy/engine/test_resupply_engine.py` (Task 5.18) and `tests/unit/strategy/formulas/test_colony_output.py` (Task 5.19) for hardcoded reference values with derivation comments in docstrings. (Originally cited `test_projectile_manager.py`, but that file was deleted by upstream PROJ-321 — corrected by PROJ-325 Phase 1 Task 1.5 per OpenCode 323-review FND-P2-003.)
- **Hard-assertion regression guard with adjustable threshold**: see `tests/regression/test_deprecated_code_removed.py` (Task 4.2) — kept as hard count-based assertion (`assert total <= EXPECTED_X_COUNT`) where the threshold is reviewed/raised when intentional additions land. This preserves the regression signal without converting to a non-failing advisory pattern. (Earlier wording described this as "advisory soft assertions"; that mischaracterized the implementation. Corrected by PROJ-325 Phase 1 Task 1.5 per OpenCode 323-review FND-P2-005.)

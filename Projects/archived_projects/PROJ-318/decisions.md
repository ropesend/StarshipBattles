...
| 2026-04-28 | Phase 6 existing-theme validator sweep records known asset debt instead of requiring all-pass | Phase 5 proved current assets still have 151 portrait size mismatches and 20 missing portraits. `validate_theme.py` now truthfully rejects declared undersized portraits, so 8 existing themes fail until the out-of-scope asset regeneration work is done. The Phase 6 checklist was corrected to record the sweep result rather than dilute validation rules. |

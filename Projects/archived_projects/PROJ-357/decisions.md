...
Post-merge OpenCode review (`Reviews/results/2026-05-05_055830_code_proj-357-review-fleet-aura-provider-identity_req-req_20260505_055830_bbffca/`) flagged 1 MAJOR finding. MINOR/INFO findings deferred per remediation scope.

| Finding ID | Severity | Verdict | Rationale |
|------------|----------|---------|-----------|
| CQ-001 | MAJOR | Fixed | Derelict ship aura contribution was inconsistent with UI: `_recalculate()` and `initialize()` only filtered on `is_alive`, while `get_active_bonuses()` also filtered `is_derelict` and `_get_provider_fingerprint()` already keyed on it. Math, fingerprint, and UI now all agree. Added `is_derelict` skip (not drop — recovery should restore contribution) at `_recalculate()` (alongside the existing `is_alive` and `is_operational` skips), and at the registration scan in `initialize()` and `register_ship()` to avoid registering providers on already-derelict ships. Two new tests in `test_fleet_aura_provider_identity.py` cover (a) live derelict toggle drops and recovers contribution while retaining the provider entry, and (b) `initialize()` does not register providers for ships starting the battle derelict. Targeted suite (10/10) and full sharded suite (17,766/17,766) green. |

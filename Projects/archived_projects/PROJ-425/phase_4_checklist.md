...
- [x] Re-read the Phase 0 manager-name findings in `findings_ledger.md`.
- [x] Re-grep `rg -n "self\._cargo_manager|self\._consumable_manager|self\.cargo_manager|self\.consumable_manager" game/strategy/data/ship_instance.py game/strategy/services/ship_instance_write_service.py` and record any divergence.
- [x] Identify the canonical name(s) and capture the decision in `decisions.md`.
...
- [x] Rename on whichever side does not match the chosen canonical name. Pick one direction; do not introduce parallel accessors.
- [x] Update tests and any internal references mechanically.
- [x] **Verify:** `pytest tests/unit/strategy/ship_instance/ tests/unit/strategy/services/test_ship_instance_write_service.py -x` — all green before any logic moves.
...
- [x] Add tests that call the write service for: `toggle_component(...)` (or equivalent) + assert cache invalidation; `repair(...)` / full-repair + assert hp restore + cache invalidation.
- [x] **Verify:** tests fail today (the write service does not yet own these behaviors).
...
- [x] Cut the toggle + repair logic out of `ShipInstance` and into the write service. Centralize cache invalidation in the write service — do not leave it half on the entity and half in the service.
- [x] If the entity keeps thin forwarders for back-compat, document them as Phase-5-style demolition candidates.
- [x] **Verify:** Task 4.2 tests pass; Phase 0 toggle/repair tests still pass.
...
- [x] `pytest tests/unit/strategy/ship_instance/ tests/unit/strategy/services/ tests/unit/strategy/fleets/ -x`
...
- [x] Record post-phase `wc -l ship_instance.py` and `wc -l ship_instance_write_service.py` in `findings_ledger.md`.
- [x] Run `python Projects/scripts/phase_complete.py PROJ-425 phase_4`.
...
- [x] Manager/accessor names are consistent between entity and write service
- [x] Toggle + repair logic centralized on `ShipInstanceWriteService`
- [x] Cache invalidation is centralized in the write service (not split)

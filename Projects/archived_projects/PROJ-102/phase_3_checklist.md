...
- [x] Add `DYSON_SPHERE = auto()` to PlanetType enum (after the last existing value)

**Notes:** Added after PLANETOID

### Task 3.2: Add Galaxy Cleanup Methods [Medium]
**File:** `game/strategy/data/galaxy.py` (Galaxy class)
**Tests:** `pytest tests/unit/strategy/data/test_galaxy_cleanup.py`

- [x] Add `Galaxy.unregister_planet(self, planet)`:
...
- [x] Add `Galaxy.remove_warp_link(self, system_a_name: str, system_b_name: str)`:
...
- [x] Add `Galaxy.get_all_fleets_in_system(self, system, empires)`:
...
- [x] Add to EventType enum:
  ...
  ```
- [x] Add to EventCategory enum:
  ```python
  SUPERWEAPONS = "superweapons"
  ```

**Notes:** Updated test_event_types.py member count assertions (4→10 EventType, 4→5 EventCategory)

### Task 3.4: Dyson Sphere Image Registration [Simple]
**File:** `game/strategy/generation/planet_image_registry.py`
**Tests:** Manual verification

- [x] Verify `assets/Images/Stellar Objects/Sphere world/Sphereworld_Portrait.png` exists
- [x] In `PlanetImageRegistry._load_classifications()` or `__init__()`:
  - Add `self._type_to_images[PlanetType.DYSON_SPHERE] = ["Sphereworld_Portrait.png"]`
  - Ensure PlanetType import is available

**Notes:** Added after classifications load, before logging
...
- [x] Test `PlanetType.DYSON_SPHERE` exists in enum
- [x] Test `Galaxy.unregister_planet()`:
...
- [x] `pytest tests/ -n 12` passes (7871 passed)

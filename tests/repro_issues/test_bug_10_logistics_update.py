...
def test_ammo_usage_triggers_logistics_row():
    ...
    ship.layers[LayerType.INNER]['components'].append(railgun)
    ...
    # Current BUG behavior: Uses default reload 1.0 -> 1.0 per second (incorrect)
    
    expected_rate = 0.2
    actual_rate = getattr(ship, 'ammo_consumption', 0)
    
    # We want this to fail if it's not 0.2
    assert abs(actual_rate - expected_rate) < 0.001, f"Ammo consumption incorrect. Expected {expected_rate}, got {actual_rate}"



    # We expect 'ammo_max_usage' (or similar) to force the series of Ammo rows to appear.
    ...
    ship.layers[LayerType.INNER]['components'].append(laser)

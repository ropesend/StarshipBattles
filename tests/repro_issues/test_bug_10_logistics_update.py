...
def test_ammo_usage_triggers_logistics_row(fresh_registries):
    """
    ...
    # Update fresh_registries with test vehicle classes
    fresh_registries.vehicle_classes.clear()
    fresh_registries.vehicle_classes.update(test_vehicle_classes)

    ship = Ship(name="TestShip", x=0, y=0, color=(255, 255, 255), ship_class="TestClass", registries=fresh_registries)

    # 2. Add a Weapon that uses Ammo
    # Railgun: Uses Ammo on activation
    railgun = Component({"id": "railgun", "name": "Railgun", "mass": 10, "hp": 50, "type": "Internal"}, registries=fresh_registries)
    ...
    laser = Component({"id": "laser", "name": "Laser", "mass": 10, "hp": 50, "type": "Internal"}, registries=fresh_registries)

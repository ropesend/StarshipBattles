...
class TestBuilderWarningLogic(unittest.TestCase):
    def setUp(self):
        ...
        self.patcher = patch('builder_gui.BuilderSceneGUI._create_ui')
        self.mock_create_ui = self.patcher.start()
        self.addCleanup(self.patcher.stop)
        
        # Mock internal managers
        self.p1 = patch('builder_gui.SpriteManager')
        self.p2 = patch('builder_gui.PresetManager')
        self.p3 = patch('builder_gui.ShipThemeManager')
        self.p4 = patch('builder_gui.UIConfirmationDialog')
    ...
    def test_change_type_empty_ship(self):
        ...
        with patch('builder_gui.VEHICLE_CLASSES', {'Station': {'type': 'Station', 'max_mass': 5000}}):
            with patch.object(self.builder, '_execute_pending_action') as mock_execute:
                self.builder.handle_event(event)

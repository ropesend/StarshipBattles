...
class TestMaintenanceDisableKeepsFleet:
    ...
    def test_maintenance_disable_keeps_fleet_registered(
        ...
        # Empire has ZERO resources - maintenance will fail
        emp1.resource_pool = {}
        emp1.max_storage = {}

        # Process maintenance tick
    ...
    def test_maintenance_partial_disable_keeps_fleet(
        ...
        fleet = Fleet(701, emp1.id, loc, speed=15.0)

        # Ship 1: free (no resource_cost, no components with cost)
        free_ship = make_mock_ship_instance("Free Ship", emp1.id, registries=fresh_registries)
        free_ship.design_data = {
            'name': 'Free Ship',
            'vehicle_type': 'Ship',
            'stats': {'mass': 10},
            'layers': {}  # No components = no maintenance cost
        }

        # Ship 2: expensive (will fail maintenance)
        expensive_ship = make_mock_ship_instance("Expensive Ship", emp1.id, registries=fresh_registries)
        expensive_ship.design_data = {
            'name': 'Expensive Ship',
            'vehicle_type': 'Ship',
            'stats': {'mass': 100},
            'layers': {
        ...
        fleet.ships.append(free_ship)
        fleet.ships.append(expensive_ship)
        emp1.add_fleet(fleet)
        _assert_registered(galaxy, fleet)

        # Empire has zero resources
        emp1.resource_pool = {}
        emp1.max_storage = {}

...
class TestShipSpawning:
    ...
    def test_fleet_id_is_unique(self, production_setup):
        ...
        Fleet IDs are now generated by Galaxy.get_next_fleet_id() (globally
        unique). A mock galaxy is required so the production spawner can
        assign distinct IDs.
        """
        ...
        # Create a mock galaxy with sequential fleet ID generation
        mock_galaxy = MagicMock()
        _fleet_counter = [0]

        def _next_fleet_id():
            _fleet_counter[0] += 1
            return _fleet_counter[0]

        mock_galaxy.get_next_fleet_id = _next_fleet_id

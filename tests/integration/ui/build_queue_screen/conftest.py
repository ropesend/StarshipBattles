...
class MockSession:
    ...
    # PROJ-472 Phase 1B: expose the ``empires.hex_build_queues`` namespace so
    # production code (BuildQueueScreen) resolves build-queue *DTOs* off the
    # mock, mirroring ``FacadeEmpireQueries.hex_build_queues``. The mock
    # projects domain sources through ``BuildQueueSourceDTO.from_domain`` (the
    # same projection the real ``EmpireSlice`` performs) so the UI receives
    # frozen DTOs, not live domain ``BuildQueueSource`` objects.
    @property
    def empires(self):

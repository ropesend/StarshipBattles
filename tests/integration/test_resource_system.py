...
# -----------------------------------------------------------------------------
# Test 8: Fleet Mixed Legacy and New Ship Instances
# -----------------------------------------------------------------------------

class TestFleetMixedLegacyAndNewShipInstances:
    """Test fleet containing both legacy string ships and ShipInstance objects."""

    def test_fleet_mixed_legacy_and_new_ship_instances(self, loaded_registry):
        """
        Create fleet with both string ships and ShipInstance,
        verify resource methods work (string ships ignored),
        verify get_ship_instances() filters correctly.
        """
        registry = loaded_registry

        # Create components for the ShipInstance
        engine = create_mock_component(
            comp_id='test_mixed_engine',
            mass=80,
            max_hp=100,
            comp_type='Engine',
            abilities={
        ...
        registry.components['test_mixed_engine'] = engine

        fuel_tank = create_mock_component(
            comp_id='test_mixed_fuel',
            mass=40,
            max_hp=80,
            comp_type='Tank',
            abilities={
                'ResourceStorage': [{'resource': 'fuel', 'amount': 10000}]
            }
        )
        registry.components['test_mixed_fuel'] = fuel_tank

        # Create a real ShipInstance
        design_data = create_test_ship_design(
            name='Modern Destroyer',
            components=[
                {'id': 'test_mixed_engine'},
                {'id': 'test_mixed_fuel'}
            ]
        )

        ship_instance = make_ship_instance(design_data)
        ship_instance.resource_levels['fuel'] = 10000

        # Create fleet with mixed content
        fleet = Fleet(fleet_id=1, owner_id=0, location=HexCoord(0, 0))

        # Add legacy string ships (old format)
        fleet.ships.append("Scout")
        fleet.ships.append("Frigate")

        # Add modern ShipInstance
        fleet.add_ship_instance(ship_instance)

        # Verify total ship count
        assert len(fleet.ships) == 3

        # Verify get_ship_instances() filters correctly
        instances = fleet.get_ship_instances()
        assert len(instances) == 1
        assert instances[0] is ship_instance

        # Verify get_ship_names() works with mixed content
        names = fleet.get_ship_names()
        assert "Scout" in names
        assert "Frigate" in names
        assert "Modern Destroyer" in names

        # Verify resource methods work (only ShipInstance considered)
        fuel_cost = fleet.get_fuel_cost_per_hex()
        assert fuel_cost == 50  # Only from the ShipInstance

        movement_costs = fleet.get_movement_resource_costs()
        assert movement_costs.get('fuel', 0) == 50

        # Verify has_fuel_for_movement works
        assert fleet.has_fuel_for_movement() is True

        # Verify consume_fleet_fuel works
        success = fleet.consume_fleet_fuel(5)  # Move 5 hexes
        assert success is True

        # Verify only the ShipInstance was affected
        assert ship_instance.get_current_resource('fuel') == 10000 - (50 * 5)

        # Legacy string ships have no state to check, but they're still in the fleet
        assert "Scout" in fleet.ships
        assert "Frigate" in fleet.ships

...
from game.ui.screens.setup import load_ships_from_entries, scan_ship_designs, BattleSetupScreen
...
class TestFleetComposition(unittest.TestCase):
    ...
    @patch('game.ui.screens.setup.glob.glob')
    @patch('builtins.open', new_callable=mock_open)
    @patch('game.ui.screens.setup.json.load')
    ...
    @patch('game.ui.screens.setup.Ship')
    @patch('game.ui.screens.setup.filedialog.askopenfilename')
    @patch('game.ui.screens.setup.tk.Tk')
    @patch('uuid.uuid4')

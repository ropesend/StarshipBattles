...
class TestShipStatsBaseline:
    ...
    def test_turn_speed_calculation_from_thruster(self):
        ...
        assert ship.total_maneuver_points == 45.0
    ...
    def test_shield_stats_calculation(self):
        ...
        assert ship.shield_regen_rate == 25.0

        # Verify abilities
        shield_ab = shield.get_ability('ShieldProjection')
        assert shield_ab is not None
        assert shield_ab.capacity == 500.0

        regen_ab = regen.get_ability('ShieldRegeneration')
        assert regen_ab is not None
        assert regen_ab.rate == 25.0
    ...
    def test_ability_values_match_legacy_attributes(self):
        ...
        ab = engine.get_ability('CombatPropulsion')
        assert ab is not None, "Engine should have CombatPropulsion ability"
        assert ab.thrust_force == 2000.0

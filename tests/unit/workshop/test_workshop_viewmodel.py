...
class TestWorkshopViewModel:
    ...
    def test_set_ship_movement_policy_updates_and_emits(self, viewmodel_setup, mock_registries):
        """set_ship_movement_policy updates ship movement_policy and emits SHIP_UPDATED event."""
        from game.simulation.entities.ship import Ship
        viewmodel, event_bus = viewmodel_setup
        ...
        viewmodel.set_ship_movement_policy("brawl_close")

        assert ship.movement_policy == "brawl_close"
        events = event_bus.get_events('SHIP_UPDATED')
        assert len(events) == 1

    def test_set_ship_movement_policy_no_change_if_same(self, viewmodel_setup, mock_registries):
        """set_ship_movement_policy should not emit if policy unchanged."""
        from game.simulation.entities.ship import Ship
        viewmodel, event_bus = viewmodel_setup
        ...
        viewmodel.set_ship_movement_policy("kite_max")

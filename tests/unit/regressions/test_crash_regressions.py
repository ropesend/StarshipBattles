...
@pytest.fixture
def panel_setup():
    """Set up pygame, display, and WeaponsReportPanel for testing."""
    pygame.init()
    ...
    # Create Panel
    panel = WeaponsReportPanel(builder, manager, rect, sprite_mgr)

    yield {"surface": surface, "panel": panel}
...
class TestCrashRegressions:
    def test_weapons_panel_unbound_local_error(self, panel_setup):
        """
        ...
        """
        surface = panel_setup["surface"]
        panel = panel_setup["panel"]
        ...
        # Setup Panel State
        panel._max_range = 100  # Global max range > 0
        panel._weapons_cache = [{'weapon': weapon_mock, 'count': 1}]

        # Mock font render to avoid pygame errors
        panel.font = MagicMock()
        panel.small_font = MagicMock()
        panel.target_font = MagicMock()

        # Mock drawing methods to checking if they receive correct args (and prove we didn't crash before calling them)
        panel._draw_projectile_weapon_bar = MagicMock()
        ...
        # panel._draw_projectile_weapon_bar.assert_called()
        ...
        # calls = panel._draw_projectile_weapon_bar.call_args_list
        # if calls:
        #     args, _ = calls[0]
        #     weapon_bar_width = args[5]
        #     assert weapon_bar_width == 0, "Should have defaulted to 0 width"

    def test_weapons_panel_zero_max_range(self, panel_setup):
        ...
        # Mock helpers
        panel._get_scaled_icon = MagicMock(return_value=None)
        panel._get_weapon_name_surface = MagicMock(return_value=pygame.Surface((1, 1)))
        panel._draw_direction_indicator = MagicMock()
        panel._draw_projectile_weapon_bar = MagicMock()

        try:
            panel.draw(surface)
        except UnboundLocalError:
            pytest.fail("WeaponsReportPanel.draw() raised UnboundLocalError with max_range=0!")

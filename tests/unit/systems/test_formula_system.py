...
Use FormulaEvaluator.safe_evaluate for backwards-compatible behavior that returns defaults.
"""
...
class TestFormulaSystemSecurity:
    ...
    def test_sandbox_allows_valid_math(self):
        ...
        result = FormulaEvaluator.evaluate("sqrt(16) + 2", {})
...
class TestFormulaSystemFunctionality:
    ...
    def test_math_functions(self):
        ...
        assert FormulaEvaluator.evaluate("pow(2, 3)", {}) == 8.0
        assert FormulaEvaluator.evaluate("ceil(4.2)", {}) == 5
        assert FormulaEvaluator.evaluate("floor(4.8)", {}) == 4

    def test_context_variables(self):
        """Test that context variables are accessible."""
        context = {'x': 10, 'y': 5, 'ship_class_mass': 1000}
        assert FormulaEvaluator.evaluate("x + y", context) == 15
        assert FormulaEvaluator.evaluate("x * 2", context) == 20
        assert FormulaEvaluator.evaluate("sqrt(ship_class_mass)", context) == pytest.approx(31.622, rel=0.01)
    ...
    def test_complex_formula(self):
        ...
        result = FormulaEvaluator.evaluate("50 * sqrt(ship_class_mass / 1000)", context)
        assert result == 50.0

    def test_error_raises_formula_exception(self):
        """Test that errors raise FormulaException."""
        with pytest.raises(FormulaException):
            FormulaEvaluator.evaluate("undefined_var", {})
        with pytest.raises(FormulaException):
            FormulaEvaluator.evaluate("1 / 0", {})
        with pytest.raises(FormulaException):
            FormulaEvaluator.evaluate("invalid syntax ??", {})

    def test_safe_evaluate_returns_zero_on_error(self):
        """Test that FormulaEvaluator.safe_evaluate returns default on error."""
        assert FormulaEvaluator.safe_evaluate("undefined_var", {}, default=0) == 0
        assert FormulaEvaluator.safe_evaluate("1 / 0", {}, default=0) == 0
        assert FormulaEvaluator.safe_evaluate("invalid syntax ??", {}, default=0) == 0


class TestFormulaSystemErrorLogging:
    """Tests for error logging in formula evaluation (ERR-002).

    PROJ-45: FormulaEvaluator.evaluate now raises FormulaException.
    Use FormulaEvaluator.safe_evaluate for logging behavior.
    """
    ...
    def test_syntax_error_raises_exception(self):
        ...
        with pytest.raises(FormulaException) as exc_info:
            FormulaEvaluator.evaluate(bad_formula, {})
    ...
    def test_safe_evaluate_logs_warning(self, caplog):
        ...
        with caplog.at_level(logging.WARNING):
            result = FormulaEvaluator.safe_evaluate(bad_formula, {}, default=0)
    ...
    def test_undefined_variable_raises_exception(self):
        ...
        with pytest.raises(FormulaException) as exc_info:
            FormulaEvaluator.evaluate("unknown_var * 2", {'x': 1})
    ...
    def test_valid_formula_no_warning(self, caplog):
        ...
        with caplog.at_level(logging.WARNING):
            result = FormulaEvaluator.evaluate("sqrt(16) + 2", {'x': 5})
...
class TestFormulaSystemValidation:
    ...
    def test_validate_formula_syntax_valid(self):
        ...
        for formula in valid_formulas:
            errors = FormulaEvaluator.validate(formula, ['x', 'y'])
    ...
    def test_validate_formula_syntax_error(self):
        ...
        for formula in invalid_formulas:
            errors = FormulaEvaluator.validate(formula, ['x'])
    ...
    def test_validate_formula_blocks_dangerous_functions(self):
        ...
        for formula in dangerous:
            errors = FormulaEvaluator.validate(formula, [])
    ...
    def test_validate_logs_on_dangerous_attempt(self, caplog):
        ...
        import logging
        with caplog.at_level(logging.WARNING):
            errors = FormulaEvaluator.validate('__import__("os")', [])

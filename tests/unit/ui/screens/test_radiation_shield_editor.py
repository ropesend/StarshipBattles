"""PROJ-458 Phase 5 (F-C-017): RadiationShieldEditor retrofit characterization."""
...
from game.ui.screens.radiation_shield_editor import (
    DefaultRadiationShieldEditorUiBuilder,
    RadiationShieldEditor,
    RadiationShieldEditorUiBuilder,
)
...
def _stub_planet(
    name: str = "P1",
    magnetic_field: float = 0.3,
    radiation_shielding: float = 0.6,
) -> SimpleNamespace:
...
def _build_bypassed(
    *, ui_builder=None,
    ...
    with bypass_init(RadiationShieldEditor):
        return RadiationShieldEditor(
...
class TestConstruction:
    def test_bypass_init_yields_instance_with_stage_one_state(self) -> None:
        ...
        assert window.magnetic_field == 0.45
        assert window.current_shielding == 0.82
        assert window._window_init_bypassed is True

    def test_bypass_init_does_not_invoke_ui_builder(self) -> None:
        mock_builder = MagicMock(spec=RadiationShieldEditorUiBuilder)
    ...
    def test_default_ui_builder_is_thin_wrapper_around_build_ui(self) -> None:
        ...
        DefaultRadiationShieldEditorUiBuilder().build(window)
        window._build_ui.assert_called_once()


class TestCharacterization:
    def test_constructor_signature_preserved(self) -> None:
        sig = inspect.signature(RadiationShieldEditor.__init__)

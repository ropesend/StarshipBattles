...
# ---------------------------------------------------------------------------
# Tests: InteractionController routes through ViewModel
# ---------------------------------------------------------------------------

class TestInteractionControllerUsesViewModel:
    """Verify the bug fix: IC must call viewmodel.pick_up_component, not ship.remove_component."""

    def _make_builder_mock(self):
        """Create a mock builder (DesignWorkshopScreen substitute)."""
        builder = MagicMock()
        builder.viewmodel = MagicMock()
        builder.viewmodel.ship = MagicMock()
        return builder

    def test_pick_up_calls_viewmodel_not_ship(self):
        """On second-click (pick-up), IC must use viewmodel.pick_up_component."""
        import pygame
        from game.ui.screens.builder.interaction_controller import InteractionController

        builder = self._make_builder_mock()
        view = MagicMock()
        view.rect = MagicMock()
        view.rect.collidepoint.return_value = True

        fake_comp = MagicMock()
        fake_comp.id = "laser"
        found = (LayerType.OUTER, 0, fake_comp)
        view.get_component_at.return_value = found

        builder.detail_panel = MagicMock()
        builder.detail_panel.rect.collidepoint.return_value = False
        builder.ship = MagicMock()

        # Set up pick_up_component to return the component
        builder.viewmodel.pick_up_component.return_value = fake_comp

        ctrl = InteractionController(builder, view)
        # First click: select
        ctrl.selected_component = found

        # Second click on same component: pick-up
        event = MagicMock()
        event.type = pygame.MOUSEBUTTONDOWN
        event.button = 1
        event.pos = (100, 100)

        with patch('pygame.key.get_pressed', return_value={pygame.K_LALT: False, pygame.K_RALT: False}):
            ctrl.handle_event(event)

        # viewmodel.pick_up_component must have been called
        builder.viewmodel.pick_up_component.assert_called_once_with(LayerType.OUTER, 0)
        # ship.remove_component must NOT have been called directly
        builder.ship.remove_component.assert_not_called()

...
class TestCharacterization:
    def test_constructor_signature(self) -> None:
        """Post-PROJ-470 + PROJ-458 signature is
        ``(rect, manager, *, window_manager, on_close_callback=None,
        ui_builder=None)``. ``rect`` and ``manager`` remain
        positional-or-keyword; everything else is kw-only."""
        sig = inspect.signature(SettingsWindow.__init__)
        params = sig.parameters
        assert list(params)[:3] == ["self", "rect", "manager"]
        for name in ("rect", "manager"):
            assert params[name].kind in (
                inspect.Parameter.POSITIONAL_OR_KEYWORD,
                inspect.Parameter.POSITIONAL_ONLY,
            )
        # PROJ-470: window_manager is kw-only and required (no default).
        assert params["window_manager"].kind is inspect.Parameter.KEYWORD_ONLY
        assert params["window_manager"].default is inspect.Parameter.empty
        # on_close_callback is preserved but is now kw-only with a None default.
        assert params["on_close_callback"].kind is inspect.Parameter.KEYWORD_ONLY
        assert params["on_close_callback"].default is None

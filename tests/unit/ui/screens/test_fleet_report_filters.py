...
PROJ-494 T2.10: `make_mock_ship` factory hoisted to `tests/fixtures/ship_mocks.py`.
"""
import pytest
...
# PROJ-494 T2.10: shared factory used to live here as a 98-line module-level
# helper; now imported from the shared fixtures module.
from tests.fixtures.ship_mocks import make_mock_ship
...
class TestHasWarpCapability:
    ...
    PROJ-494 T3.13: 5 mass/warp_tonnage tests collapsed into 1 parametrized.
    """

    @pytest.mark.parametrize(
        "mass,warp_tonnage,expected",
        [
            pytest.param(1000, None, False, id='no_warp_drive'),
            pytest.param(1000, 1500, True, id='sufficient_warp_drive'),
            pytest.param(1000, 1000, True, id='equal_warp_tonnage'),
            pytest.param(1000, 500, False, id='insufficient_warp_drive'),
            pytest.param(0, 1000, False, id='zero_mass'),
        ],
    )
    def test_has_warp_capability(self, mass, warp_tonnage, expected):
        """has_warp_capability returns True iff warp_tonnage >= mass and mass > 0."""
        from game.strategy.services.component_abilities import has_warp_capability

        ship = make_mock_ship(mass=mass, warp_tonnage=warp_tonnage)
        assert has_warp_capability(ship) is expected

...
class TestFormationCore:
    ...
    def test_add_arrow(self):
        ...
        assert len(self.core.arrows) == 1
        assert self.core.arrows[0] == [150, 200]
        assert len(self.core.arrow_attrs) == 1
        assert self.core.selected_indices == {0}
    ...
    def test_move_arrow(self):
        ...
        assert self.core.arrows == [[1,1], [2,2], [0,0]]
        assert self.core.arrow_attrs == [{'n':1}, {'n':2}, {'n':0}]
        # Selection should follow the moved item (new index 2)
        assert self.core.selected_indices == {2}

        # Move index 2 (A) back to 0 -> A, B, C
        self.core.move_arrow(2, 0)
        assert self.core.arrows == [[0,0], [1,1], [2,2]]
    ...
    def test_delete_selected(self):
        ...
        assert self.core.arrows == [[0,0], [2,2]]  # 1 removed
        assert self.core.arrow_attrs == [{'n':0}, {'n':2}]
        assert self.core.selected_indices == set()
    ...
    def test_generate_shape_circle(self):
        ...
        assert len(self.core.arrows) == 8

        expected_radius = 200
        for x, y in self.core.arrows:
            dist = math.hypot(x - center[0], y - center[1])
            assert dist == pytest.approx(expected_radius, abs=1.0)
    ...
    def test_generate_shape_disc(self):
        ...
        assert len(self.core.arrows) == 10
        radius = 200
    ...
    def test_generate_shape_x(self):
        ...
        assert avg_x == pytest.approx(0, abs=5.0)
    ...
    def test_load_from_file(self):
        ...
        with patch('builtins.open', mock_open(read_data=json.dumps(sample_data))):
            ...
            assert len(self.core.arrows) == 2
            assert self.core.arrows[0] == [50, 60]
            assert self.core.arrows[1] == [70, 80]
            assert self.core.arrow_attrs[0]['rotation_mode'] == 'relative'
            assert self.core.arrow_attrs[1]['rotation_mode'] == 'fixed'
    ...
    def test_toggle_rotation_mode(self):
        ...
        assert self.core.arrow_attrs[0]['rotation_mode'] == 'fixed'
        assert self.core.arrow_attrs[1]['rotation_mode'] == 'relative'  # Unchanged

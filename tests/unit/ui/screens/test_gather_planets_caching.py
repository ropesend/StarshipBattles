...
def test_gather_planets_second_call_returns_cached_list(
    ...
    first = gather_planets(galaxy, empire, facade_state=fake_state)
    second = gather_planets(galaxy, empire, facade_state=fake_state)
...
def test_gather_planets_cache_isolated_per_empire(
    ...
    session, galaxy, empires = smoke_turn1_scenario
    e0_planets = gather_planets(galaxy, empires[0], facade_state=fake_state)
    e1_planets = gather_planets(galaxy, empires[1], facade_state=fake_state)
...
def test_gather_planets_invalidate_all_drops_cache(
    ...
    session, galaxy, empires = smoke_turn1_scenario
    first = gather_planets(galaxy, empires[0], facade_state=fake_state)
    fake_state.invalidate_all()
    second = gather_planets(galaxy, empires[0], facade_state=fake_state)
...
def test_gather_planets_no_facade_state_is_uncached(
    ...
    session, galaxy, empires = smoke_turn1_scenario
    first = gather_planets(galaxy, empires[0])
    second = gather_planets(galaxy, empires[0])
    assert second is not first
...
def test_gather_stars_second_call_returns_cached_list(
    ...
    """Second call within the same turn returns the same list object."""
    session, galaxy, empires = smoke_turn1_scenario
    first = gather_stars(galaxy, facade_state=fake_state)
    second = gather_stars(galaxy, facade_state=fake_state)
    assert second is first
...
def test_gather_stars_no_facade_state_is_uncached(
    ...
    session, galaxy, empires = smoke_turn1_scenario
    first = gather_stars(galaxy)
    second = gather_stars(galaxy)
    assert second is not first

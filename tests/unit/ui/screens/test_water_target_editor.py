"""PROJ-458 Phase 4 (F-C-017): WaterTargetEditor retrofit characterization."""
...
from game.ui.screens.water_target_editor import (
    DefaultWaterTargetEditorUiBuilder,
    WaterTargetEditor,
    WaterTargetEditorUiBuilder,
)
from tests.fixtures.ui_widget_factory import bypass_init


def _stub_planet(name: str = "P1", surface_water: float = 0.71) -> SimpleNamespace:
    return SimpleNamespace(id=1, name=name, surface_water=surface_water)


def _build_bypassed(
    *, ui_builder=None, surface_water: float = 0.71,
    on_apply_callback=None, on_close_callback=None, race_config=None,
) -> WaterTargetEditor:
    with bypass_init(WaterTargetEditor):
        return WaterTargetEditor(
            pygame.Rect(0, 0, 600, 400),
            MagicMock(name="ui_manager"),
            _stub_planet(surface_water=surface_water),
...
class TestConstruction:
    ...
    def test_default_ui_builder_is_thin_wrapper_around_build_ui(self) -> None:
        ...
        DefaultWaterTargetEditorUiBuilder().build(window)
        window._build_ui.assert_called_once()


class TestCharacterization:
    def test_constructor_signature_preserved(self) -> None:
        sig = inspect.signature(WaterTargetEditor.__init__)

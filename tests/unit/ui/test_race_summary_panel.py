...
class TestFeat14RegistryDrivenSummary:
    ...
    def _refresh_with_mocked_uilabel(self, race_config, monkeypatch=None):
        ...
        """
        # PROJ-491 Task 2.7 + PROJ-494 T2.11 (Batch 4↔5 merge note,
        # 2026-05-24): ``RaceSummaryPanel`` is a plain class, not a
        # ``UIWindow`` subclass, so the canonical ``bypass_init`` context
        # manager does not apply. PROJ-491 T2.7 consolidated the previously
        # in-place ``__new__`` + 12-attribute wiring into a single helper
        # to keep the path explicit; PROJ-494 T2.11 then folded the helper
        # AND the surrounding mocked-pygame_gui patch context into the
        # module-level ``_make_panel_with_mocked_pygame_gui`` factory. The
        # Batch 5 helper is the broader of the two consolidations and is
        # kept here; Batch 4's intermediate stub helper became orphan
        # after the merge and was removed.

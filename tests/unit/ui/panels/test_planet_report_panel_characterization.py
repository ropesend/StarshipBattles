...
class TestConstructionLayout:
    ...
    def _construct_with_pygame_gui_patched(
        ...
        QA-OBS-A: the `show_staged_units` kwarg / staged-units side column
        was removed; staging-yard contents now render inline in
        `format_planet_info`. Only `show_complexes` remains.
        """
        manager = MagicMock()

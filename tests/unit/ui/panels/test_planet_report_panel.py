...
# ---------------------------------------------------------------------------
# QA-OBS-4: staged-units list on PlanetReportPanel.
#
# The planet's `staging_yard` holds completed fighter / mine / satellite /
# drop-pod output until the player moves it into a fleet. Before this fix
# nothing in the planet view surfaced these items — they were only visible
# inside the Transfer dialog, which is one panel-open and a dialog-open away
# from the planet view. The user's QA session confirmed they had no way to
# tell whether anything had been produced.
# ---------------------------------------------------------------------------


class TestStagedUnitsList:
    """Test that PlanetReportPanel renders staged units when present."""

    def _bypass_panel(self):
        from game.ui.panels.planet_report_panel import PlanetReportPanel
    ...
    def test_update_staged_units_renders_grouped_entries(self):
        """When the planet has staged units, `_update_staged_units_list`
        creates one UILabel per (name, vehicle_type) group with a count
        suffix. 12 identical Mk1 Mines render as a single 'Mk1 Mine x12'
        label, not 12 separate rows."""
        from game.ui.panels.planet_report_panel import PlanetReportPanel

        panel = self._bypass_panel()
        panel.staged_units_container = MagicMock()
        planet = MagicMock()
        planet.staging_yard = [
            {"name": "Mk1 Mine", "vehicle_type": "mine", "mass": 5.0},
            {"name": "Mk1 Mine", "vehicle_type": "mine", "mass": 5.0},
            {"name": "Mk1 Mine", "vehicle_type": "mine", "mass": 5.0},
            {"name": "Razor Fighter", "vehicle_type": "fighter", "mass": 12.0},
        ]
        panel.planet = planet
        # Track UILabel constructor calls
        with patch("game.ui.panels.planet_report_panel.UILabel") as ulabel:
            panel._update_staged_units_list()
            # One label per group (2 groups: Mk1 Mine x3, Razor Fighter x1)
            label_texts = [c.kwargs.get("text", "") for c in ulabel.call_args_list]
            assert any("Mk1 Mine x3" in t for t in label_texts), label_texts
            assert any("Razor Fighter" in t for t in label_texts), label_texts

    def test_update_staged_units_shows_none_when_empty(self):
        """Empty staging yard renders a 'None' placeholder label, mirroring
        the complexes-list contract."""
        panel = self._bypass_panel()
        panel.staged_units_container = MagicMock()
        planet = MagicMock()
        planet.staging_yard = []
        ...
        with patch("game.ui.panels.planet_report_panel.UILabel") as ulabel:
            panel._update_staged_units_list()
            label_texts = [c.kwargs.get("text", "") for c in ulabel.call_args_list]
            assert any(t == "None" for t in label_texts), label_texts

    def test_update_staged_units_noop_without_container(self):
        """If the container was never built (show_staged_units=False at
        construction), the update method is a no-op and does not crash."""
        panel = self._bypass_panel()
        panel.staged_units_container = None
        planet = MagicMock()
        planet.staging_yard = [{"name": "X", "vehicle_type": "mine", "mass": 1.0}]
        panel.planet = planet
        # Should not raise and should not try to render anything
        with patch("game.ui.panels.planet_report_panel.UILabel") as ulabel:
            panel._update_staged_units_list()
            assert ulabel.call_count == 0

    def test_update_staged_units_handles_missing_staging_yard_attribute(self):
        """Planets without `staging_yard` (uncolonized? DTO fallback?) must
        not crash the panel — treat as empty."""
        panel = self._bypass_panel()
        panel.staged_units_container = MagicMock()
        planet = MagicMock(spec=[])  # no staging_yard attribute
        panel.planet = planet
        # Should not raise — UILabel patched to avoid real pygame_gui
        # container validation against a MagicMock container.
        with patch("game.ui.panels.planet_report_panel.UILabel"):
            panel._update_staged_units_list()

    def test_singleton_group_renders_without_count_suffix(self):
        """Single instance: just the name, no 'x1' suffix - matches the
        complexes-list convention."""
        panel = self._bypass_panel()
        panel.staged_units_container = MagicMock()
        planet = MagicMock()
        planet.staging_yard = [
            {"name": "Solo Sat", "vehicle_type": "satellite", "mass": 3.0},
        ]
        panel.planet = planet
        with patch("game.ui.panels.planet_report_panel.UILabel") as ulabel:
            panel._update_staged_units_list()
            label_texts = [c.kwargs.get("text", "") for c in ulabel.call_args_list]
            assert any(t == "Solo Sat" for t in label_texts), label_texts
            assert not any("x1" in t for t in label_texts), label_texts

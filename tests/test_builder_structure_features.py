...
from components import Component, MODIFIER_REGISTRY, ApplicationModifier
...
class TestBuilderStructureFeatures(unittest.TestCase):
    def setUp(self):
        ...
        # Mock Builder GUI
        with patch('builder_gui.BuilderLeftPanel'), \
             patch('builder_gui.BuilderRightPanel'), \
             patch('builder_gui.LayerPanel'), \
             patch('builder_gui.ModifierEditorPanel'), \
             patch('builder_gui.WeaponsReportPanel'):
    ...
    def test_add_remove_actions(self):
        ...
        # Mock VALIDATOR
        with patch('ship.VALIDATOR') as mock_val:
